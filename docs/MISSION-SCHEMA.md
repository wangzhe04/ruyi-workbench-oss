# Mission Schema 冻结文书 v1.1(2026-07-30,第74波交付,对齐 PLAN v4)

> 2026-09-11（第 121 波 K1，34 号文 §8.1）：原名 `PRETENDER-SCHEMA.md`。交办台壳层已整层退役，
> **但本文书定义的 Mission / Intervention 读写模型仍是在用的数据契约** —— `/api/missions`、
> `/api/interventions`、`src/13e-pretender-index.js` 的投影索引、管家视角的看板与抽屉全靠它。
> 故改名保留，不归档。后端的 `13e-pretender-index.js` 本波【不】改名（拍板⑤，改名会漂
> `durable-state-inventory` 与 route 产物，已记债）。

状态:**冻结 v1.1**。本文件是 Mission / Intervention 全部读/写模型的**字段级权威定义**,与 `./archive/PRETENDER-PLAN.md` **v4** 配套。v1.1 为 Escapade 2.4 交互提问协议的只增不改修订:冻结 question typed payload、选项+自填答案和旧 `multiSelect`/`answer[]` 兼容映射,不改变 Intervention 状态机:
...
依据:`./archive/PRETENDER-PLAN.md` v4 §4 第74波;`docs/archive/PRETENDER-GATE-REVIEW.md`;`docs/UI-VNEXT-CONCEPT.md` §8–§11。
