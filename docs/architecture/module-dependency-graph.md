...
| 2 | `02-session-store.js` | foundation | 170 | 40 | 11 |
| 3 | `03-bridge-guard.js` | foundation | 71 | 20 | 5 |
| 4 | `04-permission-runtime.js` | foundation | 98 | 28 | 7 |
| 5 | `05-claude-engine.js` | engine | 23 | 93 | 12 |
| 6 | `05b-kimi-bridge.js` | engine | 119 | 62 | 12 |
| 7 | `05c-kimi-search-policy.js` | engine | 42 | 11 | 2 |
| 8 | `05d-kimi-prompt-parts.js` | engine | 15 | 5 | 3 |
| 9 | `06-provider-engine.js` | engine | 94 | 40 | 9 |
| 10 | `06b-prompt-registry.js` | engine | 4 | 1 | 1 |
| 11 | `06c-agent-loop-hooks.js` | engine | 1 | 3 | 2 |
| 12 | `07-autonomy.js` | orchestration | 220 | 66 | 13 |
| 13 | `08-agent-runs.js` | orchestration | 87 | 78 | 13 |
| 14 | `09-workflow.js` | orchestration | 23 | 174 | 13 |
| 15 | `10-context-governance.js` | orchestration | 80 | 59 | 10 |
| 16 | `11-native-tools.js` | tools | 85 | 22 | 5 |
| 17 | `12-tool-dispatch.js` | tools | 9 | 67 | 8 |
| 18 | `13-http-router.js` | transport | 34 | 199 | 17 |
| 19 | `13b-api-domain-routes.js` | transport | 4 | 37 | 7 |
| 20 | `13c-overlay-routes.js` | transport | 11 | 12 | 3 |
| 21 | `13d-core-domain-routes.js` | transport | 17 | 80 | 9 |
| 22 | `13e-pretender-index.js` | transport | 36 | 24 | 7 |
| 23 | `14-main.js` | entrypoint | 1 | 344 | 17 |
...
| `01-config.js` | `10-context-governance.js` | forward | `runProcess` |
...
| `02-session-store.js` | `07-autonomy.js` | forward | `activeAgentRuns`, `agentRunDir`, `revokeAllGrants` |
| `02-session-store.js` | `08-agent-runs.js` | forward | `appendAgentRunEvent`, `bumpRunIntervention`, `listAgentRuns`, `saveAgentRun` |
| `02-session-store.js` | `10-context-governance.js` | forward | `runProcess` |
...
| `04-permission-runtime.js` | `07-autonomy.js` | forward | `legacyAccMemoryMigrationComplete` |
...
| `05-claude-engine.js` | `07-autonomy.js` | forward | `buildClaudeAgentDefinitions`, `buildMemoryCheckPrompt`, `buildMemoryConflictMap`, `buildMemoryPromptSection`, `buildMissionPromptSection`, `classifyToolPacks`, `memoryGlobalDir`, `memoryProjectDir`, `resolveMemoryPreflight` |
...
| `05d-kimi-prompt-parts.js` | `04-permission-runtime.js` | backward | `IMAGE_ATTACH_MAX` |
...
| `06-provider-engine.js` | `07-autonomy.js` | forward | `buildMemoryCheckPrompt`, `buildMemoryPromptSection`, `buildMissionPromptSection`, `buildResponsesInputItems`, `toolPackForName` |
...
| `07-autonomy.js` | `00-boot.js` | backward | `SKILL_ID_RE`, `URL`, `appendUsageLedger`, `cachedInputTokensFromUsage`, `claudeCostFields`, `computeProviderCost`, `cp`, `crypto`, `fs`, `fsp`, `makeId`, `nowIso`, `os`, `path`, `paths`, `safeJsonParse`, `text` |
| `07-autonomy.js` | `01-config.js` | backward | `BUILTIN_AGENT_ROLES`, `CLAUDE_PERMISSION_MODE_MAP`, `atomicWriteJson`, `batchSafeSpawn`, `buildUserEnvelope`, `cmdLineBudgetFor`, `decodeClaudeCliText`, `detectClaudePath`, `effectiveAnthropicEnv`, `generateAgentNodeMcpConfig`, `mergeAgentRole`, `normalizeAgentRole`, `readConfig`, `safeSessionId`, `spawnCmdLineLength` |
| `07-autonomy.js` | `02-session-store.js` | backward | `BRIDGED_WRITE_PATH_ARGS`, `collectBridgedWriteTargets`, `loadSession`, `registerIntervention`, `saveSession`, `settleIntervention`, `unprefixedBridgedName` |
| `07-autonomy.js` | `03-bridge-guard.js` | backward | `existsExecutable`, `isSensitiveDataPath`, `normalizeCwd`, `pathWithinRoot` |
| `07-autonomy.js` | `04-permission-runtime.js` | backward | `activeChildren`, `killChildTree`, `logEvent`, `parseClaudeEvent`, `pendingPermissions`, `pendingPlans`, `redact`, `resolveBridge`, `runAutomaticInterventionDecision` |
| `07-autonomy.js` | `05-claude-engine.js` | backward | `activeOpenAiProvider`, `providerBaseWithV1` |
| `07-autonomy.js` | `06-provider-engine.js` | backward | `appendResponseLanguagePolicy`, `providerRawCompletion`, `toolRequirementsMet` |
| `07-autonomy.js` | `06b-prompt-registry.js` | backward | `getPromptPack` |
...
| `07-autonomy.js` | `11-native-tools.js` | forward | `globToRegExp` |
...
| `08-agent-runs.js` | `07-autonomy.js` | backward | `acquireResourceLease`, `activeAgentRuns`, `agentRunDir`, `agentRunFile`, `agentRunWriteChains`, `bridgedToolTier`, `buildOpenAiTools`, `classifyToolPacks`, `fetchOpenAiModels`, `inferToolResources`, `loopAbortExempt`, `loopWarnOnly`, `nativeToolGate`, `nativeToolTier`, `neutralizeInjectedPrefixes`, `normalizeAgentResources`, `openAiStreamOnce`, `releaseResourceLease`, `runClaudeSubAgentOnce`, `toResponsesTools`, `toolPackForName` |
| `08-agent-runs.js` | `09-workflow.js` | forward | `estimateContentTokens`, `estimateHistoryTokens`, `launchPersistedAgentRun` |
| `08-agent-runs.js` | `10-context-governance.js` | forward | `COMPACT_RESEED_TAIL_MAX_TOKENS`, `evaporateHistory`, `maybeCompactSubHistory`, `noteWindowOvershoot`, `providerContextWindow`, `providerSummaryCall`, `recentTurnsBoundary`, `recordCompactUsage`, `truncateToolResult` |
...
| `09-workflow.js` | `02-session-store.js` | backward | `applyMissionUpdate`, `bridgedWriteRelativePathArg`, `buildTurnSummary`, `bumpMissionChangeSeq`, `captureWorkspaceTurnBaseline`, `createTurnSegmentBuilder`, `finalizeMissionAfterTurn`, `isUntitledSessionTitle`, `journalReadIndex`, `loadSession`, `normalizeTodoItems`, `reconcileWorkspaceTurnBaseline`, `registerIntervention`, `repairProviderHistoryPairing`, `saveSession`, `settleIntervention` |
...
| `09-workflow.js` | `04-permission-runtime.js` | backward | `activeChildren`, `buildUserContentParts`, `clearPendingPermissions`, `clearPendingPlans`, `clearPendingQuestions`, `collectBridgedTools`, `extractToolImages`, `getBridgedClient`, `hasImageAttachment`, `hasPendingQuestionForSession`, `lastAssistantEngine`, `logEvent`, `pruneOldImages`, `redact`, `requestUserQuestion`, `resolveBridge`, `stopSession`, `stripToolImageFields` |
...
| `09-workflow.js` | `07-autonomy.js` | backward | `ACTION_VIEW_MIN_CHARS`, `ACTION_VIEW_TOOLS`, `MAIL_GLOBAL_MAX`, `MAIL_PER_SENDER_MAX`, `MAIL_QUEUE_MAX`, `MAIL_TEXT_MAX`, `POOL_CHAIN_MAX`, `POOL_GRACE_MS`, `POOL_MAX_TOTAL`, `acquireResourceLease`, `actionTargetMeta`, `activeAgentRuns`, `agentRunFile`, `bridgedToolTier`, `buildMemoryCheckPrompt`, `buildMemoryConflictMap`, `buildMemoryPromptSection`, `buildOpenAiTools`, `classifyRuntimeToolFailure`, `cleanupAgentWorktree`, `compareToolRetrievalShadow`, `consumeGrant`, `createAgentWorktree`, `createToolLoadingState`, `drainSteerQueue`, `estimateToolSchemaTokens`, `extractMemoryRelationProposals`, `failoverStickyBase`, `finalizeAgentWorktree`, `getAgentRoleLibrary`, `inferToolResources`, `looksLikePlan`, `loopAbortExempt`, `loopWarnOnly`, `nativeToolGate`, `nativeToolTier`, `normalizeAgentResources`, `openAiStreamOnce`, `projectActionModelView`, `proposeMemoryRelation`, `releaseResourceLease`, `remapAgentResources`, `requestNativePermission`, `requestPlanApproval`, `resolveMemoryPreflight`, `resumeInFlight`, `toResponsesTools` |
...
| `09-workflow.js` | `10-context-governance.js` | forward | `COMPACT_RESEED_TAIL_MAX_TOKENS`, `contextWindowFromTable`, `evaporateHistory`, `isContextOverflowError`, `maybeAutoCompact`, `measureObservationReductionShadow`, `noteEstimateSample`, `noteWindowOvershoot`, `providerContextWindow`, `providerConversationContextWindow`, `providerSummaryCall`, `recentTurnsBoundary`, `recordCompactUsage`, `truncateToolResult`, `upsertCompactMarker`, `writeHistorySnapshot` |
| `09-workflow.js` | `13-http-router.js` | forward | `buildModelHint`, `resolveNodeModel` |
| `10-context-governance.js` | `00-boot.js` | backward | `URL`, `appendUsageLedger`, `cachedInputTokensFromUsage`, `computeProviderCost`, `cp`, `crypto`, `fs`, `fsp`, `makeId`, `nowIso`, `os`, `path`, `paths`, `text`, `zlib` |
| `10-context-governance.js` | `01-config.js` | backward | `DurableJsonStore`, `PERMISSION_MODES`, `batchSafeSpawn`, `readConfig`, `readJsonBody` |
| `10-context-governance.js` | `02-session-store.js` | backward | `MISSION_MAX_TEXT`, `MISSION_STALL_LIMIT`, `configForSessionEngineRoute`, `createSession`, `evaluateMissionCheck`, `inferSessionEngineRoute`, `journalBytesAdjust`, `journalDir`, `journalGc`, `loadSession`, `maybeFinalizeMission`, `missionProgressDigest`, `saveSession`, `withJournalWriteLock` |
| `10-context-governance.js` | `03-bridge-guard.js` | backward | `normalizeCwd` |
| `10-context-governance.js` | `04-permission-runtime.js` | backward | `driverAutoSessions`, `killChildTree`, `logEvent`, `redact`, `stopSession`, `turnSettlers` |
...
| `10-context-governance.js` | `07-autonomy.js` | backward | `activeDriverRuns`, `bindDriverRun`, `buildResponsesInputItems`, `fetchOpenAiModels`, `revokeGrantsForRun` |
...
| `12-tool-dispatch.js` | `07-autonomy.js` | backward | `bridgedToolTier`, `compareToolRetrievalShadow`, `listCompactTools`, `listWorkbenchMemories`, `proposeMemoryRelationRevoke`, `proposeMemoryRelationTool`, `proposeMemoryRevision`, `proposeWorkbenchMemory`, `readWorkbenchMemory`, `searchToolCatalog` |
...
| `13-http-router.js` | `07-autonomy.js` | backward | `CORE_MEMORY_CHAR_CAP`, `CORE_MEMORY_MAX`, `MEMORY_EXCLUSION_MAX`, `MEMORY_MAX`, `activeAgentRuns`, `activeDriverRuns`, `analyzeMemoryMaintenance`, `applyMemoryRelationProposal`, `autonomyGrants`, `buildClaudeAgentDefinitions`, `confirmMemoryRelation`, `decideMemoryProposal`, `deleteMemory`, `deleteMemoryRelation`, `draftMemoryFromSession`, `dryRunGrantFiles`, `fetchOpenAiModels`, `getAgentRoleLibrary`, `listGrantsView`, `listMemoryProjectGroups`, `listMemoryRelations`, `loadMemoryRegistry`, `migrateLegacyAccMemory`, `migrateMemory`, `normalizeGrant`, `projectAgentRoleFile`, `projectKeyForCwd`, `proposeMemoryFromSession`, `proposeMemoryRelation`, `readClaudeProjectAgentRoles`, `readMemoryItem`, `readProjectAgentRoles`, `resolveCoreMemoryState`, `revokeAllGrants`, `revokeGrant`, `saveMemory`, `saveProjectAgentRoles`, `toolPackForName`, `validateMemoryProposalSave` |
...
| `13-http-router.js` | `10-context-governance.js` | backward | `agentConversationContextMeta`, `cachedContextLength`, `configuredConversationWindow`, `learnedWindowCap`, `pickFile`, `pickFolder`, `resolveContextWindow`, `revealInExplorer`, `runAgentExternalCompact`, `runProcess`, `runProviderCompact`, `streamChat`, `truncateToolResult` |
...
| `13d-core-domain-routes.js` | `07-autonomy.js` | backward | `STEER_QUEUE_MAX`, `activeAgentRuns`, `agentRunFile`, `applyAgentWorktree`, `cleanupAgentWorktree`, `consumeGrant`, `getAgentRoleLibrary`, `nativeToolGate`, `nativeToolTier`, `toolIsRevertible` |
...
| `14-main.js` | `07-autonomy.js` | backward | `NATIVE_TOOL_PACKS`, `NATIVE_TOOL_TIER`, `acquireResourceLease`, `adaptiveMetaToolSchemas`, `agentResourcesConflict`, `analyzeMemoryMaintenance`, `applyAgentWorktree`, `applyMemoryRelationProposal`, `bridgedToolTier`, `buildClaudeAgentDefinitions`, `buildCoreMemoryPromptSection`, `buildMemoryCheckPrompt`, `buildMemoryConflictMap`, `buildMemoryPromptSection`, `buildOpenAiTools`, `buildToolCatalog`, `classifyClaudeSubagentFailure`, `classifyRuntimeToolFailure`, `classifyToolPacks`, `compareToolRetrievalShadow`, `confirmMemoryRelation`, `createAgentWorktree`, `createToolLoadingState`, `deleteMemoryRelation`, `effectiveMemorySelection`, `estimateToolSchemaTokens`, `extractMemoryRelationProposals`, `fetchOpenAiModels`, `finalizeAgentWorktree`, `getAgentRoleLibrary`, `inferToolResources`, `legacyAccMemoryMigrationComplete`, `listMemoryRelations`, `listWorkbenchMemories`, `loadMemoryRegistry`, `memoryProposalIsDuplicate`, `memoryProposalPrefilter`, `memoryProposalSimilarity`, `memorySearchTerms`, `migrateLegacyAccMemory`, `normalizeAgentResource`, `normalizeAgentResources`, `parseMemoryProposalDecision`, `proposeMemoryFromSession`, `proposeMemoryRelation`, `proposeMemoryRelationRevoke`, `proposeMemoryRelationTool`, `proposeMemoryRevision`, `proposeWorkbenchMemory`, `rankRelevantMemories`, `readClaudeProjectAgentRoles`, `readProjectAgentRoles`, `readWorkbenchMemory`, `releaseResourceLease`, `remapAgentResources`, `resolveCoreMemoryState`, `resolveMemoryPreflight`, `resourceBlockers`, `saveMemory`, `saveProjectAgentRoles`, `searchToolCatalog`, `toolPackForName` |
...
| `14-main.js` | `10-context-governance.js` | backward | `COMPACT_MARKER_MIN_SAVED_TOKENS`, `COMPACT_RESEED_TAIL_MAX_TOKENS`, `MODEL_CONTEXT_TABLE`, `agentConversationContextMeta`, `calibratedEstimate`, `chunkHistoryByBudget`, `compactHistoryFromSession`, `configuredConversationWindow`, `contextWindowFromTable`, `contextWindowOverrideKey`, `estimateFactor`, `extractContextLength`, `fitHistoryForSummary`, `isContextOverflowError`, `learnedWindowCap`, `measureObservationReductionShadow`, `noteEstimateSample`, `noteWindowOvershoot`, `openCompactMarker`, `providerContextWindow`, `providerConversationContextWindow`, `providerSummaryCall`, `recentTurnsBoundary`, `reduceObservationContent`, `rehydrateObservation`, `resolveCompactionProvider`, `resolveContextWindow`, `upsertCompactMarker`, `validateStructuredSummary`, `writeHistorySnapshot` |
...
1. `00-boot.js` ↔ `01-config.js` ↔ `02-session-store.js` ↔ `03-bridge-guard.js` ↔ `04-permission-runtime.js` ↔ `05-claude-engine.js` ↔ `05b-kimi-bridge.js` ↔ `05c-kimi-search-policy.js` ↔ `05d-kimi-prompt-parts.js` ↔ `06-provider-engine.js` ↔ `06b-prompt-registry.js` ↔ `06c-agent-loop-hooks.js` ↔ `07-autonomy.js` ↔ `08-agent-runs.js` ↔ `09-workflow.js` ↔ `10-context-governance.js` ↔ `11-native-tools.js` ↔ `13-http-router.js` ↔ `13b-api-domain-routes.js` ↔ `13c-overlay-routes.js` ↔ `13d-core-domain-routes.js` ↔ `13e-pretender-index.js`
