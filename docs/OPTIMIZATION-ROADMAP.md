...
第56波评审 GO(有条件 4 条)后,所有者拍板 Pretender 详细规划,成文 **`docs/PRETENDER-PLAN.md`**(波次路线 P1 第74–75波 → P2 第76–80波 → P3 第81–85波 → P4 第86–88波,共 14 波,各阶段出门闸冻结评审四条件)。三项开口决策定案:

- **D1 · mission.id 主键 = 方案 B**:引入 `missionId` 稳定主键,session 主键/文件布局不动,新会话 `missionId=sessionId` 派生(1:1),存量只读派生不回写;schema/API 从第一天以 missionId 为对外名,为「一单多会话」留 1:N 扩展口。
...
- 其后按 PRETENDER-PLAN 进第74波(P1 schema 冻结文书)。
