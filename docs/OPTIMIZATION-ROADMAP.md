...
| **106 · Agent SoC 证据收敛（推进中）** | #13a／13a-t 已交付默认关（回合 token 预算保护＋长命令时间轴软硬终态＋字节轴 shadow 计数，44 项合成门全绿）；#1 Ruyi 布局、#2a、#3／#9 限定场景待推；#7 仅在回载与耗时证据成立后进入 | 沿用 22 号单轴、非劣、权限／配对／恢复零回归门；不以完成数量冒充收益 |
