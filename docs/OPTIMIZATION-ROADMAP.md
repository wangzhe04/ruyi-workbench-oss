...
> 立项理由（用户 2026-09-03）：现状对新手尤其电脑小白太不友好。摸底：首跑引导卡、14 个 Playbook 任务卡、人话的测试连接错误、292 行新手手册都已存在，但应用内零帮助入口、两壳引导不一致且不可重开、体检与启动错误全是技术话、桌面控制可用性无提示、预设全要手填、node 回落路径露黑窗。方案：118a 壳无关欢迎向导（预设只填 Key、前置校验、测试连接、人话安全档、可重开）→ 118b 人话体检与 desktop-control 健康项 → 118d 应用内帮助路由与菜单 → 118c 启动错误落文件并弹窗、隐藏黑窗、端口自动改用 → 118e Ollama／LM Studio 零配置预设 → 118f 小白走查与离线包实测。详见 [`optimization-plan/28-wave-118-onboarding.md`](optimization-plan/28-wave-118-onboarding.md)。**建议提前到 110 之后立即做**（改动面小、零风险、117 管家可复用同一份步骤定义），待拍板。
