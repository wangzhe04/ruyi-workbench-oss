...
| 线 | 状态 | 依据 |
|---|---|---|
| **Escapade 发布线** | **2.8.0 候选，范围冻结中**。已归档的最后一个发布条目是 `v2.7.0`（2026-09-13，一台两视）；2.7.0 之后第 123–127 波的用户可见变化已写进 `CHANGELOG.md` 的「未发布」节（2.8.0 候选，**发布日期留白，由 107 波 R1／R2 填**）。磁盘 `package.json` 仍是 `2.7.0`（R1 那一刀 bump 版本三角）；`CONFIG_SCHEMA` 自 2.7.0 未变（`11`），**2.8.0 不 bump**、无配置迁移脚本。**本地最新标签是 `v2.6.2`，`v2.7.0` 从未打标签**（R2 那一刀补打；推标签即公开，是用户的决定） | `ruyi-workbench/package.json`、`git tag`、`CHANGELOG.md`、[`optimization-plan/46-wave-107-release-approval.md`](optimization-plan/46-wave-107-release-approval.md) |
