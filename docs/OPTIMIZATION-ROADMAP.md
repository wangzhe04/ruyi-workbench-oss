...
Escapade 2.0 已完成公开发布。以下是从既有 52e 未竟项和已登记体验债整理出的**内部**优先级，不把任何单项预先绑定到 `2.0.1`、`2.1` 或 `2.2`：每次 Release 在范围冻结与质量门通过后再决定版本号。

1. **第53波 · 交付与更新闭环**：实现 overlay 更新 GUI v1（应用内选择 zip、check / preview / verify、可恢复的向导），把现有 `Manage-Overlay` 的离线能力安全地带到产品界面；同时补齐发布物清单、校验和与兼容提示的自动检查。
2. **第54波 · 体验质量收口**：视觉回归门 v2（深浅主题可复现基线与像素/布局比较）、消息增量渲染前置研究（为 `role="log"` 可访问性而不是全量重读）、剩余键盘等价与 CSS 分层收口。所有项保持简易/专业模式和三主题对称。
3. **第55波 · 生态与可维护性**：MCP 管理面板、远程 connector 兼容层评估，以及后端剩余 agent-runs / permission / session 域拆分；继续把接口契约、离线打包和安全护栏一同交付。
4. **第56波 · Pretender 立项门**：只做「交办台」概念验证、迁移边界和双壳成本评估；通过明确的 go / no-go 评审后，才定义 **Ruyi Pretender 3.0** 的范围、兼容策略与时间表。未通过前，Pretender 不是待交付功能清单。

**Escapade 发布准入**：每个候选版本至少完成版本三角（`package.json` / `00-boot.js` / `facts.json`）、构建新鲜度、相关行为/视觉/提示词回归、Full/Slim 离线包完整性与 checksum；真实外部环境 probe 单独标注，不以跳过项伪装为通过。
