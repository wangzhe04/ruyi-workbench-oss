...
| **第 103–107 波架构／上下文前序列** | **推进中**：第 103、104 波已交付（路由治理、依赖契约、39 项持久化清册、职责内聚、`CompactionPlan` 与规则／契约快照）；第 105 波逐项取证中，105a（observation_recall + reducer）已通过真实历史与 DeepSeek 自发采用门并默认开启，105b（session-notes.md 状态外置，只写切片）亦经真实历史门后默认开启，105c（摘要实体确定性抽检）已交付、取证期默认关闭（成本承担行为，flip 判据见其 Release Brief）；106 汇入 Agent SoC 证据，107 为发布批准点 | [`optimization-plan/23-architecture-repayment-sequence.md`](optimization-plan/23-architecture-repayment-sequence.md) |
