...
| **105 · 上下文行为实验** | `observation_recall`、session notes、实体校验、估算分桶、单发优先与 map-reduce 跨块保真 | 逐项受控取证；105a–105f 已通过各自门并默认开启（均可显式 false 回退），其余保持默认关／canary；为 #7 建立生产消费者，不通过不启用 |
