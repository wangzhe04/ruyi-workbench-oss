...
这份方案刻意把“文案资源”和“代码接入”分离。资源表保留在 `docs/i18n/` 作为审校基准，并在应用侧以同内容的 `app/public/locales/` 随包发布；后续实施者只需按键名接入，不能让运行时资源与文档基准漂移。
...
- 已完成：零依赖 `app/public/js/i18n.js`（`t`、`tCount`、占位符插值、语言回退、`data-i18n` / `data-i18n-attr`）。
- 已完成：`locale: auto | zh-CN | en-US` 配置校验、首次按浏览器语言解析并持久化、设置页语言选择和 `<html lang>` 更新。
- 已完成 P0 首批：浏览器标题、侧栏导航、会话搜索、主视图标签、输入区、设置标题/常用标签与无障碍名称。
- 已完成 P1 首批：会话分组/计数/操作、工作区选择与首启拖放区、消息窗口、任务账本、已知错误类别与 CLI 缺失卡片，以及本轮变更、文件操作/预览、撤销反馈、审计时间线、动态授权书、工具权限请求、计划审批卡、工作流启动/执行状态/运行控制/任务池审批/编辑器核心操作和用量摘要/预算反馈。
- 已完成 P1 动态覆盖：Provider 配置卡与价格/上下文窗口、模型切换菜单、权限模式与能力矩阵弹层、产物列表、工具调用摘要、快捷键、命令面板、品牌状态、首启/空状态和技能库均在语言切换时重绘；会话重绘会保留非底部阅读位置。
- 已完成：内置 `source=builtin` 技能、命令与一键任务的名称、说明和一键任务表单字段，以及「存为一键任务」编辑流程由资源表本地化；固定的能力不可用原因对内置、用户和项目条目统一翻译。用户和项目自定义的 SKILL.md/任务内容仍按作者原文显示，避免把自由内容误当产品文案改写。
- 已完成：静态质量门会校验运行时语言包与此目录完全一致、双语键和插值占位符一致，以及运行时与配置归一化的核心行为。
- 已完成 P2 基线：所有 HTTP 错误响应均返回包含 code、params、message 的 error 对象；已为高频鉴权、会话、变更和文件路径错误提供细粒度代码，旧端点自动归一为稳定的通用代码以保持兼容。
- 已完成：伪本地化安全检查会遍历全量英文资源，验证文本膨胀与插值占位符保持；浏览器截图回归待浏览器自动化运行环境恢复后补跑。
- 待完成：其余静态页、未覆盖的动态 UI 文案，以及双语浏览器路径的视觉截图回归。
...
运行时语言包位于 `ruyi-workbench/app/public/locales/`，并必须与此目录保持完全相同。键名以界面语义而非当前 DOM、英文或中文原文命名：
...
后续迁移应以功能域拆分小 PR，禁止把 `app.js` 全文件格式化或做全局字符串替换；每个 PR 只改其负责函数及对应 locale 键。合并顺序建议先落运行时和资源，再逐域迁移调用点。
