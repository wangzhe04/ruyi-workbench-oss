...
#### 3.6 用户拍板的两项调整（2026-09-04）

**一、`runtimeMemoryVectorRecallV1` 默认翻开。** 合成门实测 +5pp，未达 §3.3 写的 +10pp。这个门槛是「自动翻牌线」而不是「否决线」：证据是正收益、零回归、且关掉即逐字节回到旧行为，所以用户可以直接拍板开。门本身不改，它仍然诚实报告实测值。

**二、四个写死的容量常量改为可配并大幅抬高。**

| 旋钮 | 配置键 | 旧值 | 新默认 | 钳位 |
|---|---|---|---|---|
| 核心胶囊席位 | `coreMemoryMaxItemsV1` | 24 | **200** | [0, 2000] |
| 核心胶囊字数 | `coreMemoryCharBudgetV1` | 4200 | **16000** | [0, 200000] |
| 每轮检索条数 | `memoryRelevanceMaxV1` | 3 | **8** | [0, 64] |
| 会话固定选择 | `memoryFixedSelectionMaxV1` | 12 | **64** | [1, 1024] |
| 相关索引字数 | `memoryIndexCharCapV1` | 2600 | **6000** | [500, 100000] |

模块里的同名常量降级为「调用方没传 config 时的兵底」，**且兵底值就是新默认值**——否则同一个记忆库在不同入口会给出不同的席位数。config 已穿到 `resolveCoreMemoryState` / `buildCoreMemoryPromptSection` / `buildMemoryPromptSection` 与两处固定选择上限。

**成本口径（要说清楚）**：核心胶囊在【易变层】，不进前缀缓存，每回合按实际字数付输入 token。但胶囊只装用户**主动标为 core** 的条目，没标就不花钱，所以把天花板抬高本身是安全的；真正的闸门是字符预算。实测：18 条示例核心记忆在旧的 4200 字下只能进一半，新预算下 17 条（1 条已过期不入选）全部入席、共用 7965 字。

**过程中被夹具当场抓到的一个自引入 bug**：钳位函数写成 `Number(config && config[key])`，`config` 为 null 时 `Number(null)` 是 **0**，而 0 是合法钳位值 —— 于是「没传配置」被静默解读成「上限为 0」，整个胶囊消失。已改为先取原值再转数字。（同样写法在 `execResultCacheMaxEntries` 里也存在，是 110-2b 纯搬家带过来的旧代码；它的所有调用点都传了真 config，故为潜伏项，**登记为待办**，本批不顺手改。）

**三件测试从「钉死魔数」改为「验机制」**：`memory-toolbox.static` 另锁「限额必须经读配置的判定函数解析」（否则用户改了配置也不生效）；`workbench-memory-core` 默认预算下断言全入席、再显式传窄预算断言溢出仍只进候补；`workbench-memory` 的截断契约改用窄上限直接验函数，不再依赖「夹具息好能溢出」这个脆弱前提。

**出门**：串行全量 269 pass / 0 fail / 1 flaky（`mission-result`，同样是 109 波起就在名单里的既有偶发件），快通道 53/53。
