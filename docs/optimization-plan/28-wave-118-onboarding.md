...
- **118f 走查（2026-09-04，主会话亲跑）**：干净临时数据目录起真服务，逐面核对本波全部交付。**通过**：首跑卡「开始引导」＋副标题「六步、几分钟，配好就能干活；中途可以随时退出」；侧栏帮助菜单五项（使用手册／管理员手册／重新打开引导／查看日志／打开数据目录）全为可点动作、**零路径文本**；手册在应用内打开，左侧七节目录、右侧正文、中英切换齐备；体检面板完成人话化——「运行方式·从源码目录运行，改动可以直接生效」「工具接入·已经准备好把工作台的工具交给 AI 使用」「排版组件·回复里的表格、代码块可以正常排版」，原始 id／detail 收进「技术详情」折叠区；**新健康项「桌面控制·已就绪，AI 可以用 104 个桌面工具替你操作电脑」**直接回答了「AI 能不能操作我的电脑」；向导第 3 步六张预设卡含两张新的本机免 Key 卡（Ollama／LM Studio）。**发现并当场修掉一处**：向导面板 `.modal.onboard-wizard` 沿用浮层玻璃档，计算样式实测背景仅 `rgba(158,178,224,0.086)`，背后首跑卡大号标题整段透上来（118a 走查已记，118f 复现并用计算样式确认非动画残留）；按本层已有的 `.modal.help-viewer`／`.modal.help-logs-modal` 先例改为 `background: var(--panel)` ＋ `backdrop-filter: none`，修后计算样式 `rgb(21,29,43)` 实底，D51 CSS 载荷 SHA 同步重钉并写明理由。
- **118f 未能覆盖（诚实登记）**：离线包（Full／Slim）解压后双击实测未做——需要现场构建含 ACC 与 winsdk wheel 的发行件，属打包线工作量，建议并入下一次发布前的打包演练；桌面壳 WebView2 降级弹窗只做到编译通过与静态锁，未运行验证（本机 WebView2 可用，走不到降级路）；启动提示条目前只在经典壳呈现，预览壳是否镜像留待管家壳（117）统一处理。

- **118 出门回归（2026-09-04）**：串行全量 `run-all.js` = **265 pass／1 fail／1 flaky**（266 ran／7 skipped；端口审计 25 个带内端口零撞车、unit 全过、构建新鲜）。唯一失败 `interventions-snapshot` 的签名是 `auth.token_invalid` 403，**单跑连续三次 ALL PASS**；该件在第 109 波（早于本波所有改动）与第 110 波的回归里已多次进入偶发名单，判定为**既有 flaky，非 118 回归**。**第 118 波宣告出门。**
- **交给 107 前置偶发治理的线索（2026-09-04）**：`interventions-snapshot` 的失败签名是「先若干请求成功、随后统一 403 `auth.token_invalid`」，指向 token 绑定被中途改写或客户端连到了另一实例，比单纯超时更值得优先查。同时评估 118c 端口回退的副作用——`killPortOnStart:false` 或占用者非工作台时现在会**静默改用 port+1**，对普通用户是好事，但会让「按端口连接」的调用方（测试夹具、外部脚本）在原端口连到别的实例；建议给回退加显著日志并把实际端口写进可断言的位置，供夹具校验。
...
- **118b（2026-09-03，opus 实现／主会话亲核）**：新增 `js/health-i18n.js` 纯函数映射（`describeHealthItem` → label／hint／next／severity，覆盖 `computeHealth` 全部 id ＋未知 id 兜底；`next` 文案零路径零命令，由静态锁断言，指向应用内目的地）；`renderDoctor` 重写为「状态灯＋人话名称＋一句话现状＋状态药丸＋怎么办按钮＋折叠技术详情」，原始 id／detail 只留折叠区；红点摘要落首跑卡与侧栏设置按钮；新健康项 **`desktop-control`** 六态（`ready`／`disabled`／`not-installed`／`python-missing`／`preparing`／`unreachable`），只用 `config.desktopMcp` ＋ `peekCapabilities()`（只读缓存不触发探测）＋外部 MCP 登记＋一次 `existsSync`，条目形状与响应信封零新增；**未读 `acc-install-latest.log`**——工作台源码不知道该路径，为它新造常量属跨组件耦合，裁决记在静态锁 G2；CLI `doctor` 默认 JSON 逐字节不变，新增 `--human`。**顺带修复 HEAD 既有缺陷**：`navigation-controls.js:770` 的 `openModal('settingsModal')` 调用了从未注入的自由标识符 `fillSettings`，每次程序化打开设置都抛 `ReferenceError` 并吞掉其后语句（包括 118a 首跑卡「去配置」的页签切换）；组合根补注入并给 `provider-settings` 注入 `switchSettingsTab`，`app.js` 行数护栏按 SPEC §6 重钉 1240→1280 并注明来源。门：新静态锁 27 断言（含手册锚点与 `##` 标题逐字比对）、`--fast` 50/50、依赖图零新增边、`facts` e2eCount 269、九件定向 e2e 全绿；全量 3 fail／2 flaky 经串行复跑全部通过且与改动面零交集。提交 `3bcfdbc`。
- **118d ＋ 118g（2026-09-03，opus 实现／主会话亲核）**：侧栏常驻帮助入口聚合五项全真动作（使用手册／管理员手册首次有 UI／重新打开引导／查看日志／打开数据目录），复用共用实例登记处使组合根仅 +3 行（1244/1280）。新增 `POST /api/open-path`：token 级双重校验，body 只收 `data|logs|workspace|manuals` 源码枚举，**不接受任何路径型字段**，目录全部由服务端常量或 `session.cwd` 推导，handler 块内零 `path.join/resolve`；执行复用 `DesktopShell.revealInExplorer`（路径经环境变量交给 PowerShell 助手，零 shell 拼接），**刻意不用 `--open` 的 `cmd /c start`**（`03-bridge-guard.js:688` 已记录它会在 `& | &&` 上劈参数）。新增 `GET /api/logs/tail`：token 级，`lines` 夹到 [1,2000]，文件名由服务端按固定正则在已知目录挑，只读尾部 ≤2MB，**只回 basename**，缺文件 200 降级，读失败只回机器码以免把绝对路径带回前端；前端等宽只读面板配「复制全部」（复制内容，非路径）与「打开日志目录」。设置页签加「?」按页签跳手册小节，锚点与两份手册标题逐字比对入锁。README 顶部加「新手从这里开始」，命令行段下沉。118g：MCP 模板「复制」改为**「一键应用到配置」**（写入前展示新增／覆盖摘要），「复制」降级为「复制 JSON（排错用）」；两处「打开数据目录」改走枚举通道；全仓 14 个同族命中逐个处置，`file.reveal`／产物区 reveal／输入侧粘贴路径／复制内容类保留并在白名单写明理由。**走查另修四处既有缺陷**：① 118a-fix 遗留——`normalizeApiErrorPayload` 会把 `{error:'help.doc_missing'}` 结构化，使判定恒假、缺手册时印 `[object Object]` 而不走应用内降级，新增双形状判定 `helpErrorTag()`；② MCP 导入错误同因印 `[object Object]`；③ 应用失败报「没有可导入的条目」而非真原因；④ 日志面板玻璃档透字、「复制 JSON」按钮绝对定位飘出弹层。门：新增 `help-menu.e2e.js`（24 条，22 个枚举外注入样本全 400）与 `copy-path-guard.static.e2e.js`（34 条）；`--fast` 51/51；依赖图零新增边；route-inventory 104 判定点／95 鉴权行双向零漂移；`facts` e2eCount 271。提交 `718f1ae`。
