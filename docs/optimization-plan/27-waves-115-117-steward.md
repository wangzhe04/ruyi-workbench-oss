...
| 档位 | 允许的动作 | 原语 |
|---|---|---|
| 只提议（默认） | 生成提议卡与计划卡；所有写动作等用户一键批准 | 相当于 `ask`；批准即执行 `steward_decide`／`steward_thread_continue` |
| 有界自动 | 白名单内直接执行（含计划卡直接开跑）：瞬时失败重试 ≤1 次、预算暂停后按用户预设上限加一次、按用户预设默认选项答复 question／plan、重启后 `resumeTier` 可自动的 run 续跑、新建线程；白名单外仍提议 | 白名单为持久化策略文件 `<data>/steward/policy-v1.json`（设置页勾选＋从批准记录归纳的建议项须用户确认一次）；授权书：**管家不自签**，改为「管家提议授权书 → 用户一键签发」；permission 类待决在有界档**永不自动放行 edit／exec**，只可放行 read |
| 全自动 | 除永久豁免清单外的所有动作直接执行，**计划卡不再逐次确认，新建线程默认继承全自动**；受熔断、并发与费用上限约束，每个动作可撤销（checkpoint／rewind） | 相当于 `bypass`；启用需二次确认并显示风险说明；一键停机常驻；线程头部与计划卡显示「全自动」徽标 |

**档位继承（用户 2026-09-03 决定）**：管家新建的线程默认继承用户当时的管家档位；用户可对单条线程降档（永远允许降，不允许由管家升）。线程头部显示继承来源，便于回溯「它为什么没问我」。

**永久豁免清单（任何档位都不自动执行，主会话 2026-09-03 裁决并经用户确认）**：
1. **不可撤销且外溢的动作**：以用户身份对外发送内容（邮件／IM／发帖）、支付与交易、删除工作文件夹之外的数据、安装卸载软件、修改系统设置。这类动作没有 checkpoint 可回滚，一律降级为提议卡等用户按。
2. **管家不得自我扩权**：调宽任一线程的 `permissionMode`、签发授权书、关闭审计或停机开关、修改本清单。允许自我扩权的自治系统没有底线可言。
...
- **默认不给全貌**：首屏只显示「现在这一件」，其余线程折叠成一行摘要（「另有 4 条在跑，1 条等你」）。多线程的真实痛点是焦虑而不是看不见，全貌是按需展开的第二层。
- **看板视图**（展开后）：按事项分组，每组一行事项聚合状态，组内每线程一行：状态徽标、当前动作一句话、已运行时长、等待原因（来自 116h 的单一原因）、费用。支持按事项／工作文件夹／状态过滤，不做自定义列。
- **排队可解释**：等待中的线程必须显示为什么在等；「等并发位」时显示前面还有几条。
- **一键操作**：每行可暂停、停止、提升优先级（插队即占用下一个并发位）、打开抽屉；批量操作只提供「全部暂停」。
