...
- **116-1 出门（2026-09-05，Fable）Release Brief**：
  ...
  - **治理清单**（不阻塞出门）：`realhist-fixtures` 缺失的两件回放 e2e 应进 `run-all` SKIP 或改为条件跳过；`eol-policy` 165 处存量漂移单独立 hygiene 切片（含 CSS SHA 重钉）；`steward-inbox.e2e` 重启轮询断言时序偶发；`run-all --parallel` 报告行带文件名；依赖图扫描器对函数内 `const text` 误报；`updateSessionMeta` 与活回合收尾 save 的竞态。
