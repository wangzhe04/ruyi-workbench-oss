...
#### 11.16.4 用户第十轮追加两条（2026-09-09 夜，第五张截图）

> 「现在的 UI 排版也有很多问题，上面的线程胶囊在跑完后是有了，但最好还是有了线程后就显示出来；
> 然后每段会话离的太近了，你看图，很密」

**追加①「胶囊要在线程一有就出现」＝ §11.16.2 V1 的第 ③ 件。** 用户观察到的「跑完才有」正是那条 bug 的现象：
chip 的唯一来源是线程卡，而线程卡只在 `trigger.kind === 'inbox'`（＝线程跑完、收件箱叫醒管家）那一支挂。
V1 把 `executedThreadSessionId` 兜底从该分支里放出来之后，**管家开线程那一刻**就有 chip，不必等它收工。
这条不另开刀，只在此记明用户口径：**判据是「线程存在」，不是「线程收工」**。

**追加②「每段会话离得太近」——真问题，根因是单位变了而间距没跟着变。**
今天的间距是 117l-B2 定的三档，那时对话流的单位是**一条消息**：组内 `--sp-1`(4px)、组间 `--sp-3`(12px)
（`.steward-msg { margin-top: var(--sp-3) }` ＋ `.steward-msg-ruyi:not(.is-group-start) { margin-top: 0 }`）。
F1 之后单位变成了**一张线程卡**——卡头 ＋ 管家按语 ＋ 交付卡 ＋ 展开 ＋ 动作键，一张卡几百像素高，
**而两张卡之间仍然只有那 12px**。块大了三四倍，分隔没变，于是读起来就是一堵墙。

**改法（V4，独立一刀，在 V1 出门之后做——两者同一个 CSS 层，不并行）**：把三档扩成四档，
新增**卡间**档（两张线程卡之间、以及线程段与其后非线程行之间），取值一档更大（`--sp-5` 量级，实测定）。
纪律照旧：
- 只加**卡间**这一档，**组内／组间两档一个像素都不许动**（它们是 117l-B2 用户走查的结论，动它就是回退别人的结论）；
- 判据挂在**已有的类**上（`.is-thread-start` / `.is-thread-end` 是 F1 已经在维护的段界），**不新造第二套分组状态**；
- 锁要钉「卡间 > 组间 > 组内」这个**序关系**，不要钉具体像素值（钉数值 = 又一把「钉文本长什么样」的锁）。

**追加③「胶囊显示不全」（用户 2026-09-09：「还有那个胶囊显示不全，一块修了吧」）——并入 V4。**
根因查实：频道条是 `overflow-x: auto`，但**滚动条被刻意隐藏**（`scrollbar-width: none` ＋
`.steward-channels::-webkit-scrollbar { height: 0 }`），而 chip 是 `flex: 0 0 auto; white-space: nowrap`。
于是线程一多，右侧的 chip 就滑出可视区**且没有任何提示**：不换行、无滚动条、无边缘渐隐、无箭头 ——
「看不见也摸不着」。

**更要命的一点**：「全部线程」那枚排在**最末尾**，它是通往看板的逃生舱；一旦被挤出去，用户既看不到
剩下的线程，也找不到「去看全部」的入口 —— 两条路同时断掉。

**V4 对这一条的要求与约束**（改法留给执行者，但这三条是硬的）：
1. **不许存在「看不见也摸不着」的 chip**：要么全部可见（换行），要么**可见地**可滚（保留滚动条或给边缘渐隐/箭头这类明确 affordance）。
2. **「全部线程」必须始终可达** —— 它不能跟着一起滚出去（钉这一条，而不是钉某种具体实现）。
3. 粘条占高不能失控：它是 `position: sticky`，换行方案要有高度上限（超出仍可滚），否则线程一多就把正文挤没。

锁同样钉**事实**不钉像素：「在 N 条线程下，所有 chip 要么可见、要么可经可见的 affordance 到达；且『全部线程』恒可达」。

### 11.17 117x 设计页 · 模型选择器重做（用户第十轮走查④；Fable 设计）

> 用户原话：「这个模型选择做的也很一般」。真机现状：一个 30 余项的**扁平裸列表**，无搜索、无分组，
> 直接印原始 id（`qwen3.5-livetranslate-flash-realtime-2026-05-19` 这种），且混入 audio-realtime／
> image／ocr／livetranslate 等**与文本任务无关**的端点。

#### 11.17.1 先划清：我们真握着什么（这一节决定界面能说什么）

**握着的**（都可查证）：

| 事实 | 出处 |
|---|---|
| `id` 与 `label`（`label` 绝大多数就等于 `id`） | `steward-chips.js:371` 读 `provider.models[]`；来源是端点 `/models`（`provider-settings.js:226` 回填） |
| 它属于哪个 provider、当前引擎是什么 | `resolveEngineRoute`（全仓唯一那份「会话级 ＞ 全局回落」） |
| 当前选中项、全局默认值 | 同上 |
| **按模型的真实用量** | 账本 `usage/YYYY-MM.jsonl` 每条都带 `model`（实测一行：`{ts, sessionId, engine, provider, model, inTok, outTok, cost, …}`） |

**没握着的 —— 界面里一个字都不许出现**：价格、速度、**per-model 的能力**。

**更正（M2 证伪，主会话核到行）**：本节原先把「上下文窗口」也列在「没握着」里，**不成立**。
`13-http-router.js:412-420` 给每个模型对象补 `contextLength`，来源有**三层**：live 探测 → 探测缓存 →
**版本化名称表** `contextWindowFromTable(id)`（按名字查表）；经典壳 `navigation-controls.js:372` 已经在印 128K／1M 徽标。
所以它是**部分握着**：openai 端点、且探测到或表里有才有；Claude-CLI 那条 `offlineModelList` 一律没有。
**M2 按原文办、没加徽标，是对的** —— 但理由不是「没握着」，而是下面这条：
名称表那一层是**按名字猜的**（与「不像文本模型」同一性质），而路由器发出的对象**不带来源标记**，
界面分不清「探测到的」与「查表猜的」。要印就得先让路由器标来源，那是另一把小刀，**登记为候选**，不在 117x 里做。
注意最后一条：能力矩阵是 **provider 级**（`provider.vision` / `provider.reasoning`，见 `provider-settings.js:842`），
**不是模型级**。所以不许给某个模型标「支持工具调用／支持视觉」——那是我们不知道的事。

**一处必须补的缺口**：`buildUsageSummary`（`00-boot.js:464`）只聚合到 **engine / provider / session / day**，
**没有 byModel**。账本里的 `model` 字段一直在写，只是没人聚合它。所以「常用置顶」要么补这个维度（用真事实），
要么退化成浏览器本地的猜测 —— **本设计选前者**：多一个聚合维度，不新增路由。

#### 11.17.2 设计

**结构**（从上到下）：

1. **搜索框** —— 仅当候选 > 8 项时出现（少的时候多一个框是噪音）。输入即过滤，匹配 `id` 与 `label` 的子串，命中段高亮。
2. **跟随全局** —— 永远第一项，语义不变。
3. **常用** —— 由 `byModel` 用量派生：最近 30 天用过的，按**最近一次使用时间**排，最多 5 条。**没有用量就整段不出现**（不摆一个空标题）。
4. **这个端点的全部模型** —— 当前 route 的 provider 优先；多 provider 时按 provider 分组，组标题是 provider 的 `label`。
5. **「看起来不是文本模型」** —— **默认折叠，不是隐藏**（见下）。

**每一行显示什么**：
- 主行：`label`（等于 id 时就显示 id；命中搜索时高亮匹配段）。**不改写 id、不美化成别的名字**——用户要复制粘贴的是真 id。
- 副行：**只说我们真知道的** —— 「上次用 · N 天前 · 共 M 回合」，数据来自 `byModel`。**没有用量就不出副行**（不显示「0 回合」，那是把「不知道」说成「零」）。
- 标记：当前项 `aria-checked`；全局默认那一项给一枚「默认」徽标。

**键盘**：输入即过滤、↑↓ 移动、Enter 选定、Esc 关闭（与既有 chip 菜单的 Esc 栈同一条通道，不新开）。

#### 11.17.3 那条「不像文本模型」的判据 —— 折叠，不是过滤

用 **id 子串**判断（`audio` / `realtime` / `image` / `ocr` / `tts` / `embed` / `rerank` / `livetranslate` / `video`）。
三条硬纪律：

1. **必须折叠，不许隐藏。** 判据是「按名字猜的」，猜错的代价是用户找不到一个真实存在的端点。折叠区标题**明写**「按名字猜的，可能猜错」，一键展开。
2. **子串表写在一处**，不许散落（本仓已经为「第二份表」栽过四次）。
3. **不许因此改变可选性** —— 折叠区里的模型点了照样能选。

#### 11.17.4 一处实现

chips 工厂（`steward-chips.js` 的 `buildModelMenu`）同时服务 **2.0 顶栏 / 线程详情栏 / 看板**三处，改在这里**一改三处到位**。
经典壳若另有一份模型菜单（`modelOptions` 的注释说「来源与经典壳模型菜单同一份数据」——**同数据不等于同渲染**），
本刀**只调查并如实记录，不合并**：合并渲染是另一件事，要单独立项，别在这一刀里顺手做。

**M1 的数据前提已核实（主会话亲自查，不是推断）**：`byModel` 是**纯聚合**，不需要先补采集。

- 落盘行的形状里**本来就有 `model`** —— `00-boot.js:392`：`model: String(entry.model || '')`；
- 全仓 15 处 `appendUsageLedger({…})` 调用，**凡是真会写进账本的那 11 处都传了 `model`**
  （`09-workflow.js:3122` 用的是简写 `model,`，别被字符串查找漏判成没传）；
- 因此**历史行也带着模型名**，「最近用过／常用置顶」开箱即准，**不用等新数据攒起来**。
...
会把它们**原样丢掉**；它们传的 `meta` 字段 `rec` 里也根本没有。
调用处的注释写着「审计失败不阻断」，可见**意图是留一条审计痕迹，实际一条都没留下**。
与用量看板和 117x 都无关（那 4 处本来就不该进用量统计），但「以为记了、其实没记」的审计是自己的一类问题。
...
- 不给模型编造任何元数据（上下文窗口／价格／速度／能力）。
- 不按「推荐程度」排序 —— 我们没有依据，排出来的是假权威。
- 不自动隐藏任何模型（只折叠）。
- 不在这一刀里合并经典壳那份渲染。

#### 11.17.7 验收（可证伪）

1. 真机 30+ 模型下：搜索、分组、键盘、折叠区都可用（截图 ＋ DOM 断言）。
2. **没有任何模型是「看不见也摸不着」的** —— 与频道条 §11.16.4 追加③ 同一条纪律：折叠区可展开、搜索能命中折叠区里的项。
3. 副行**只在真有用量时**出现：把 ledger 里某模型的记录清掉 → 该行副行整体消失，而不是显示「0 回合」。
4. `byModel` 聚合与账本逐行重算一致；**反向验证**：改一条 ledger 记录 → 聚合跟着变；把 `model` 字段删掉 → 该条不进 byModel 而不是记成空串。
5. 「跟随全局」仍是第一项，语义与 `resolveEngineRoute` 一致（M2 不许新增第二处路由回落判据）。

#### 11.16.5 用户第十轮再追加两条（第六张截图）

> 「还有单线程的总结回复和原文最好也中间隔些空间，然后我看里面还参杂了一些线程推进的原文，也不要有」

**追加④「总结与原文之间要留空」——并入 V4。** 这是 §11.16.4 追加② 的**卡内**版本：那一条说的是卡与卡之间，
这一条说的是**同一张卡内部**——管家的按语、交付卡、动作键三块贴在一起。同一次改，四档间距一次定完：
**卡内块间**（按语↔交付卡↔动作键）＞ 组内，但 ＜ 组间；**卡间**最大。仍然只钉序关系，不钉像素。

**追加⑤「交付原文里混进了线程推进的过程叙述」——找到精确解，不用猜。**

现状：`stewardDeliverableFrom`（`steward-conversation.js:145`）取的是「该回合最后一条助手消息」的 `content`，
而那条消息的 `content` 里**本来就混着**工具调用之间的过程叙述（截图里的「我先联网核实最新数据」
「搜索后端对中文长查询分词太差，我改用脚本直连…」）。

**仓里已经有一份精确的数据可用**：第 54 波 EC-D 的**有序叙事账本** `segments`
（`02c-turn-segments.js`：「one ordered narrative ledger … reconstruct the actual text -> tool -> text sequence」）。
实测每一条落盘的助手消息上都有 `segments`，形状是 `{ id, type: 'thinking' | 'text' | 'tool', text }`，
而前端本来就在拉整个会话（`GET /api/sessions/:id`）——**所以这是纯前端改动，不动后端、不加路由**。

**新判据（精确，非启发式）**：

> **交付原文 ＝ `segments` 里【最后一个 `type:'tool'` 段之后】的所有 `type:'text'` 段拼接。**
> `type:'thinking'` 段**一律不进交付**（它是思考，不是交付）。
> 没有 `tool` 段时 ＝ 全部 `text` 段拼接（与今天的 `content` 等价，回落一致）。

用户抱怨的那几行，恰恰就是**夹在工具调用之间**的 text 段 —— 按这条判据它们自然落在「最后一个工具段之前」，
不需要正则、不需要猜「哪句像过程」。

**连带修正 §11.16.1 ⑨ 对「它刚说」的改法**：那里写的是「收工后也取尾巴 ≤3 句」。
有了 segments，更准的规则是：**在跑 → 最新那一段（现状不变）；收工 → 最后一个工具段之后的文本**，
再从中取 ≤3 句。以本节为准，V2 照这条做。

**边界要写进注释**：`segments` 缺席的老会话（EC-D 之前落盘的）必须回落到今天的 `content` 整段，
**不许因为拿不到 segments 就显示空白** —— 那是把「读不到」演成「它没交付」。

#### 11.16.6 更正：§11.16.1 ⑨ 里「两枚『看全文』共用同一个 i18n 键」是**错的**（V1 证伪，主会话已核）

我在 §11.16.1 ⑨ 写的是「两枚『看全文』共用同一个 i18n 键 `stewardShell.drawer.fullText`」。**不成立。**
V1 核过、主会话复核过：

- 交付卡那枚用的确实是仓里的文案键；
- **琥珀色那枚的文案是【模型自己写的】** —— `13h-steward-runner.js:673`：
  `label: String(row.label || '').slice(0, STEWARD_ACT_LABEL_MAX) || stewardActLabel(row.tool, row.args)`。
  用户截图里的「看英伟达分析全文」就是模型现编的一句词，**不是仓里的任何一个键**。

**为什么修法仍然对**：用户感到的混淆是真的（同屏两个「看…全文」去两个地方），
给交付卡那枚一个**说清去处**的自有文案（「到 2.0 视窗看全文」）就解决了它这一半。

**但由此多出一件本文原先没有的事，归 V3（提示词）**：既然那枚 act 的词是模型写的，
**混淆就可能换个说法再回来**（下次它可能写「查看完整分析」）。06b 应当约束**开线程类 act 的措辞**：
说「打开线程」这类**去处**，而不是「看…全文」这类**内容**词——否则它会和交付卡那枚永远撞车。

另两处 V1 的证伪也记下，免得后人照抄我的错：
- 代码里那句注释「同一个词、同一个动作」指的是**抽屉**那一枚，而 `openClassicView() → openClassicWindow(id)`
  与 `fullTextOf()` **确实是同一个动作** —— 「同一个动作」这半句今天仍然成立，不成立的只是「所以共用一个词没关系」。
- 「交付卡放出来会为每一行多发一次请求」**不成立**：卡头与交付卡 `await` 的是 `loadDeliverable` 的**同一个缓存 promise**。
  拦住它的真理由是「刚开的线程这一回合根本没有交付，放出来只会给每条用户回合垫一句『这一次没取到原文』」。

#### 11.16.7 裁决：交付边界从「最后一个 `tool` 段」扩到「最后一个 `tool` 或 `subagent` 段」

V4 落地 §11.16.5 时**严格照原判据只认 `tool` 段**，并把「要不要把 `subagent` 也算进活动段」如实登记成一笔债、
拒绝顺手改。**它做对了**：放宽判据是判断题，不该由执行者在实现途中自行拍板。以下是主会话的裁决与理由，
`steward-conversation.js` 里那段注释引用的就是本节。

**裁决：`ACTIVITY = ['tool', 'subagent']`。**

理由：**会产生「过程叙述」的活动有两种** —— 自己调工具，以及派子代理去干。后者同样会在段与段之间留下
「我先派个 agent 去查」这类叙述，而那正是本刀要挡在交付之外的东西。只认 `tool` 会把派子代理那一路漏掉。

**同时明确：有意【不】扩到「所有非 `text`/`thinking` 段」。**
`mission` / `workflow` 这类**记账段有可能尾随在正文之后**。一刀切会把边界推到正文末尾，于是交付原文整个被切掉、
界面退化成一句「这一次没取到原文」——**把「读得到」演成「没交付」，比漏掉一句叙述坏得多**。
判据宁可窄一点，错在少挡，不错在多挡。

**这条危险不是推演，是实测出来的**：反向验证时把判据改成「所有非 text/thinking 段」，AA7c 当场红，
交付原文实测变成空串 `「」`。锁 AA7b（`subagent` 算边界）与 AA7c（尾随记账段不得把交付清空）
两个方向各自独立反向验证过。

**V4 一并纠正了本文三处事实错误，照录以免后人照抄**：

| 我原先写的 | 实测 |
|---|---|
| 两张管家卡之间是 12px | **4px** —— 两张卡都是 `.steward-msg-ruyi`，被判成同一组，吃的是最小那一档。比记录的更糟 |
| 组间间距 ＝ token 上的 12px | **16px**（feed gap 4 ＋ margin-top 12 的合成值）。四档比大小必须按**合成后**的值算 |
| segments 形状 `{ id, type, text }` | `tool` 段**没有 `text` 字段**（键为 `id,type,toolCallId,name,batchId,status`）；实测段类型还有 `question` |

**另记一处讽刺（措辞已按实际代码校准，别照抄我第一版的说法）**：
`steward-conversation.static.e2e.js:846` 的 **W8e 原本是【两半】合起来的一条断言** ——
前半句钉 `.steward-channels` 上有 `overflow-x: auto`，后半句钉窄屏下 chip 触达高度 ≥44px。
**前半句钉住的正是本轮要修掉的病**：一个把滚动条藏起来的横滚容器，chip 滑出去就「看不见也摸不着」
（§11.16.4 追加③ 报的就是它）。V4 重钉时把前半句摘掉、后半句原样留下（它仍成立），
「chip 不许消失」改由 AA 段按**事实**钉（换行 ＋ 可见滚动条 ＋「全部线程」不在滚动容器里，见 `:1054`）。

所以今天 grep `W8e` 只会看到那半句 44px，**不要因此以为「锁钉着 bug」这件事没发生过** —— 发生过，
在前半句里。这正是「钉住哪个事实必须成立」与「钉住今天恰好长这样」的活样本：
后半句钉的是一条**必须成立的事实**（手指点得到），所以它活了下来；
前半句钉的是**当时恰好的实现手段**（用横滚实现溢出），于是实现一旦被判定为病，锁就站在了病那一边。

#### 11.16.8 派 V2 / V3 时的两条裁决，与一处新发现

**裁决一：切模型/引擎那句提示【无条件常显】，不做「在不在跑」的判断。**

§11.16.2 的 V2 行写的是「**在跑时**切模型/引擎要说清『本回合跑完后生效』」。派单时改成无条件。三条理由：

1. **这句话恒真。** 「下一回合生效、不打断正在跑的回合」在两种情形下都对：在跑时它回答「会不会打断」，
   不在跑时它同样准确。既然恒真，就没有理由让它的出现依赖一个判断。
2. **那个判断本仓已知不可靠，而且代码自己写着。** `steward-drawer.js:586`（`renderLastSay()` 里那段注释）说明 `isLive()` 拿不到
   `resumable.live`（`GET /api/sessions/:id` 的 live 分支根本不回这个字段）就回落到五态，而挂在提问上的回合
   五态是 `needs_you`，于是**恒判成不在跑**。把一句本来无条件为真的话，押在一个已知坏掉的判据上，是自找的。
3. **chips 有三个宿主**（抽屉、看板行、2.0 顶栏），而看板行手里只有 `GET /api/missions` 的卡片，
   连 `engineRoute` 都没有（`steward-chips.js:143` 那段如实记）。要三面都判活性就得再喂第四种数据 ——
   为一句恒真的话付这个代价不值。

**裁决二：「它刚说」收工后仍取【开头】≤3 句，不是 §11.16.1 ⑨ 写的「改取尾巴」。**

⑨ 当时的诊断没错（取开头必然取到开场白），但**修法是在还没想到 segments 之前的粗糙近似**。
有了 §11.16.5 的账本判据，正确的次序是：**先按 segments 过滤掉过程叙述，再从剩下的文本取开头**。
过滤之后第一句就是收口结论；这时候再取尾巴反而会取到末尾的风险提示与注意事项。
换句话说 ⑨ 的「取反了」是**症状**，`content` 里混着过程叙述才是**病根**——病根一治，取头就是对的。

**新发现（提高了 V3 的分量，也带出一件后续波次的候选）：前端那两刀够不到三个显示面。**

主会话核这条链子核到行：

- `13d-core-domain-routes.js:539` → `lastSay: String(head.summary || '')`
- `13h-steward-runner.js:262` 注释原文：「`lastSay` 取会话头的 `summary`：那就是上一回合助手最终文本的**前 160 字**（09 收尾处写的）」
- `13h-steward-runner.js:345` → `lastSay: head.summary || ''`
- `06i-steward-core.js:143` `buildStewardDigestLine` 把 `t.lastSay` 截到 200 字塞进管家总览行

所以**看板的「它最后说」、管家总览行、以及管家转述时读到的那句话，走的是服务端 `head.summary` ＝ 最终助手文本的前 160 字，完全不经过 `segments`**。
V4（交付卡）与 V2（它刚说）都是**纯前端**改动，够不着这三个面。

**两个后果**：

1. **V3 不是锦上添花**，它是这三个面**目前唯一**的修法 —— 让线程把结论写在最前面，前 160 字才会是结论。
   §11.16.1 ⑨ 说「对四个显示面同时生效」当时是推断，现在有了机制证据。
2. **登记一件后续候选（不进 117v）**：服务端也改用 segments 口径生成 `head.summary`。
   它是同一件事的**结构版**——按本文自己的纪律（§11.16.5「精确判据，不用正则猜」），
   靠模型自觉终究不如靠账本。但它动的是落盘数据的生成口径，范围与风险都超出本波，
   **不在 V3 里顺手做**。V4 已经把提取规则写成纯函数，将来搬到服务端有现成的一份。

### 11.18 117y 设计页 · 管家的话「说得完」（用户第十一轮拍板②；Fable 设计）

> 用户原话：「但是得保证话能说全，不要硬截，用户体验不好；还有管家的话最好不要用展开的二级菜单了，
> 显示完吧；通过提示词去约束说的话长度」

**这一条推翻了我自己的建议。** 我当时建议「结掉，不动数字」，理由是交付卡已经承载原文、转述本该短。
用户接受了「不再压数字」，但指出了我没看的那一半：**问题不在长度定多少，在于「怎么变短」。**
今天是两种最差的做法各占一处 —— 后端**硬切字符**，前端**折起来让你再点一次**。两者都是把
「让它少说」的责任推给了运行期和用户，而那本该是提示词的事。

#### 11.18.1 病灶（三处，都已核到行）

| # | 位置 | 现状 |
|---|---|---|
| ① | `13h-steward-runner.js:548` | 解析成功那一路：`String(value.say).slice(0, STEWARD_SAY_MAX)` —— **裸 slice，连省略号都没有**。601 字的回复在第 600 字处无声断掉，可能断在半个句子、半个词中间 |
| ② | `13h-steward-runner.js:536` | JSON 解析失败的兜底：`raw.trim().slice(0, STEWARD_SAY_MAX)` —— 同一把裸刀 |
| ③ | `steward-conversation.js` `finishSay()` | 管家正文超 8 行就 `clampIfLong` ＋ 一枚「展开」按钮。**话是完整的，但要用户多点一次才看得到** |

**必须分清、不要一起改的一处**：`06i-steward-core.js:633` 的 `stewardClipSay` 截到 200 字**并加「…」**，
喂的是**总览行与待决一行话**（`13k:97/191` 的 `lastAssistantText`、`stewardPendingOneLine`）。
那是**列表里的一行摘要**，截断加省略号正是对的做法，**本刀不碰它**。
用户说的「管家的话」是它在对话里说的那段话，不是列表行。

#### 11.18.2 判据的转向：600 从「运行期强制」变成「提示词请求」

今天 `STEWARD_SAY_MAX = 600` 同时扮演两个角色 —— 06b 输出契约里写给模型看的 `≤600 字`，
和 13h 里那把真刀。**本刀把这两个角色拆开**：

- **`STEWARD_SAY_TARGET = 600`** —— 提示词说给模型听的目标。**运行期不再据此裁剪。**
- **`STEWARD_SAY_CEILING = 4000`** —— 只防病态载荷（尤其 ② 那条兜底会把**整份原始模型输出**灌进来）。
  取 4000 的理由：约为目标的 6.7 倍，任何守规矩的回复都碰不到它；而它仍然拦得住把几十 KB 灌进对话的情形。

**触到天花板时也不许裸切**：在 06i 新增纯函数 `stewardTrimSayAtSentence(text, ceiling)` ——
在天花板之前的**最后一个句末标点**处切（同表：。！？.!?），一个句号都找不到才退回裸切；
**切了就要明说**，末尾缀一句诚实的话，不许假装这就是它说完了。
放 06i 的理由：它是纯函数与常量的归宿，`13h -> 06i` 本来就是合法后向边。

#### 11.18.3 三刀

| 刀 | 做什么 | 独占文件 |
|---|---|---|
| **S1 · 后端不硬截** | 拆两个常量；`:536`/`:548` 改走 `stewardTrimSayAtSentence`；句界裁剪与诚实标记落在 06i | `app/src/13h-*.js`、`app/src/06i-*.js`（动 `src/` → 必须 `build.js --check`） |
| **S2 · 提示词管长度** | `steward.rules` 加一条：**把话说完整，宁可少说一件事，也不要说半句**；中英同步。**顺手还掉 V3 登记的那笔债** —— 给 `rules` 加一条字数闸（它已从 117l 的 4 条长到 7 条，而 `stable` 有 ≤2500 硬闸、`rules` 一个数字都没人看着） | `app/src/06b-*.js`、钉提示词的锁 |
| **S3 · 前端不折叠管家正文** | `finishSay()` 不再 `clampIfLong`／`collapseToggle`；`.is-lead` 首段处理保留 | `js/steward-conversation.js`、它的两件 e2e |

**S3 明确不动交付卡的折叠。** 用户说的是「管家的话」；交付卡里是**线程自己写的交付原文**，
实测能到几千字（117s-H2 那次量到 2687 字），把它整段摊开会把管家的按语挤出屏幕。
折叠的那一处实现（`clampIfLong`／`collapseToggle`）**保留**，只是从两个调用方减到一个。

#### 11.18.4 撞锁（都是合法改动撞的，重钉不放宽）

- `steward-conversation.static.e2e.js:691-694` **Q6**：钉的是「`clampIfLong` 恰好出现 3 次 ＝ 定义＋交付卡＋管家正文」。
  S3 之后是 **2 次**。重钉时**判据要跟着换**：从「两个调用方」改成「**只有交付卡一个调用方，且管家正文那一路没有折叠**」——
  这是本刀的事实，不是把数字从 3 改成 2 就完事。
- 同文件 `:704-705`：`clampIfLong(node, text)`（管家正文）那半句要去掉，`clampIfLong(body, found.text)`（交付卡）保留。
- `steward-conversation.e2e.js:1321/1438`：`.steward-say-acts` 上的展开按钮断言 —— 管家正文行上**不该再有**那枚按钮。

#### 11.18.5 验收（可证伪，必须反向验证）

1. **700 字的 say 一个字都不少地落盘并显示。** 这是本刀的核心断言 —— 它同时证明「运行期不再按 600 裁」
   与「前端不再折起来」。修前这一条必红（600 字处断掉）。
2. **触到 4000 天花板时，切在句号上，且末尾有明说。** 造一条 5000 字的 say，断言结尾不是半句话。
...
#### 11.17.8 117x-M2 交付记录（`12b0df1`；主会话逐条复核）

**核过的**：9 个文件全在范围内，一个 P1 的文件都没碰；四个改动文件 NUL 0／CRLF 0；「不像文本模型」的子串表
**全 `public/js` 只此一处**（`livetranslate` 字面量恰好出现 1 次，且只在 `steward-chips.js`）；chips 代码剥掉注释后
`.reasoning`／`.vision`／`contextLength` **零出现**（红线一守住）；8 个新 i18n 键四份文件各 8 条、全扁平。
CSS 载荷锁由主会话从 HEAD 续钉（`801d85e5` → `07e82fa0`），**M2 独立算出的值与主会话逐字相同**，两条路互证。

**主会话自己的一次翻车，记下来**：第一次手写复现哈希算法时**复现错了函数**——复现的是 `readFrontendCss()`，
而两个消费者哈希的是 `readLayerPayload()`（无层头、去每文件首行、CRLF 归一、分组拼接）。自证当场不过，
**没有钉**。改为拦截 `fs.readFileSync` 让消费者自己的函数直接读 git blob——同一份代码换数据源，
不再手写第二份算法。这条也写进了锁旁的注释，下次续钉照做。

**架构**：判据（`stewardModelMenuView`，纯函数）＋ 渲染（`draw()` 只做翻译）；三个面共用工厂，看板紧凑行那一支原样。
「常用」的 30 天／5 条裁剪在前端一处（常量导出、N5a 钉住）；用量拉 `range=all`（`month` 是本自然月，月初会把上月用过的
全判成没用过），模块级缓存、懒发、失败不缓存不编数据。

**M2 抓到并当场修掉的两条假锁**（值得学）：① 原断言「行看不见」——行被**删掉**时它也过，改成「行必须在 ∧ 且看不见」；
② 原用同一份夹具断言「400 天前那条不进常用」，但它排在最后、恒被「最多 5 条」挡住，**窗口放宽到十年也绿**——
另起 29 天进／31 天不进的夹具才证得伪。

**它登记的债，主会话裁决**：

| 债 | 裁决 |
|---|---|
| 没有真浏览器 DOM 断言（夹具 `MODEL_PRESETS` 里没有一个非文本 id，折叠区在真机夹具里根本不出现） | **立一把小刀**：给 `steward-drawer.e2e` 的夹具加一个带 30 项（含 audio/image/realtime）的 provider，钉四条。单测是驱动真工厂跑的，逻辑有覆盖；缺的是真 CSS 与真焦点 |
| 英文复数「1 turns」（chips 只注入 `t`，加 `tCount` 会破坏零 import 叶子纪律） | **登记不修**。修法不在 chips 里加 import，而是让宿主把 `tCount` 与 `t` 一起注进来——归下一把动 chips 工厂签名的刀 |
| 上下文窗口「没握着」不成立 | **设计页已更正**（§11.17.1）。徽标不加：名称表那一层是按名字猜的，路由器发出的对象不带来源标记，界面分不清「探测到的」与「查表猜的」。要印先让路由器标来源，登记为候选 |
| 同一 id 在「常用」与「全部」各出现一行，徽标副行跟着出现两次 | **接受**——选择器的常规做法；锁按「带徽标的行的 id 只能是全局默认」钉，不钉行数，正确 |
| 搜索期间无法手动收起折叠区 | **接受**——命中了却藏起来就是「看不见也摸不着」 |
| `followGlobal` 在 agent 引擎下字面不严丝合缝（空模型是交给 CLI 默认，不是回落 `config.model`） | **登记**。这个状态本来就可达，M2 只是给了入口；措辞归 i18n 小修 |
| `range=all` 读全部月份账本 | **接受**——一次页面生命周期只发一次、懒发；用量看板本来就提供 all 档 |
...
#### 11.18.7 117y-A（S1＋S2）交付记录（`6cd57f8`；主会话逐条复核）

**核过的**：16 个文件全在范围内、零 B/M2 文件；`STEWARD_SAY_MAX` 全 `src/` 零残留；`13o:338/:353` 两条解析路径都走
`stewardTrimSayAtSentence(…, STEWARD_SAY_CEILING)`，零裸 `slice`；`06i:633 stewardClipSay` 函数体与 117y 之前**逐字节相同**；
graph 49/390、forwardEdges 67、build 新鲜；五文件 NUL 0／CRLF 0。

**主会话用导出的纯函数直接探针（不信报告）**：700 字 → 一字不少；5000 字 → 4033 字、切在句末、末尾带
`(话太长,先说到这里;后面还有,是工作台截断的,不是我说完了。)`；6000 字无标点 → 退回裸切仍带标记；`stewardClipSay(300 字)` → 201（200＋…）。
`rules` 中文 7 条，闸 2200（低于 stable 的 2500——否则「stable 塞不下就往 rules 挪」是绕开稳定层预算的后门；这个理由对）。

**A 抓到的两条自己的假绿/假红，照录**：① 5000 字样本句 15 字时 600 恰好是整句边界，「修前必红」的断言修前假 PASS——换 11 字句才真红；
② 正则 `\([^)]*STEWARD_SAY_CEILING\)` 被兜底那处实参 `raw.trim()` 自带的括号吃掉一个匹配，**一条假红**。两条都写进了锁的注释。

**一条新模具，进纪律**：`module-dependency-graph.js` 把**裸参数名当跨模块符号**。A 在 06i 新增纯函数时参数叫 `text`，
命中 `00-boot` 的顶层同名符号，生成 `06i → 00-boot` 一条边并把 06i 拽进 SCC，`--check` 直接红，错误信息是「新增循环边」——
**指不到参数名**。改名 `value` 后恢复。**06i 里新增带参数的纯函数，参数名不能与更早模块的顶层符号同名。**

**越界一处，接受**：`14-main.js` 导出 `stewardTrimSayAtSentence`／`stewardClipSay`／两个常量——这是把验收 2 与 4 从「正则钉源码」
升成真行为断言的唯一路，B 不碰它，零冲突。主会话的探针也正是靠这几个导出做的。

**它登记的债，裁决**：

| 债 | 裁决 |
|---|---|
| 验收 1 只证到「后端 → 落盘」半程，没有一条锁把「700 字进 DOM 一字不少」端到端接起来 | 主会话核了前端链：`steward-conversation.js` 的 say 渲染路径上**没有任何** `slice`／`substring`／长度判断，B 的 V6 又钉了「不点任何按钮就全在 DOM 里、文字相同」。两半在中间**没有第三个裁剪点**，所以已接上；**端到端单锁仍登记**，归下一把动 steward-runner.e2e 的刀 |
| `STEWARD_SAY_TARGET` 没有运行期消费方 | **接受现状**。真正单点化要把 06b 的 `≤600 字` 改成插值，会破坏 stable「常量文字、跨进程逐字节恒定」；静态锁 (d) 已钉两者同源，够用 |
| `rules` 英文余量只剩 ~450 字，第 8 条会撞闸 | **这是闸该做的**。撞了再谈：要么英文包精简，要么把最稳的几条挪回 stable（先给英文 stable 腾地方，2453/2500） |
| 句末标点表不含省略号、分号、换行 | **登记**。一段用「…」收尾、4000 字内无句号的 say 会退回裸切（仍带标记）——极端情形，先观察真机有没有 |
| 裸参数名撞顶层符号（依赖图模具） | **进 32 号文 §4 纪律**（见第 12 条） |

**§11.18.6 登记的「正则钉调用点撞定义签名」审计，主会话做了一遍窄形状扫描**：`dev-harness/` 里形如 `/name\(params\)/` 且与某定义签名逐字同形、
且不是在数次数的断言——**0 条**（唯一真例是 S3 已修的 Q6d）。**如实记边界**：只覆盖裸正则字面量这一种形状，同一个模具嵌在更长模式里、
或用字符串拼出来的正则，这一扫看不到。**记为「窄扫干净」，不记为「已审计」。**

### 11.22 117w-W2 设计页 · 同文件夹多线程（§11.16.3 ⑥前半；Fable 设计；**先决：W1 落地**）

> 用户原话（第十轮）：「要能运行一个文件夹里开多个线程，但是管家要能知晓并在同文件夹里的线程优化提示词告知存在」

#### 11.22.1 先核现状（都在 T2 拆出的 `13n-steward-arbiter.js`）

| 事实 | 出处 |
|---|---|
| 写锁粒度是 `cwdKey`，**一律按「写」处理**——注释原话：「一个回合会不会写文件在开始时无法预知(模型还没说话),按只读乐观放行的代价是两条线程真的一起改同一棵树。只读回合的识别与放宽留后续波」 | `13n:182-184`、`stewardArbiterCwdLock` `:185` |
| **同 cwd 并发 ＋ rivals 已经存在于一支**：待决豁免那一路 `stewardArbiterGrantPending` 不占锁、不挡别人，并算出 `rivals`（同 cwd 其它在跑线程的标题） | `13n:272-284` |
| **但 `rivals` 只发到事件流**（`onEvent 'acquired'`），**`13o` 与 `06b` 里零出现**——它从来没进过任何提示词 | `13n:284`；grep 计数 0/0 |
| **没有 `read` 权限档**：`PERMISSION_MODES = ['default','acceptEdits','plan','auto','bypass']` | `01-config.js:381` |
| **`plan`／`dontAsk` 是机械执行的只读**：07 的权限判定对非 read 档工具直接 `block`（不是提示词承诺） | `07-autonomy.js:500`、`:801` |
| 子代理另有注册层的硬帽 `tierFilter:'read'`（只注册 read 档工具） | `07:63`、`:103-104`、`:1694`；`TOOL_TIER_RANK = {read:0, edit:1, exec:2}` `00-boot:221` |
| 线程回合的易变前缀落点：`volatileExtras`／`turnVolatile`，管家收件箱走同一投放位置、经 `StewardHooks` 延迟绑定（09 → 06i 后向边） | `09-workflow.js:1444-1482` |

所以原稿两句话各对一半：「rivals 名单已经在算」**对**，「喂进提示词」**是新管道**不是改道；「只读回合的识别」**今天就有机械判据**，不必等模型开口。

#### 11.22.2 判据：只读必须是「机械上做不到写」，不是「它说它只读」

**只读回合 ＝ 下列任一成立**（都在回合入口就知道，且都由代码执行）：
1. 该会话 `permissionMode ∈ {plan, dontAsk}` —— `07:500/801` 会 `block` 任何 edit/exec 档执行；
2. 该回合以 `tierFilter:'read'` 启动 —— 注册层根本没给 edit/exec 工具。

**明确不算**：模型在 say 里承诺只读、任务标题看起来像研究、`permissionMode:'default'` 但「大概率不会写」。原注释说的乐观放行**仍然不做**。

**放宽的内容（且只有这些）**：
- 只读回合**不占** `cwd-write` 锁：不被写者挡，也不挡写者；`stewardArbiterCwdLock` 对它返回 null。
- 只读回合与写回合、只读与只读，都可同 cwd 并发。**两个写回合仍然互斥，一字不改。**
- 并发时**双方都拿到 `rivals`**（写者进场时看到有只读者在读；只读者进场时看到有写者在写）。

**`rivals` 进提示词（新管道）**：在 `09:1445` 的 `volatileExtras` 追加一段，文本由 06b 出（中英同步）：
「同一工作文件夹里此刻还有：「标题 A」（在写）、「标题 B」（只读）。它们可能正在改你要读的文件；写文件前先重读，不要凭旧内容改。」
- 数据从 13n 的 `running` 表在**授予时**取，经 `StewardHooks` 延迟绑定送到 09（13n 晚于 09，只能这么走，与收件箱同款）。
- **用标题不用 id**（117l D5 纪律）；**只列在跑的**，不列排队的。
- 这段是易变层：**不进 stable、不吃前缀缓存**；预算：最多列 5 条，超出写「…另有 N 条」。

#### 11.22.3 切法（一刀，两个提交，串行；**W1 ①②③ 全落地之后才派**）

| 提交 | 做什么 | 面 |
|---|---|---|
| ① 只读谓词与锁放宽 | `13n` 加 `stewardTurnIsReadOnly(entry)`（读 permissionMode / tierFilter，**不读任何文本**）；`stewardArbiterCwdLock` 对只读 entry 返回 null；`rivals` 计算抽成一处，写者与只读者都算 | `13n`、`13q`（入口传 tierFilter）、`thread-arbiter.e2e`、`steward-events.static` |
| ② rivals 进提示词 | 06b 加文案；`StewardHooks` 加一个「给我这条会话此刻的 rivals」的钩；09 在 `volatileExtras` 追加 | `06b`、`06i`（钩子契约）、`09`、`prompt-snapshot.static`、`capabilities.e2e`（真请求体里看到那一行） |

**为什么先 W1**：W1 把「本来就该分开的事」分到各自的子工作区之后，剩下真在同一个 cwd 的才是「确实该共处一室」的情形——那时放宽的面才小到看得清。

#### 11.22.4 待拍板一处：要不要放宽这条安全边界

| 选项 | 内容 | 代价 |
|---|---|---|
| **A（推荐）** | 只对**机械只读**回合放宽（plan/dontAsk 或 tierFilter read），两个写回合仍互斥 | 放宽面很窄；用户想让研究线程与写线程共处，得把研究线程设成 plan 档（W1 派生的子工作区 + 这条，已覆盖用户报的场景） |
| B（原稿「只读乐观放行」） | 按模型意图/标题猜只读 | 猜错＝两条线程一起改同一棵树，正是原注释拒绝的那件事 |
| C | 不放宽，只做 rivals 进提示词 | 最安全；但用户报的「同一个文件夹被占着」在 W1 之后若仍出现，就只剩排队 |

推荐 A。理由：它放宽的每一分都由 `07:500/801` 兜底，管家或模型说什么都改变不了工具面。

#### 11.22.5 验收（可证伪）
1. 同 cwd：plan 档线程 vs 在跑的 default 写者 → **不被挡**，其提示词含 rivals 行（标题、不含 `sess_`）；
2. 同 cwd：default 线程 vs 在跑的 default 写者 → **仍被挡**（回归保护，一字不改）；
3. 同 cwd：写者进场时已有 plan 只读者在跑 → 写者不被挡，且写者提示词里列出只读者；
...
**折叠区去重小刀交付（`4ee212d`；主会话核过）**：三个文件（`steward-chips.js`、`unit/steward-model-menu.test.js`、`steward-drawer.e2e.js`），`recent` 先算、
`folded = visible.filter(nonText && !inRecent)`，`inRecent` 按 `(group, id)` 去重；`looksNonTextModel(` 仍恰好 2 处（N5b 不动）。反向：撤掉去重单测 6 红、e2e 7 红（M8d 超时把 E3* 冲成级联，非真红）。
重钉的既有断言都是「判据跟着事实换」——M4b 拆成「四个都被打了 `data-model-non-text`」与「折进去的正好是那三个没账的」两件事分开对账，不是把 4 改 3。

**它登记的三条，裁决**：

| 债 | 裁决 |
|---|---|
| 「全部模型」分组段与「常用」对**文本**模型也重复（常用 5 行＋分组段同一 id 再一行） | **接受，且与折叠区去重不矛盾**。原则：**「常用」是快捷视图，「全部」是完整清单**，快捷＋完整并存是选择器的常规做法（§11.17.8 已裁过）；而**折叠区的语义是「因猜非文本而被藏起来的」**，一个已在常用段露面的项不再是「藏着的」，把它计进「× N」就是假数。两处裁决各按各的语义 |
| 去重 key `(provider, model)` 眼下等价于只按 id（`modelOptions()` 一次只出一个 provider 的候选） | **接受**，为将来一张菜单混多 provider 预留的形状；没有夹具能证伪 provider 那一半，**登记** |
| 有账的非文本 id 被 `slice(0,5)` 挤出常用后会回到折叠区并计数 | **正是裁决的自然推论**（「还没露面的」）；无专门锁，**登记** |

#### 11.19.8 W1 提交②③ 交付记录（`21e3b49` ＋ `3a5aa1c`；主会话逐条复核）

**核过的（含我自己两次核错后重核）**：18＋15 个文件零前端；工作树干净；**真实 `~/Ruyi` 不存在**（零污染）；
`04:6 sanitizeFsSegmentName` 的正则字面量 `/[<>:"/\|?*\x00-\x1F]/g` 与抽取前 `makeAttachmentRecord` 内联的**逐字相同**，只加了 `String()` 包壳
（我第一次用宽泛正则 grep 命中的是 04 里另一把 `/[^A-Za-z0-9_]/g`，**核错了一次**，改为直接比对两处源码才算数）；
`STEWARD_WORKSPACE_TABLE_MAX = 20` 只在 `06i:46` 定义、`STEWARD_CWD_CANDIDATES_MAX` 零残留；`13o:302` 候选表推在总览（`:303`）**之前**；
`steward.stable` zh 段与 `f1e512e` 逐字节相同（1280 B）；graph 49/390、forwardEdges 67、build 新鲜；七个源码文件 NUL 0／CRLF 0；
`steward-quick-ask.e2e.js` 的越界 diff 恰好 3 行（给夹具指定 `stewardWorkspaceRoot`，否则跑测试会在真机建 `~/Ruyi/…`——它逐一核了另外 10 个会调 thread_new/quick_ask 的夹具，只这一个漏）。

**它与设计页的三处出入，全部接受**：① `thread-<id8>` 按去掉 `sess_` 后取 8 位（字面前 8 位只剩 3 个有效字符，几乎必撞名）；
② 「标题全是非法字符 → 回落」与「非法字符换 `_`」矛盾（换完永不为空），改为并连续替身 → 剥首尾替身／点／空格 → 空才回落，副作用 `"报告"`→`报告` 更干净；
③ `durable-state-inventory` 没报 stale，是**新增**了一条目录面 `steward-workspace-derived`（工作台第一次主动在用户主目录下建目录，该记）。

**它抓到的两条假锁与两条「真红伪装成别的病」，照录**：O6 夹具第一版 3 行时常量分叉与否都绿——必须用 20 行才证得伪；
N12 清空动作在派生失效时会删掉夹具临时 HOME，后面几十条锁全变 `steward.disabled`；O5c 裸下标在表被拿掉时抛 TypeError 打断整件。三条都修在锁里。

**它登记的十条，裁决**：

| 债 | 裁决 |
|---|---|
| **`01-config` 对 `workspaces` 的 20 行帽子**（`if (clean.length >= 20) break;` 两处）使候选表折叠句**生产不可达**；更重要的是**用户已有 20 个工作区时派生行下次 normalize 就被截掉**，线程 cwd 于是指向表外目录、再用会被拒 | **真洞，主会话亲核了那两行。立 W1④，合并后第一刀**：帽子抬到 64（表上限 20 因此在生产形状下可达、可测）；派生若追加后仍会超帽 → **fail-closed** 拒开线程并明说「工作区表已满」，**绝不**出现「目录建了、行没了」。锁：帽满时派生要么行落盘要么拒，二者必居其一 |
| `steward-quick-ask.e2e.js` 越界 3 行 | **接受**，且是必要的（见上） |
| `13f-native-tool-schemas.js` 越界（工具 schema 真实住址，派单没列） | **接受**，§11.19.4 的面补上它 |
| `route-inventory.json` `coveredBy` 多出 M2 那份 e2e | **接受**——这正是 §11.20 新纪律（并行刀之后统一重跑）在起作用 |
| 没有「真回合请求体含候选表」的锁（无现成请求体捕获面） | **登记**，与 117y 的同一条并案 |
| `write:false` 仍能当 cwd | 已归 E-手②（§11.19.7） |
...
#### 11.19.9 W1④ 交付记录（`b9453c7`；主会话逐条复核）

**核过的**：16 个文件零前端零 13o；`WORKSPACE_TABLE_CAP = 64` 只在 `01-config:499` 定义，清洗块里裸 `20` **零出现**、两处都读常量；
`13k:166 stewardWorkspaceTableFull` 在 thread_new（`:551`）与 quick_ask（`:822`）都挂在 `createSession` **之前**；`STEWARD_WORKSPACE_TABLE_MAX` 仍 20
（两个数治两件事：64 是表能长多大，20 是到访层印多少行——折叠句因此在生产形状下可达，P5b 实测「…另有 5 个工作区未列出」）；
graph 49/390、forwardEdges 67、build 新鲜；三文件 NUL 0／CRLF 0；真机 `~/Ruyi` 不存在、真机 config 的 5 行 workspaces 未被碰。
反向：退回帽 20 ＋ 摘掉预检 → P 段 12 红，其中「P2b 目录建出来了**仍绿** ∧ P2c 行落盘**红**」正是要治的坏形状；`steward-relay-channels` 的 20 行池在 64 帽下仍绿。

**它做得比简报对的两处**：① 锁 1 用 63/64 行而不是我写的「20 行表→第 21 行被吞」——20/21 的夹具修后是绿的、钉不住 64；
② 抬帽后 `steward-tools.e2e` O5/O6 与 `static` ⑨ 的注释「01-config 自己截到 20 行，所以折叠句生产不可达」**不再成立**，它改了注释、断言一条未动。
越界改了 `durable-state-inventory.js:82` 那条「caps at 20 rows」的陈述——不改就是生成物里留一条假事实，**接受**。

**它登记的债，裁决**：

| 债 | 裁决 |
|---|---|
| **帽检查与登记之间有竞态**：两条线程在 63 行时同时过预检 → 两次 append → 65 行 → 下次 normalize 截一行，「目录建了、行没了」在这个窄窗口仍可复现；`mutateConfig` 串行化了写但不重算帽子 | **真，登记为下一把小刀「先占位再建目录」**：在 `mutateConfig` 的串行段里做「占位＋帽检查」原子化，目录在占位成功后才建；占位失败不建目录。与下一条同刀 |
