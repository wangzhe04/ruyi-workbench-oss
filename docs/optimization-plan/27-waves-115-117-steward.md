...
**派单顺序**：116-3 已收口（2026-09-07，三 commit 全入库）→ 116-4（引擎侧第四源＋唤醒链，派单稿全文见下）→ 116-5（线程自动摘要，需先出一页设计再派）→ 117j（前端走查修复，派单稿全文见下；串行以免回归互相冲突；117j 的 W2-4 依赖 116-4 的 `?since=`，线程标题改用 `brief.title` 依赖 116-5——若 116-5 未出门，117j 先按 `stewardShortTitle` 截断做）。
