...
## 8.13 干净全量回归的真实数字，与它翻出来的三件事（2026-09-08 深夜，主会话亲跑）

### 数字

隔离工作树、`--parallel 4`、`realhist-fixtures` 就位、`build --check` 绿、跑完 0 个残留浏览器：
...
15 条失败逐条**串行**复验：**13 条单跑全绿**（并行负载压出来的）。
确定性红只有 **2 条**，都在 `9425b8b` 里重钉了（见下）。

### ① 两条确定性红：都是「锁钉死了一个字面量，一次合法改动把那个字面量挪走了」

| 件 | 钉的是 | 谁挪走的 | 红了多久 |
|---|---|---|---|
| `kimi-agent-cli.e2e.js:122` | `navigation-controls.js` 里**硬编码的中文**「默认（Kimi 原生压缩）」 | 117q-B3a 把它换成 `t()` 搬进 locale（正事） | 自 B3a 落地起 |
| `pretender-narrative-notifications.e2e.js` D1 | 模板串里的**变量名** `${feed.cursor}` | 117m-A3（`90691d3`，我自己那一刀）把形参改名成 `revision` | 自 117m 起 |

加上此前重钉过的 `pretender-dispatch-home` D2、`steward-walkthrough` F4，**本波四条同模具**。

> **锁不要钉「文本长什么样」，要钉「哪件事必须成立」。**
> 钉用户可见文案的锁，一做 i18n 必假红；钉变量名的锁，一改名必假红。
> 这类锁不是在守护契约，是在**阻止重构**，而且它们坏掉的时候**没有任何人会发现** ——
> 直到某次全量回归里冒出来，还被当成新问题去追。
> （另一面见 27 号文 §11.12 的横向账：注释里写了「唯一判据／恒／一定」却没有断言钉着的，
> 是同一枚硬币 —— 一个是钉错了东西，一个是压根没钉。）

重钉时两条都配了**更强**的伴随断言并做了反向验证：把游标写死成 `after=0` → D1 真的红；
把某一份 locale 里的 `ctx.compact.defaultKimi` 清空 → 新加的 D0b 真的红。

### ② 「等在一个并不蕴含你要的那件事的信号上」还剩两种变体没修

本波已修的三种（P1-31／P1-32／P1-33）之外，这趟回归又刨出两族：

| 变体 | 回归里的样子 | 面 | 状态 |
|---|---|---|---|
| ~~HTML `meta[name=wcw-token]` 抓 token~~ | `FAIL UI token scraped` ×5 | 82 件用这个惯用法 | **已复核，改判 —— 见 §8.14** |
| 一个我 grep 漏了的健康等待助手 | `FAIL workbench listening on :<port>` ×7、`workbench up on :<port>` ×2 | ≈9 处 | 未派 |

第二族（grep 漏掉的健康等待助手）的根因与 P1-31 完全同形，**不要**当成新问题重新论证；派单时直接引这一段。

> **第一族已被推翻**（2026-09-09 复核，§8.14）：HTML 抓取这条路在【首次启动】**没有**竞态（同时刻双读实测：文件那条路 11/60 输，HTML 这条路 0/60）；
> `FAIL UI token scraped` 的真根因是抓取那一次的 `timeout: 1500` 在重载下被击穿（已复现 6/64，全部是超时、零空 token）。
> **它是预算族，不是 P1-31 那个「等错信号」族。**

### ③ 一条真信号，而且它是**断言**不是等待预算 —— 要用户拍板

```
② cold start /health ready < 5000ms (got 6115ms)
```

这条与上面两族**不一样**：它不是「等得不够久」，它是一条**断言**，写的是
「这台机器冷启动应该在 5 秒内答 `/health`」。本机实测 HEAD **4657ms**、`dd8f15f` **6282ms**，
也就是说这个阈值现在**骑在真实耗时上**。

- 把它按实测放宽 = **放宽一道门**，那是产品决定，不是维护动作，**我不自己拍**；
- 不放宽 = 承认冷启动确实变慢了，该去查为什么，那是另一个立项。

**挂起，等用户拍板**（与 §8.5 那两条并列）。

### ④ 19 个抖动件的名单（登记，未治理）

`changes-diff`、`context-window`、`e4-no-usage`、`index-dedup`、`session-atomic`、`usage-accum`、
`budget-guard`、`claude-cmdline-guard`、`repo-hygiene`、`mcp-ops-closure`、`rewind`、`session-index`、
`autonomy-durability`、`capabilities`、`onboard`、`port-fallback`、`provider-compact`、`shell-session`、
`steward-settings`。

治理纪律见 §8.12 末段：**抖动的件只能用通过率比较，不能用单次采样比较。**

**117r 补登一件（干净回归那趟没抓到，本波实测出来的）**：`steward-conversation`。
主会话在 117r 五刀全部落地后的合并态连跑 4 次：**3 绿 1 红**。
红那次只留下了汇总行（`FAIL (1)`），没留下是哪一条 —— **如实记：判定行没抓到**。
117r-D3 的执行者独立报告了同一现象，并用 `git show HEAD~1` 取出**改动前**的原始件连跑两次复现，
据此判定与本波改动无关。两处观察互相印证，但**都没拿到失败断言名**，
治理时要先跑一轮带完整输出的采样。


## 8.14 `getToken()`（HTML `meta` 抓 token）这一族的复核：**首次启动没有竞态；真正的假红源是 `timeout: 1500`**

117q-P1-33（`ff4939c`）的提交说明把这族留成了尾巴：

> 驱动整套 `dev-harness/` 下与 `getToken()`(HTML meta 抓取)同款 idiom 的另外约 75 个文件未纳入本刀 ——
> ……该抓取路径读的是内存态 `RUNTIME.token`（listen 与其赋值之间零 await），与本刀修的「直读落盘文件」
> 竞态机制不同，留作后续单独复核。

本节就是那次复核。**面 82 件**（`grep -l 'name="wcw-token"' dev-harness/`，其中 `agent-team-mode.e2e.js`
的 `browserToken()` 已在 P1-33 里改成轮询）。

**两句话结论**：

1. **首次启动的竞态不存在，81 件不比照 P1-33 改。** 机制、语义、实测三层都指同一个方向（§①②③）。
2. **但 §8.13 ② 记的 `FAIL UI token scraped ×5` 是真的，只是根因不是竞态 —— 是抓取那一次给的
   `timeout: 1500` 在重载下被击穿。已在本机复现（6/64，全部是超时、零空 token），见 §④。**

据此，§8.13 ② 表里「HTML `meta[name=wcw-token]` 抓 token → 已登记为独立任务」那一行**改判**：
它**不是** P1-31/P1-33 那一族（等错了信号），而是**预算族**（等对了信号但等得不够久）。

### ① 源码事实（四条，都可 grep 复验）

| # | 事实 | 位置 |
|---|---|---|
| 1 | `RUNTIME.token` 全仓**只有一处赋值**，此后既不轮换也不清空 | `13-http-router.js:1776`（`grep -rn 'RUNTIME.token =' app/src/` 唯一命中；`RUNTIME` 在 `01-config.js:2266` 声明后从未重绑，也没有 `Object.assign(RUNTIME, …)`） |
| 2 | `listen` 返回（`:1772`）到该赋值（`:1776`）之间**零 await**：中间只有一个模板串和一次**同步**的 `crypto.randomBytes(16)` | `13-http-router.js:1772-1776` |
| 3 | **落盘在赋值之后**（`:1777` `await atomicWriteJson(runtime.json)`）—— P1-33 修的那条竞态就长在这里，也正是两条路径**不同形**的地方 | `13-http-router.js:1777` |
| 4 | `index.html` 的 token 是**请求期实时读内存态**，不是启动时快照 | `01-config.js:2455`：`.replace('__WCW_TOKEN__', … (RUNTIME.token \|\| ''))` |

事实 2 不是靠人自觉维持的，**已经有源码锁钉着**：`start-error-surface.e2e.js:265-270` 逐字断言
「listen 与 `RUNTIME` 握手常量赋值之间零 await」，注释里还留着当初踩坑的现场
（第一版把 `consumeStartError()` 放在 listen 之后，capabilities e2e 稳定复现空 token）。
**本节不需要再加锁 —— 该加的那把锁已经在，钉的正是本节赖以成立的那条不变量。**

事实 1 顺带说明：首次启动判据取「非空」是**充分**的（与 P1-33 同口径），
不必像 P1-32 的重启点那样要求「与旧值不同」。

### ② 事件循环语义：为什么「零 await」就够

要证的命题比「`/health` 200 蕴含 token 已就位」**更强**：

> **只要一个请求被应答过（任何请求，不限于 `/health`），`RUNTIME.token` 就已经就位。**

推导：

1. `'listening'` 由 Node 在 **nextTick** 回调里发出（`net.js` 的 `emitListeningNT`）；
2. `await listenOnce(...)` 的恢复、以及 `listenWithFallback` 那层 `async` 返回的恢复，全是 **microtask**；
3. nextTick 队列与 microtask 队列在**回到事件循环之前**排干，排干过程中新入队的 microtask 也在同一个
   checkpoint 里跑完 —— 所以 `listen → :1776` 这条同步续体**必定**在这个 checkpoint 内跑完；
4. 新连接的 accept / 解析 / 分发只可能发生在 **poll 阶段的回调**里，即那个 checkpoint **之后**。

所以不存在「已经能应答、但 token 还没写」的窗口。P1-33 那条竞态之所以真实，是因为它要等的东西在
`:1777` —— 一个 **await 之后**的落盘，那才是 poll 阶段能插进来的位置。

### ③ 实测（本机、本 HEAD）

探针：从工作台起来**之前**就开始 `connect()` 轰击目标端口，一连上立刻裸写 `GET /`，
把「可服务后头 40 ms」内收到的**每一个**响应都收下来，数其中渲染出空 `meta` 的比例。
连接能在 `uv_listen()` 成功的瞬间进内核 backlog（早于进程发出 `'listening'`），
所以这是客户端物理上**最早**能被服务到的时刻。

**A. 合成对照 —— 先证明探针不是瞎的，再隔离机制**（60 轮／形状，handler 同步读 token）

| 形状 | listen 与 `RUNTIME.token=` 之间放了什么 | 空 token 响应 |
|---|---|---|
| `A0` | 什么都没有（= 真实路由的形状） | **0 / 5558** |
| `M0` | `await null` ×3（只让出 **microtask**，不回事件循环） | **0 / 5564** |
| `I0` | `await setImmediate`（**回事件循环的最小让出**） | **18 / 5510**，15/60 轮命中 |
| `C0` | `await fsp.readFile`（线程池往返） | **1231 / 5586**，44/60 轮命中 |

`M0` 与 `I0` 这一对是本节的**机制证据**：分界线正好是「有没有回到事件循环」，
不是「源码里有没有出现 await 这个词」。真实路由连 `M0` 都不如 —— 它一个 await 都没有。
`C0` 同时证明探针**灵敏**（23% 的响应能抓到空 token），所以后面那些 0 不是探针钝。

**B. 真实工作台 —— 同一个进程、同一时刻、两种机制对照**（60 轮冷启动，每轮全新 HOME）

在 `/health` 首次答 200 的**同一瞬间**，先按 P1-33 修掉的那种写法单次直读 `<data>/runtime.json`，
紧接着按 `getToken()` 的写法抓一次 HTML：

| 读法 | 结果 |
|---|---|
| `runtime.json` 单次直读（P1-33 修掉的写法） | ok 49 / **ENOENT 11** → **竞态 11/60（18%）** |
| `index.html` `meta` 抓取（`getToken()`） | ok 60 / 空 0 → **竞态 0/60** |

> 本节最硬的一条证据：**同一个进程、同一时刻，文件那条路 18% 会输，HTML 这条路一次都没输。**
> 顺带是 P1-33 那刀在真实二进制上的复现 —— 11 次全是 ENOENT，没有一次「文件在但 token 为空」，
> `atomicWriteJson` 的 rename 语义决定了读不到半截。

**C. 真实工作台 —— burst**（40 轮，收下可服务后 40 ms 内的全部响应）

**1966 / 1967 带非空 token。** 唯一那个不匹配的样本，字段自证不是「服务吐了空 token」：
`metaPresent:false, status:"?"`，且发生在首个响应之后 **4141 ms** —— 那是探针自己的 5 s 套接字上限，
是一个**始终没被应答**的连接。真竞态的签名长什么样，看 `C0` 的样本：`status=200, metaPresent=true, token=""`。
（这个「没被应答」的样本本身预告了 §④。）

### ④ `FAIL UI token scraped ×5` 的真根因：`timeout: 1500` 被击穿（已复现）

按 `audit.e2e.js:24-25` **逐字**照抄健康轮询 + `getToken()`，在两档负载下重跑（每档一轮全新冷启动）：

| 负载 | 探针数 | `getToken()` 结果分类 | 健康等待 | `GET /` 时延 |
|---|---|---|---|---|
| 4 路并发 + 4 CPU burner | 40 | **ok 40，非 ok 0** | p50 3400 / max 4083 ms | — |
| 8 路并发 + 12 CPU burner | 64 | **ok 58，`timeout1500` 6，空 meta 0** | p50 23947 / max 37573 ms | min 4 / **p50 584 / p90 2083 / max 2828** ms |

六个失败样本全是同一种：`why:"timeout1500"`，`ms` 落在 **1505 / 1535 / 1544 / 1637 / 1815 / 2828**，
即请求**还在飞**，预算就到期了。**一个 `empty-meta` 都没有。**

两条要点：

- **`GET /` 的 p90 已经是 2083 ms，越过 `1500` 这条线。** 而 82 件里 **41 件**的抓取预算正是
  `timeout: 1500`（`grep 'name="wcw-token"' dev-harness/*.js | grep -o 'timeout: *[0-9]*'` → 40 个 `1500` +
  1 个 `2000`；另 41 件的抓取走各自的 `get()` 助手，预算另算）。同一批文件里 `getJson` 普遍给 `5000`、
  `reqJson` 给 `15000` —— **抓 token 这一次给的是全文件最紧的预算。**
- 为什么刚起来的工作台会慢：`listen` 之后紧跟着一串 fire-and-forget 的开机风暴
  （`syncMcpServersToClaude` / `getCapabilities` / `storageSweep` / `warmPretenderProjectionIndex` /
  `autoResumeInterruptedRuns`）。它们**不挡 listen，但挡事件循环** —— `/health` 是同步答的所以照样通，
  而 `GET /` 要 `await fsp.readFile(index.html)`，排在风暴后面。
  这也解释了 §③C 那个「连上了但 5 s 没被应答」的样本。

**如实记两件事**：4 路那档（更接近回归实际的 `--parallel 4`）**没能复现**，
所以「回归里那 5 条就是这个原因」是**强倾向性判断，不是坐实**；
以及本机 8 路那档的健康等待 p50 已经到 24 s，负载形态与真实回归并不同构。

### ⑤ 结论与建议

1. **81 件的抓取逻辑不改。** 首次启动的 HTML `meta` 抓取**不存在** P1-33 那条竞态。
   §8.13 ② 表里那一行按 §④ **改判为预算族**，不要再当「等错信号」派单。
2. **不新增锁。** `start-error-surface.e2e.js:265-270` 钉的就是本节赖以成立的不变量，位置和判据都对。
   反过来说：**那把锁一旦被人以「反正只是加一个 await」为由拆掉或放宽，本节全部结论立即失效** ——
   §③A 的 `I0` 行给出了代价：一次 `setImmediate` 量级的让出，就足以让 0.3% 的响应吐空 token。
3. **建议的下一刀（未做，等拍板）**：把那 41 件里 `timeout: 1500` 抬到与同文件 `getJson` 同量级（`5000`）。
   - 为什么不是包一层轮询（P1-33 的做法）：轮询治的是「**值**还没就位」，而这里值一直就位，
     没就位的是**响应**。轮询当然也能把它盖住，但盖住之后注释里会写下一条**假的**因果，
     下一个人照着这条假因果去别处找竞态 —— 本波 §8.12 说的「缺陷主要来自派单稿」就是这么长出来的。
   - 为什么要拍板而不是顺手做：§5 已经写死「合并时不许把各件预算**拍平成一个默认值**」，
     41 件逐件抬预算是**口径决定**；而且本节只在 8 路+12 burner 这档复现出来，4 路那档是 0/40。
4. 复现脚本是一次性的，**没有入库**（跑满四组约 25 min，不适合进 `run-all`）。方法在 §③④ 写全了，
   照着重建即可：合成四形状对照 + 真实工作台「同时刻文件／HTML 双读」+ burst + 逐字照抄的负载跑。

### ⑥ 顺手翻出来的另一件事：**重启点**再抓 token 是**另一族**真隐患（未修，登记）

本节只否掉了**首次启动**。82 件里有两件在**同端口重启后**再抓一次 token，
而它们等的是「`/health` 有响应」—— **这个信号不区分是哪个进程在答**：

| 件 | 位置 | 现在等的 |
|---|---|---|
| `session-index.e2e.js` | `:196` `killp(wb); await sleep(700);` → `:201` 重启 → `:204` 健康轮询 → `:205` `getToken()` | `health(WB_PORT)` 非空 |
| `steward-quickask-board.e2e.js` | `:100` `killp(wb); await sleep(400);` → `:121` 重启 → `:122` `waitUp()` → `:123` `getToken()` | 同上 |

`taskkill /T /F` 只是**发出**终止；旧进程若还没死透，`/health` 会由**旧进程**答 200，
`getToken()` 于是抓回**旧 token**；等旧进程真死、新进程接管，后续所有 `x-wcw-token` 调用一律 403。
这与 P1-32 治的是同一族（「等在一个并不蕴含你要的那件事的信号上」），只是判据不同。

**正确判据现成**：`/health` 的响应体与响应头都带 `overlayId`（`13-http-router.js:1759-1763`，
注释写得明明白白 —— *"version alone can't prove a restart, so echo the per-process overlay id"*）。
重启点应当等 **`overlayId` 变了**，而不是等「有响应」。

**未修，登记。** 理由：判据从「非空」变成「与旧值不同」，与 **P1-32 同形**而非 P1-33 同形，是另一条刀；
且面只有 2 件，不该混进本节「81 件不动」的结论里。
`session-index` 正在 §8.13 ④ 的 19 个抖动件名单上 —— 治理那批时优先验这一条。
