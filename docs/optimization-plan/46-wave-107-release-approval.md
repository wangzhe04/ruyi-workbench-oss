...
| 引擎 | 105h、#13a／13a-t、#1 G1、111 五项 | 关 | 105h 无净收益、#1 G1 未过门、#13a 只有合成读数；**111 五项的真模型读数已到手，见 §5 E1**（111d／111e／111b 过门并建议默认开、111a 主指标反向、111c 量不出来；E1 只报不翻） | **2026-09-18 拍板后分家**：111d／111e／111b **翻成默认开**（T1，含 schema 12 一次性迁移）→ 归「已交付」且**有真模型读数**；111a／111c 仍是实验（默认关，Brief 要写明「量到了但没过门」与「量不出来」是两件不同的事） |
...
- ~~`CONFIG_SCHEMA = 11`（`00-boot.js`）自 2.7.0 未变，**本波不 bump**~~ → **2026-09-18 T1 作废：bump 到 12**。
  用户拍板「三个压缩开关默认打开」之后才发现，光翻 `defaultConfig` 的默认值**对所有存量安装都无效**
  （`{ ...defaultConfig(), ...raw }` 里 raw 永远赢，而历次整份回写已经把这些键冻在盘上；主会话实测见 §5 T1），
  所以必须有一次性迁移，而本仓迁移的既定通道就是 `incomingConfigSchema < N`。这是**显式改判，不是静默豁免**。
- 从 2.7.0 升级：新键取默认值——**升级用户会自动获得代批开、定时任务开、新线程跟随上次引擎**，
  **外加 T1 的三个压缩开关**（`runtimeHistoryReadDedupV1`／`runtimeSummaryPromptI18nV1`／`runtimeReseedTailUnitsV1`）。
  后者与前三者不同：它**会改写盘上已有的 `false`**（那个 `false` 不是用户的选择，是历次整份回写的产物），
  这一点 CHANGELOG 必须点名。**升级之后用户自己关掉的（schema 已是 12），永远不会被再打开**——有常驻单测守。
...
> - **翻默认怎么落地**（主会话实测「光翻默认对存量安装无效」后交回的三选一）：选 **「翻默认 + 一次性迁移」**，见 §5 T1。
>
> **2026-09-18 用户拍板 · 3.0 正名（43 号文 §2 107 行 ⑤ 那件「与发布批准分开的决定」）**，原话：
> 「按你的建议来，2.8.0 先不叫 3.0；把当前 128+ 后所有的既定计划完成后再正式 3.0 发布吧」。
> - **2.8.0 以 Escapade 2.x 发布，不正名。**
> - **3.0 的前提** ＝ 主会话建议的**偿债波**（默认值冻在盘上的结构性缺陷、存盘层陈旧快照守卫通用化、G3 三个测试缺口）
>   ＋ **128+ 全部既定项**（H01、H02→H03、X01、X02、X03、E02、114d、114e、22 号文引擎候选池）
>   ＋ 路线图 :86 的 **P4 收尾硬门**（安全红队终审、性能终验、双主题／双语／无障碍终审、离线升级与数据迁移恢复演练）。
> - **「完成」的口径（主会话理解，已告知用户）**：H02／H03 与引擎候选池**按设计就是证据触发**，
>   所以对它们而言「完成」＝ **走完各自的证据门、有结论**（做了，或证据不支持而明文关闭），不是「全部落地」。
> - **到 3.0 时要重议的一件**：路线图 :81 写明「若未来重新立项 3.0，需从（跳过正式人因验证）这个决定重新评估」。
>   现在不问，3.0 发布批准时再议。
...
### T1 · 111b／111d／111e 翻成默认开 ＋ schema 12 一次性迁移（2026-09-18，提交 `9127ecf`）

**为什么这一刀比「改三个 false」大得多**：用户拍板「默认打开」之后，主会话先去核「改默认值够不够」，
结论是**不够**——

- 用户真机 `config.json` 里这三个键（连同 111a／111c 那两个）**都已实打实写着 `false`**，`configSchema` 是 11。
- 机制：`normalizeConfig` 是 `{ ...defaultConfig(), ...raw }`（raw 永远赢），而 `readConfig` 只要 `changed` 为真
  就把**整份合并后的配置**序列化落盘（`01-config.js:1577`）——于是任何一次迁移写盘都把**当时所有默认值**冻在盘上。
- 主会话拿**没在改动中的** `dist/Ruyi-slim`（2.7.0 构建产物）实测确认：用户真配置过一遍归一 `changed = false`、三个键仍 `false`、**不会被纠正**。
- ⇒ **光翻默认只对「盘上没有这个键」的全新安装有效**，对所有存量安装是空操作。
- **结构性缺陷（记债不修）**：此后**每一次改默认对存量安装都无效**。根治要改写盘策略（只落用户真正改过的键），面太大。

于是把这件事作为**决策**交回用户（三选一：翻默认＋一次性迁移／只翻默认／只改这台机器），
用户选 **「翻默认 + 一次性迁移」** 并明确接受「会改盘上的配置」这个代价。

**改了什么**

- `01-config.js:123/129/137` 三个键 `false → true`；三段注释改成「**缺省不再等价旧行为**，只有显式 `false` 才回旧路」。
- `00-boot.js:38` `CONFIG_SCHEMA` **11 → 12**。
- `01-config.js:826-830` 迁移（在严格布尔归一循环**之后**）：`incomingConfigSchema < 12` 时把三个键里的 `false` 改成 `true` 并置 `changed`。
  **必须写 `= true`，不能写 `delete`** —— 这是开工前就点名的陷阱，反向验证 ⑥ 专门钉它。
- `01c-runtime-flags.js` 三个判定函数上方的口径注释同步（「显式 false / **缺省**保证…」→「只有显式 false」）。
- 顺手修 `01-config.js` 里一处**注释说假话**：105b 那段写着 `runtimeSessionNotesInjectV1`「(默认关)」，
  而它实际默认开、它自己的注释也写着「真实历史+端到端采用门通过后默认开启」——105d 翻默认时漏改的陈注。
- 锁：`runtime-optimization.static.e2e.js` F2／F3／F4 各两条改判 ＋ 新增 `unit/config-schema-12-migration.test.js`（5 组常驻）；
  另有两处硬钉 `CONFIG_SCHEMA` 字面值的静态件跟着改（现在钉 12：`asr-config-ui.static.e2e.js:44`、`steward-config.static.e2e.js:100`）。
- 文档：CHANGELOG 中英、`ADMIN-GUIDE_CN/EN`、`ARCHITECTURE_CN/EN`、README（unit 件数 52→53）。

**锁为什么不能简单反着写**（这一条值得单记）：原来 F2 有两条——「默认是 `: false`」＋「别处没有 `: true`」。
翻开之后第二条**不能**改成「别处没有 `: false`」：默认点自己就是 `: true`，那条会退化成一句**永真的空话**。
判据改成**钉出现次数**：`runtimeXxxV1:` 在整份产物里**恰好 1 处**。于是「默认点被改回去」由第一条红、
「别处又写了一遍默认值」由第二条红，两种事故各有各的锁，且 `hits === 0`（默认点没了／本条静默失效）也会红。

**主会话亲验（不是转述）**

- 树与提交：HEAD `9127ecf`，`git status` 干净，19 个文件。
- 生成器链：`build --check` 新鲜；依赖图 **PASS（53 modules, 420 edges）**；`route-inventory --check` **OK（137／125）**——两项与基线同数。
- 全量：主会话直接读 `dev-harness/last-run.log` 汇总行 = **356 pass / 0 fail / 0 known-fail / 0 unexpected-pass / 1 flaky / 356 ran / 7 skipped**（flaky＝`steward-settings.e2e.js`，重跑过）。
- 新 unit 件与 `runtime-optimization.static` **主会话自己各跑一遍**：5/5 pass、`ALL PASS`。
- 19 个改动文件逐字节扫 NUL：**0**。
- **迁移的端到端亲验**（用新构建产物、隔离 HOME、把用户真配置只当数据读，六种情形）：

  | 情形 | 结果 |
  |---|---|
  | A 用户真机那份（schema 11，三键 false） | 三个都变 `true`、`changed = true`、新 schema 12 → **拍板真落地了** |
  | B 幂等（把 A 的结果再过一遍） | 保持开、`changed = false` → 不会反复写盘 |
  | **C 升级后用户自己关掉（schema 12 ＋ 显式 false）** | **保持 `false`** → 本刀最该守住的一条成立 |
  | D 老 schema 6／9／10／11 | 四种都迁 |
  | E 全新安装（盘上没这三个键） | 取新默认 `true` |
  | F 脏值（手改写了字符串 `"false"`） | 先压成布尔再迁移 → `true` |

- **主会话自己做的反向验证**（不止看子代理的）：把 `< 12` 改成 `<= 12`、重建、跑单测 → **红，且红的理由正是那条性质**：
  「`runtimeHistoryReadDedupV1` 在 schema 12 下必须保持 false，actual: true, expected: false」。还原后 `git status` 干净、`build --check` 新鲜、单测 5/5。

**子代理报的、与计划不符的四处（主会话已核，属实）**

1. 计划**漏了两处会当场变红的硬断言**（两件静态锁都 `assert /const CONFIG_SCHEMA = 11;/`，现已钉 12：`asr-config-ui.static.e2e.js:44`、`steward-config.static.e2e.js:100`）。已改钉当前值，并把各波真正要守的「我这批键没有迁移分支」补成独立断言。
2. `ADMIN-GUIDE_EN.md` **不是「同位置同改」**——英文手册是删减版，没有 CN 那三行表格，要改的是 §8.2／§8.3（改完全文 334 行，`:259`／`:276`）。另外 CN §7.4「`CONFIG_SCHEMA` 仍是 11／升级用户会自动获得三件」与 CHANGELOG 2.8.0 节头那句「无 configSchema bump」计划都没点到，均已改。
3. 计划写 `ARCHITECTURE_CN.md:3`，实际路径在 `ruyi-workbench/docs/` 下（行号对）。
4. `delete` 陷阱的**机制**与计划描述不同：计划设想迁移在布尔循环**之前**（`delete` 会被循环压回 `false`），
   实做放在循环**之后**（脏值先压成布尔再统一迁移），于是 `delete` 的后果是本趟返回 `undefined`、判定函数 `=== true` 全落空。
   **结论不变**（必须 `= true`，锁照样红），只是理由是 `undefined` 而不是 `false`。

**没做 / 不确定**

1. 子代理**没有去读用户真机配置复核**「三个键都写着 `false`」这个前提（那是主会话的实测；主会话在本节 A 情形里又亲验了一遍）。
2. **没实测「回滚到 2.7.0 之后这三个键留在盘上」的行为**。2.7.0 没有这些键、也不读它们，推断无害，但没跑。
   → 归入 ⑧ P1 的降级演练一并验。
...
### F7b · 修法设计（主会话定）→ 已落地，提交 `b10202a`（交付记录见本节末）
...
#### F7b 交付记录（2026-09-18，提交 `b10202a`）

**真凶找到了，而且不是垂死回合**：负载复现里每个红轮都有一条被闸挡下的写，`writer:'session_meta'`、`keys:[launchedBy, stewardLastTurn]`——
是 13k `stewardRecordLaunchOutcome → updateSessionMeta` 的**读改写**：它在回合 settle 之后才 load，load 落在撤回那一存之前、save 落在之后。
69 波那道「等 settle」的闸只等垂死回合自己，**管不到回合之外的写者**——这解释了为什么 `rewind_autostop`／`rewind_settle_timeout` 两条日志都没出现。
代数闸不看是谁写：撤回之前读出来的副本，谁拿着都写不回去。

**实现**（`02-session-store.js`）：`sessionRewindGenHighWater`（与 `missionChangeSeqHighWater` 同处）＋ 头上 `rewindGen`；
`saveSession` 链内落盘前比对，低于水位整次丢弃并记 `session_stale_save_dropped`（被丢的写也跳过索引刷新——本来就没写盘）。
**设计之外补了四处，主会话逐条审过、全部认可，其中两处是守在丢数据那一侧的关键**：

1. **抬代数与入链原子**（`claimRewindGeneration`，在 `saveSession` 入链前的同步段里调）。两次撤回交错时后读的那次不抬、按陈旧丢，`rewindSession` 如实回 `rewind_superseded`，不说 `ok:true`。
2. **撤回那一存落盘失败时链内退水位**。**不退的后果**：盘上仍是旧代数，这条会话此后每一次存盘都被当陈旧丢掉——整条会话从此存不进东西。
3. **`loadSession` 自愈**：读到的代数低于水位 ⇒ 等写链跑完再读，有界两次。**不做的后果**：撤回刚抬水位、那一存还没落盘时来读的人拿着旧代数，**它之后的每一次存盘都会被误丢**——这才是真正的静默丢数据。
4. **`updateSessionMeta` 被丢时在新读的副本上重放补丁**（直接路径与延后路径都做，有界两次）。改名、`stewardLastTurn`、`threadBrief` 是真数据，不能跟着撤回删掉的正文一起丢。

**读数**：
- **确定性单测** `dev-harness/unit/session-rewind-gen.test.js` **9 条**（A 承重、B 丢数据方向、C 未撤回照常、D 连续两次撤回、E 模拟重启、F 元数据重放、G 读自愈、H 水位回退、I 两次撤回交错），每条都做了定点反向。
- **负载复现**（`f7-run.js --cpu 6`，只算有效轮）：修后 **23 轮 0 红**；只关比对 **19 轮 3 红**；比对与自愈都关（≈修前）19 轮 2 红；只关自愈 20 轮 0 红 ⇒ **承重的是代数闸**。
- **全量** `--parallel 4`：`356 pass / 0 fail / 2 flaky / 356 ran / 7 skipped`，exit 0（flaky：`steward-conversation`、`ec-d-performance`）。
  前一轮全量红过一件真的：`audit-w23` 的 P2#8 源码锁 6000 字窗口被新增代码挤出去——**修法是把抬代数抽成函数、改源码去适配，锁本身没动**。

**主会话亲验**：单测自跑 9/9；生成器链 **53/421、137/125 不变**；`build --check` 新鲜；13 个文件零 NUL；`git diff -- dev-harness/` 为空（**没改任何既有测试或锁**）。
**亲手做 B 的反向**：在 `loadSession` 里让新读的对象丢掉 `rewindGen`、重建、单跑 → **B 红，原文「撤回之后新写的那一条必须落盘（实得 0 行 —— 正常的存盘被当成陈旧丢了 = 静默丢用户数据）」**。
还原时撞到一个自己的坑：破坏时跑过一次构建，它把 `manifest.json` 的行区间按多出的那一行重算了，只还原源码与产物不够——
`build --check` 当场报 14-main 行区间过期，重新构建一次后 `git diff --stat` 与交付时逐文件一致。**反向验证要连生成物一起还原**（记忆 `reverse-check-can-be-wrong` 同族）。

**偶发件里又有 `steward-conversation`**——按记忆 `flaky-recurrence-means-bug` 的规矩**不打发**。
子代理称这次首跑**没有任何断言红行、是超时**，形状与 Q1 那次「G4 断言红」不同，且修后负载 23 轮零红——证据倾向「已修好、这次是负载超时」，
**但这是转述**（run-all 的首跑 FAIL 行只进控制台，`last-run.log` 不记）。→ Q1b 冻结树重跑由主会话亲自跑、**把完整控制台存文件**，再偶发就能直接看首跑是断言红还是超时。

**已知剩余（照实写进 Brief，挂偿债波「存盘层陈旧快照守卫通用化」）**：
1. **其它读改写调用方**（技能、记忆、`/api/todo`、`/api/mission` 的 check／start／update、手动压缩、08 工作流摘要、13d 检查点重存等）若恰好跨在一次撤回上，**会被丢且接口照样回 ok**——窗口很窄，有日志、无重放。修前同样的交错是把撤回盖回去；现在是把它们自己的小改动丢掉。**这仍然是一句假话，只是更罕见**，通用化的时候要给它们都配上重放或如实报错。
2. **旁车文件不过闸**：会话笔记、检查点 journal、干预／变更流水。垂死回合在撤回后仍可能写进会话笔记，**笔记可能把已撤回的内容再注入下一回合**。
3. **撤回不停任务驱动器在两个回合之间的续跑**（修前就有）：它的存盘现在都被丢了（不再盖回撤回），但它还会接着跑、接着花 token，直到预算用完或判停滞。
4. `rewind_superseded`（两次撤回交错时）在管家壳里以原串显示，没有 i18n。
5. 两类没有代数的对象（本进程里被撤回过的固定 id 管家会话再用字面量重建；v1bak 回退出来的对象）在重启前会一直被丢——界面上都走不到，只写在注释里。
6. 撤回自己也是读改写：别的元数据补丁若落在撤回的 load 与 save 之间，会被撤回盖掉（修前就有，代数闸不管这个方向）。
7. **经典壳的撤回**（回溯、整单回退、行动流水撤销）走同一个 `/api/session/rewind`，同一道闸一起覆盖；**没有单独做经典壳的负载复现**。

**同一刀期间的事故（主会话记，不是 F7b 交付的一部分）**：用户正在用的正式工作台（pid 10416，端口 8765，当天 16:34:59 还在跑回合）在这一刀期间没了。
桌面壳还开着 ⇒ 不是用户关的；F7 那一刀 14:30 已结束 ⇒ 排除；F7b 报告称「收尾把 node 残留进程清到 0」——**嫌疑最大，但子代理执行记录为空，没能坐实**。
主会话核过**用户数据无损**：配置可解析、C1 的 MCP 清理仍在；当天在用的会话头与正文一致（8 条、零坏行）；无残留临时文件。
**没有替用户拉起工作台**（正式实例，从主会话起的进程会挂在主会话底下），已告知用户按平时方式重启。
**根因在派单**：只写了「跑完收尸」，没写「只许杀自己 spawn 的 PID、严禁按进程名杀」，也没点名用户的工作台是禁区。已入记忆 `subagent-cleanup-kills-user-workbench`，此后每份派单都带上。
另：取证代理留下的 `f7-run.js` 收尾会**按名字杀本机所有 msedge**——再用它之前要先改掉。

### Q1b · 冻结树重跑（2026-09-18，主会话亲跑，HEAD `b10202a`）

**做法**：`node dev-harness/run-all.js --parallel 4`，**输出重定向到文件、不接管道**（`scratchpad\q1b-console.log`，首行 `START 18:59:10 HEAD b10202a`，末行 `EXIT=0 END 19:21:58`）。
判结束看进程（run-all 进程数 0）不看文件；`last-run.log` 写于 19:21:58，与 END 对得上。

**读数**：`356 pass / 0 fail / 0 known-fail / 0 unexpected-pass / 3 flaky / 356 ran / 7 skipped`，**run-all 退出码 0**。
（注意：后台命令本身报的「exit 0」是末尾那句 `echo` 的——**真退出码只看日志里的 `EXIT=`**。）

**三个偶发件，这次能直接看首跑红在哪**（控制台结尾的 `[首跑]` 摘要；控制台交错把它们标在了别的文件名上，名字以 `last-run.log` 逐件表为准）：

| 件 | 首跑 | 跨轮 | 处置 |
|---|---|---|---|
| `steering.e2e.js` | 没有 FAIL 行（超时／被杀） | 首次 | 跟踪 |
| `mission-index-scale.e2e.js` | 性能门：列表热 P95 **350 ms**，门槛 300 ms | 首次 | 性能门在 4 路并行下本就吵，跟踪 |
| **`steward-settings.e2e.js`** | **断言红** `B3 勾开总开关后 GET /api/steward/state 可用且 running` | **第二次**（T1 那轮也偶发，当时是「没有 FAIL 行」） | **按真缺陷取证 → F8** |

**`steward-conversation` 这次首跑就过**——F7b 修完后它在冻结树上干净了。

**主会话漏用自己规矩的一处，照实改**：`mission-threads.e2e.js` 在 **M1 与 Q1 两轮**都偶发过——**早就是第二次了**，
我却在 Brief 未完成项 12 里写成「再出现一次就按真缺陷取证」。按规矩它也该取证 → 一并进 F8。

**B3 为什么分不出来**：它**不是**读太早——`:355-366` 是 `waitForHttp` 轮询 200 次 × 80 ms（**至少 16 秒**）等管家报 running。
~~负载下整件 66 s／跑、单跑 12 s（慢 5 倍多），「慢到超出窗口」讲得通；~~ **（被 F8 证伪：133 s 是「120 s 超时＋一次正常重跑」，不是每跑慢 5 倍——主会话把总耗时除以二当成了单跑耗时）**但勾开关是「存配置 → 再 start」两步，
**负载下某一步丢了**同样讲得通（本仓前科：设置保存写回没读到的状态；127 波 F6 在同一设置面修过两处真的丢更新）。
紧跟着的 **B4「开关落盘」**本可以一锤定音，但 run-all 的首跑摘要只记第一条红，看不到 B4。→ F8 要在负载下把两步的状态码、盘上开关值、到 running 的毫秒数一起记下来。

**进程快照对照**（跑前 18:59 记、跑后对照）：如意桌面壳 `msedgewebview2` **6 → 6**；Chrome 进程 8 → 6，
但 **Chrome 主进程（pid 15692，16:37 启动）一直活着**——减少的是标签页的渲染进程，**本轮没有误伤用户的浏览器**。

### A2 · 语音 `chat-audio` 协议：用户真 key、经产品路由、配对实测（2026-09-18，提交 `84f1ff4`）

**为什么要做**：A1 全程假桩，这是拍板 9「补语音协议」那条功能**唯一缺的实证**。
**配对**：每个候选两臂，同一段音频（SAPI 合成「今天下午三点提醒我检查A股收盘计划」，16 kHz 单声道，155,726 B）、同一个端点、同一把 key，**只切 `provider.asrProtocol`**。
**经产品路由**（`POST /api/audio/transcribe`），所以地址拼法、鉴权头、data URI、回体解析、usage 映射、记账、错误信封都在测量范围内——直连上游只能证明协议对，经路由才能证明产品对。

| 候选 | transcriptions（旧） | **chat-audio（新）** | 上游原话（新协议失败时） |
|---|---|---|---|
| **MiMo** `mimo-v2.5-asr` | 502（上游 404） | **200，字准确率 100%**，828 ms，记账 1 行 | —— |
| **百炼** `qwen3-asr-flash-2026-02-10` | 502（上游 404） | **200，字准确率 100%**，786 ms，记账 1 行 | —— |
| 百炼 `fun-asr-flash-2026-06-15` | 502（上游 404） | **502** | **上游 400、空体**——端点在（不是 404），但这个模型不接这种请求形状，多半要走百炼自己的专用转写接口。**不是如意的问题** |
| 混元 `hy-asr-3.0-preview` | 502（上游 404） | **502** | **上游 402 `permission_error`「无免费试用额度、未开后付费」**——**请求已走到计费层**，协议很可能是对的，是用户账号没给语音识别开通计费。**开通后能否转写未验证** |

两次运行 MiMo 与 qwen3 读数一致（都是 200／100%）。成功臂各记账 1 行，失败臂 0 行。

**隔离（这一条差点出事，记下来）**：主会话写的第一版探针只把临时工作台的**数据目录**换成了 TMP，**没换系统家目录**——
而如意启动时会往 Kimi／Claude Code 同步 MCP 列表，那会把一份**空列表写进用户真实的 Kimi 配置**。
跑之前自查发现，照 `dev-harness/lib/self-isolate-home.js` 把第一行改成自隔离（`USERPROFILE`／`HOME`／`APPDATA`／`LOCALAPPDATA` 一并换掉，真家目录取自 `fixture-home.REAL_HOME` 只读）。
**跑前跑后给三份真实配置拍指纹**：Kimi `mcp.json` 与如意 `config.json` **逐字节未动**；`.claude.json` 是当前会话自己在写的活文件，只比 MCP 段——**未变**。读数文件扫过无密钥。

**对 2.8.0 的含义**：语音**对 MiMo 与百炼 qwen3-asr-flash 开箱可用**（设置里把服务商的「转写协议」选成 chat-audio）；
fun-asr-flash 与混元**本版不承诺**。花费：成功臂 4 次真转写（两轮 × 两家），分币级。

### F8 · 两件「跨轮重现」的偶发件：一件是测试框架、一件是产品缺陷（2026-09-18，取证不改代码）

**件一 `steward-settings` B3 —— 根因在测试框架，不是产品，也不是等待窗口**（主会话已逐行核过）：
`dev-harness/run-all.js:288` 在任何一件浏览器测试跑完后调 `stopRuyiTestBrowsers()` **不传参数**；
`dev-harness/lib/browser-cleanup.js:8` 不传参数时匹配**整台机器上所有** `--user-data-dir=…\ruyi-`／`wcw-` 的 msedge／chrome 并 `Stop-Process`。
⇒ `--parallel 4` 下**一条车道收尸，把另外三条车道正在用的测试浏览器也杀了**；被杀的那件 CDP 调用往关掉的 socket 里发、永不返回，挂到 120 s 超时。
**这个函数本来就支持按目录限定**（传 `profileDir` 就只杀含它的），只是调用处没传。用户自己的 Chrome 不是这种 profile，不受影响。

证据：
- **历史纠正**：`steward-settings` 三次偶发总耗时 136／137／133 s，都等于「120 s 超时 ＋ 一次 13–17 s 的正常重跑」，**不是**主会话在 Q1b 节里推的「负载下每跑 66 s、慢 5 倍」——那句推断错了。Q1b 首跑的 FAIL 只有 B3（run-all 记全部首跑 FAIL 行）⇒ **B4 通过、开关已落盘**，排除「丢了一步」。
- **合成形状对照**（每形状 2 次）：「start 发出前杀掉 Edge」→ 只有 B3 红、盘上 true、挂到 120 s ——**与 Q1b 逐项吻合**；「start 请求失败」→ B3／B4／B5 都红、不挂；「start 延迟 20 s（慢假设）」→ 不挂。
- **两臂**：原版 run-all 12 轮，第 12 轮**自然复现 Q1b 原形**（Edge 在勾选后 215 ms 被杀，退出码 −1＝`Stop-Process` 特征）；同时 `steward-drawer`、`steward-conversation` 也挂到超时。
  **只关掉 `:288`／`:326` 的对照臂 30 轮：0 失败、0 次浏览器被杀**，负载反而更重。
- B3 的 16 s 窗口没问题：负载下从勾选到 running 只要 **152–797 ms**。
- ⇒ **今天大多数「没有 FAIL 行、只是超时」的偶发，很可能都是这一个原因**（`steward-conversation` 在 T1／F7b 两轮就是这个形状，但未逐次核实）。
  也就是说 43 号文 107 退出门里的「并行回归偶发治理已收」**此前并不成立**——最大的偶发源一直在框架自己身上。

**件二 `mission-threads` —— 产品缺陷**（主会话已逐行核过）：
`02-session-store.js` 的 `flushSessionIndex`（约 `:961-963`）**先把待写批次拿走并清空、再去写盘**；`listSessions`（约 `:1006-1014`）只叠加**仍在内存里**的待写批次。
⇒ 写盘途中那几毫秒，刚写的那批**既不在内存、也还没到盘上**，读到旧值。`listSessions` 那段注释写着「保证读到的永远不比最近一次存盘更旧」——**正是被这个 take-and-clear 破坏的**。
经它读的两处中招：split 的幂等判断（约 `:4402-4406`）→ **重复建事项**；事项列表聚合（`13d-core-domain-routes.js` 约 `:641`）→ 刚归类的线程短暂显示成「未归类」。
证据：**间隔阶梯**（拆一次 → 精确等 d ms → 再拆，d 取 100–260 ms 每 2 ms 一档）162 次里重复建事项 14 次，**全部落在 d=182–202 ms**，其外 0/148；
**读己之写探针**空闲 20 次里 19 次在 attach 后 192–197 ms（正是 200 ms 去抖）跳回旧值、持续 1–15 次读取；
**自然复现** 65 轮里 Q1 原形 3 次（二值：事项文件数第一次读就是 4、3 秒内不变，不是「读太早」的分布）。测试自己的直读点没问题（65 轮 `msToSettle=0`）。
**用户可见后果**：约 200 ms 内重复点「拆分」（或重试）会建出重复的空事项；看板与事项列表会短暂把线程显示成未归类。

**处置**：两件都在 F9 修——F9a 修产品（在途批次对读者可见直到落盘），F9b 修框架（每件测试的浏览器 profile 落在自己的目录、收尸只传这个目录）。
F9 **不许写任何自造负载 runner 或收尸器**，两件都用确定性测试＋一轮标准全量验证。

**同一刀的事故（第二次误杀用户程序）**：F8 的负载 runner 收尸时杀了**用户的 Ollama**（`ollama app.exe`、`ollama.exe serve` 与拉起它们的 `cmd.exe`，约 19:43）。
这一次派单里已经写明「只杀自己 spawn 的 PID、严禁按名字杀」，它也照做了——**但它认「自己的子孙」只看父 PID**：拉起 Ollama 的 `cmd.exe` 的父进程早已退出，
那个号被它的一个测试 Edge 复用了，于是整棵 Ollama 树被当成自己的。**Windows 的 ParentProcessId 在父进程死后不更新、进程号复用很勤**。
主会话核实：Ollama 确实不在跑；本机 **explorer.exe、WPS、Steam、Qoder 的父进程都已不在**——任何「按父 PID 找子孙再杀」（包括测试框架里各处复制的 `taskkill /T`）理论上都可能在号被复用时把它们带走（`/T` 是否核对创建时间**未验证**）。
没有替用户重启 Ollama（理由同工作台）。F8 自称修后改为「子进程创建时间 ≥ 父进程且晚于 runner 启动」核验，其后收尾零误杀；收工快照 chrome 6→6（主进程 15692 在）、webview2 6→6、node 1→1。
**规矩再加码**（已入记忆）：认子孙必须核创建时间；**子代理不许自造收尸器**。测试框架 `taskkill /T` 的同类隐患记入偿债波。

### F9 · 两处修复（2026-09-18，提交 `6f36b31` F9a、`a9813b7` F9b）

**F9a 产品：会话索引在途批次对读者可见直到落盘**（`02-session-store.js`）。
新增 `inflightSessionIndex`：刷写拿走待写批次的**同一拍**就放进在途表；读侧统一经 `overlayUnflushedSessionIndex` 按「盘上 → 在途 → 待写」叠加（待写最新、最后覆盖）；
写盘结束（落盘／无索引丢弃／写失败作废，三种都算）按**每发一个身份令牌**只撤自己那一发的条目；新增撤表计数，`listSessions` 读到一半被撤表就重读（最多 4 次，再不行走全量扫盘）；
同步退出刷写（PF2 那条）把在途那批也写进盘。那句「never staler」的注释改成了真话。
**确定性单测** `unit/session-index-inflight.test.js` 6 条（A 承重读己之写、B 重复拆分的端到端原形、C 同步刷写带在途、D 写失败也撤表、E 只撤自己那一发、F 读到一半被撤表要重读），每条都先断言「此刻盘上确实还是旧值」证明窗口真的摆出来了；反向四组各只红该红的。

**F9b 测试框架：收尸只收本件的根**。**审计**：全仓 29 件带 `--user-data-dir` 的测试，profile **全部经 `os.tmpdir()` 建**，无一例外。
**它没照主会话派单里「给每件设独立 TEMP／TMP」的建议做——那是个洞**：产品把桌面 MCP 的 python 探针结果跨进程缓存在 `os.tmpdir()` 下（`01-config.js:2290`，一轮约 5 s），
夹具起的服务会继承 TEMP；每件一个新 TEMP 就是每件一次冷探针——正是 123 合并时「8 路全量 37 红」那个模具。
改成**只改夹具进程自己的 `os.tmpdir()`**（`dev-harness/lib/browser-profile-scope.js` 经 `--require` 注入，环境变量原样下传），`run-all.js:288` 改为只带「本件根＋分隔符」这根针收尸、空范围一次都不收；`:326` 开跑前的全局收尸保留；
另加一把锁：夹具里任何 spawn 带 `--user-data-dir` 却指向根外，当场 FAIL。收工多一行「残留测试浏览器」，只数不杀。
**确定性单测** `unit/browser-profile-scope.test.js` 6 条，不开浏览器不杀进程；反向四组各只红该红的。

**F9 全量**（21:01–21:24，`--parallel 4`）：`354 pass / 2 fail / 1 flaky`，run-all 退出码 1。**F9b 的效果**：
「没有 FAIL 行、只是超时」那一类 **0 件**（上一轮 Q1b 是 3 件偶发）；并行车道里 8 件浏览器测试**全部首跑即过**，`steward-settings` **13.8 s**（Q1b 里它是 133 s 偶发）；**收工残留测试浏览器 0**。只有一轮，不够下统计结论。
唯一的偶发 `subagent.e2e.js` 是断言级的（有 FAIL 行），不是超时。
**两件 fail**：
1. `fixture-home.static`「件数锁定 149 == 常量 148」——**主会话的错**：提交 A2 时新增 `asr-chat-audio-probe-live.js` 多了一处带 `RUYI_HOME` 的 spawn，只跑了文档锁没跑全量。主会话已补（`02a2c30`，148→149 并注明来路），该件 ALL PASS。
2. `walkthrough-round1.browser` F1–F8「切到第二个模型后没发出 PATCH」，首跑重跑都红。它说是「已登记的抖动件」——**主会话不照单全收**：翻了 40／44／45 号文，它**跨了五个波次反复出现、从没找到根因**
   （126 波在 HEAD 上单跑 3/3 红过，「单跑红、全量里反而绿」机制未知）。症状若是真的，用户后果是「换了模型没保存」。**按规矩派 F10 取证**。

**主会话亲验**：HEAD 两个提交在、树干净；生成器链 53/421、137/125 不变、`build --check` 新鲜；**两件新单测自跑各 6/6**。
**亲手做 F9a 承重断言的反向**：让刷写拿走的那批不进在途表（＝恢复原缺陷）、重建、单跑 → **A 红**，原文「写盘途中读到的必须是刚写的新名（实得「F9a-A-旧名」—— 刷写拿走了待写批次、又还没落盘，那一批对读者不可见，读己之写被破坏）」，**B（重复拆分的原形）也红**。
这次**源码、产物、行号清单三份一起备份一起还原**（F7b 那次只还原两份吃过亏），还原后 `git status` 为空（逐字节）、`build --check` 新鲜、6/6。

**进程快照**（F9 自报，开工 20:37 / 收工 21:29）：chrome 6→6、webview2 6→6、msedge 0→0、node 1→1、ollama 0→0、`server.js mcp` PID 19936 始终在、测试浏览器 0→0。全程没有按名字或按父 PID 杀过进程。

**已知剩余（照实）**：扫盘重建路径有一处既有竞态（重建写入的快照可能晚于一发已撤表的刷写，id 集不变时旧值会被快路径一直信下去，直到该会话再存一次；与 F9a 无关）；
`00-boot.js:481` 的 `buildUsageSummary` 直读盘上索引取标题，不叠加待写与在途（用量页标题可能旧约 200 ms）；
5 件夹具收尾时仍不带参数收尸（a11y-walkthrough、focus-rail、rail-pocket、walkthrough-round1、walkthrough-round2——都在独占桶，单轮 run-all 下无害，两轮同时跑才会互杀）。

### F10 · `walkthrough-round1` 跨五个波次反复红：根因在测试（2026-09-18，提交 `223ad07`）

**机制**：模型菜单**首次打开**时拉 `GET /api/usage/summary?range=all`，**134–184 ms** 后到货、整张列表重画——顶上插进「常用」标题与用过的 `fake-model`，`fake-model-2` 从 y=278 被顶到 y=321。
指针已经停在菜单里时产品**不**重画（`steward-chips.js:889-906` 的 `:hover` 守卫，124 波有意为之）。
测试旧写法「点完 chip 睡 120 ms → 量 `fake-model-2` 位置 → 指针从 chip 瞬移过去按坐标点」：量位置（139–168 ms）**正落在用量到货分布中间**；
重画若夹在「量完」与「指针到位」之间那 3–16 ms，点中的就是「Fake」分组标题 `p.steward-chip-group`（不是按钮）→ **一发 PATCH 都没有** → F1–F8 连坐红。

**为什么判为测试问题、不是「换模型没保存」**：
- **保存链没断**：点中按钮的 10 次**全部 PATCH 200**，盘上 `engineRoute` 在 0 ms 那一读就已是新值。
- **红的那次没有点击被吞**：配置已到达（两个模型都在）、控制台零错误，只是点在了标题上。
- **盘上双读是二值的**：0 ms 与 4.1 s 两读都是旧值 ⇒ 真没发请求，不是写得慢。
- **强制形状对照**（CDP Fetch 扣住用量回包，在「量完」与「点下」之间放行，两臂只差「先把指针挪进菜单」这一步）：**旧写法 3/3 红**（每次点中 `p.steward-chip-group`、`redrawn:true`），**新写法 3/3 绿**（`:hover` 在、不重画、点中 `fake-model-2`）。
- 真人的指针是从 chip 一路划进菜单再瞄的，一进菜单列表就冻结；全仓别的测试都用 DOM `.click()`，**只有本件按坐标点**。
- 历史上把它记成「并行争抢」对 F 组**不成立**：本件在 run-all 里本来就在独占桶单独跑。

**各轮读数**（全部单跑直跑）：未改的 HEAD 6 次红 2 次（签名都是 F1–F8「实测 []」）；带探针的旧写法 11 次红 1 次（重画夹在中间：量 157／到货 161／重画 162／指针 171 ms）；修后自然跑 8 次 0 红。

**修法（只改测试，+27 行）**：先把指针落到「跟随全局」那行（永远排第一、重画前后位置不变、悬停不触发动作），新断言 **F0c** 确认菜单已进 `:hover`，之后才量、才点——正是真人的动作路径。读数与对照写进了注释。
注释里写了路由字面量，`route-inventory` 的 `coveredBy` 多了本件（F10 第一轮全量红在 `route-inventory.static`，已重新生成，diff 只有这一条 `coveredBy` 与时间戳）。

**F10 全量两轮**：第一轮 `355/1/1`（那件红就是上面的 coveredBy）；**终版树第二轮 `356 pass / 0 fail / 0 flaky / 356 ran`，run-all 退出码 0，收工残留测试浏览器 0**。

**主会话亲验**：run-all 进程已退出后读 `last-run.log`（22:37:59）＝ `356/0/0/0 flaky`；改动只有 3 个文件（测试 +27、路由清单两份），零 NUL，`route-inventory --check` OK；
**读过测试改动全文——不是靠多等一会儿把问题盖住**，而是改成真人的动作路径并加断言钉住守卫；**主会话自跑该件三次：47/47、47/47、47/47，退出码全 0**。
进程快照（F10 自报，21:33 / 22:38）：Chrome 6 个 PID 完全相同、webview2 6、msedge 0、node 1（MCP 19936）、ollama 0；没写任何收尸脚本。

**没解释的两处（照实）**：E3 在 31 次单跑里红过 1 次（猜测同族「固定睡 120 ms 就去读」，未验证）；126 波那次 **C2** 红在这 31 次里一次也没复现，仍无解释。

**F10 递给主会话判断的界面问题（主会话定性：真实的小 UX 债）**：指针还没进菜单时，列表会在首开约 140 ms 后**整体下挪约 43 px**（每个页面生命周期一次）。
鼠标用户会先划进菜单所以无感；**但触屏／笔、键盘用户没有悬停**——列表在手指下面挪位，存在点错行（甚至点成另一个模型）的可能。记入 Brief 未完成项，挂偿债波。

### Q1c · 冻结树最后一轮（主会话亲跑，HEAD `223ad07`）＋ `one-workbench-frame` K6 跨轮重现：根因在测试（2026-09-18）

**全量**：22:40:59 → 23:03:48，**`356 pass / 0 fail / 1 flaky / 356 ran / 7 skipped`，run-all 退出码 0**，收工残留测试浏览器 0。控制台全程存档；进程快照跑前（22:40:57）与跑后一致：Chrome 主进程 15692、webview2 6、node 1（MCP 19936）、ollama 0、测试 Edge 0、8765 无监听。

**那 1 件 flaky**：`one-workbench-frame.browser` K6 首跑红「管家侧的焦点仍然是 A（实测焦点卡标题「读取中…」…）」。**与 44 号文 126 波收口全量那次签名一字不差** ⇒ 按「跨轮重现就是缺陷」的规矩，不靠重跑通过放行，当场取证。

- **机制（读码）**：离开管家视角时，抽屉的 `data-shell-mode` 观察者 `closeDrawer()`（`steward-drawer.js:1646`），`sessionId` 清空；切回时 `steward-board` 的 `syncNow` 发现 `currentSessionId()` 为空、不等于钉住的 A → `openThread(A)`（`steward-board.js:1366`）；`openThread` 先把会话与显示名清空、`loading = true`、重画 → 标题「读取中…」，等会话详情读回来才落闸（`steward-drawer.js:1535-1553`）。测试的 `setLens` 只等 `data-shell-mode` 与 View Transition 收尾就拍快照，负载下取数慢一拍即撞上。
- **强制形状复现**：探针副本在最后一次切回管家前把 A 的会话详情请求拖慢 1.5 s（计数器核过只拖了 1 发）→ **原测试两跑两红**，签名与全量那次一字不差，其余断言全绿。
- **修法（只改测试，+14 −1）**：切回后先等标题离开「读取中…」（有判据的等），再判；读数「读取闸 N ms 后落下」写进 K6 标签。**断言本身一字未放宽**。
- **验证**：
  - 修后＋拖慢 1.5 s → 绿，K6 读数「**读取闸 1220 ms 后落下**」——真等过那一发慢取数，不是侥幸（run-all 不打印通过件，用一条故意的哨兵红把尾部现场带出来读的）；
  - **反向 R1**：修后＋切回管家时派一发 `steward:focus-thread` 把焦点改写成 C（121 波 K5 那类回归的形状）→ K6 两跑两红「实测焦点卡标题「选个框架」」（＝C），K7 连带红 ⇒ **等闸不遮住焦点错**；
  - 原件串行 4/4 绿；`--fast` 73/0；`route-inventory --check` OK；控制字符／CR 零；探针副本已删；进程快照前后一致。
  - 注释初稿写了路由字面量，`route-inventory` 又多挂一条 `coveredBy`（同 F10）——注释里提一嘴不是覆盖，这次改写措辞、清单回到 HEAD 原样，而不是重新生成把假归属写进去。
- **性质**：测试问题。产品按设计（117k 读取闸：读到之前不拿内部 id 冒充名字）。
...
### RC · 发布候选提交上的最后一轮全量（2026-09-19，主会话亲跑，HEAD `7ed7f91`）

23:55:24 → 00:17:33，**`356 pass / 0 fail / 0 known-fail / 0 unexpected-pass / 0 flaky / 356 ran / 7 skipped`，run-all 退出码 0**；unit 快通道 ALL PASS；收工残留测试浏览器 0。
...
### HF2 · 2.8.0 热修线第二刀：首启误报「连不上」＋ 右侧线程栏页签被拽回（2026-09-19 下午，分支 `release/2.8.0`，主会话亲做；用户选「进 2.8.0」）

**起因**：用户在另一台电脑上首启走查：「首次启动会显示连接错误、连接不上，但实际是连上了，新开个线程就好」；追问后补一句「查看日志诊断似乎显示不出具体原因」。
master 上按偿债波 128f-① 取证、修掉六处（48 号文 §4 逐条：探针同步阻塞把在线判离线并缓存 60 s、桌面壳 `MaybeNavigate` 次序、启动故障卡在出厂默认的管家视角里
看不见／乱说连不上／一抛就中止推送、诊断只有一行、单花括号占位符不插值、顶层异常不进日志），外加本刀自己带出来当场堵上的一条（配置没读到时不许把猜的语言写回）。

**做法**：master `fb3fadf` 移植为 `a26328d`。**产品文件的差异与 master 那一提交逐字节相同**（16 个文件逐个比对 `+/-` 行的 sha256）。发布线专有的差异：
新件 `boot-failure-kind.browser` 用 128d 的公共夹具，本线没有 → 逐字带上 `lib/browser-fixture.js` 与它依赖的 128c `lib/kill-own-tree.js`（只被这一件用）；
run-all 按 require 认夹具；fixture-home 150 → 151；经典样式载荷锁按规程在本线重钉（HEAD 重算 = `f762f373…` 与旧值逐字相同 → `5fcee577…`）；
`dom-contract` 登记动态建的 `#bootFailureHost`（master 全量查出，两线同改）。

**第二处（`e0c4d56`，128f-②）是这条线的全量追出来的**：`a26328d` 全量 `358 pass / 0 fail / 1 flaky`，那一件 `steward-drawer` E1…E3g 与 128d 那轮 master 全量的首跑红**签名逐字相同**
（两条线各一次、本线不含 128d）⇒ 按「跨轮重现就是缺陷」取证、**没有按偶发放行**。件里装探针后负载 6 路×3 轮 18/18 复现：点中 B 的页签 → 约 175 ms 后被切回 A、无任何聚焦事件。
根因是产品：宽屏常驻右栏下看板每一拍（轮询／行推送／窗口切回来）的 `syncNow` 见抽屉开的不是焦点线程就把它开回焦点那条，而页签点击只换了抽屉、没告诉看板 ——
**真人会碰到**：点 B、切到别的窗口再回来，回到了 A（多半自 2.7.0 常驻右栏起就有）。修：页签点击与 ←／→ 改走全仓唯一的聚焦通道 `steward:focus-thread`。
新断言 E1d（逼看板刷一拍后仍在 B）修前无负载 2/2 红、修后 2/2 绿；负载 6×3 修后 18/18 绿（修前 0/18）。master `defef06` 同改。**这一处是我按门的纪律自行纳入 2.8.0 的，单独一个提交，用户若不要可以只退它。**

**判据与读数**：
- 快通道 `a26328d`／`e0c4d56` 都是 `73/0`。**全量**（`e0c4d56`，主会话亲跑，14:02:43 → 14:24:48）：**`358 pass / 0 fail / 0 known-fail / 0 unexpected-pass / 0 flaky / 358 ran / 7 skipped`，退出码 0**；
  收工残留测试浏览器 0；进程快照前后一致（chrome 6／msedgewebview2 6／node 1／ollama 0／explorer 1／msedge 0，8765 未监听，跑前跑后与打包冒烟之后都一样）。
- **包**（出自 `e0c4d56`，工作树干净才开打、打完仍干净；同一套 `package-offline.ps1`，带新编的 `Ruyi.exe` 与 `RuyiDesktop.exe`）：
  `Ruyi-v2.8.0-slim.zip` 73.35 MB（`6e6dc896e5cc…`）、`Ruyi-v2.8.0-full.zip` 770.9 MB（`fca62557838020…`），`Ruyi-v2.8.0-SHA256SUMS.txt` 同写。
  上一版（出自 `53e1402`：full `501c63ac0ddd…`／slim `162e23c92582…`）**被本版取代**，改名留在 `dist/`（`*-hf53e1402.*`），与更早的 `*-rc83e9f7a.*` 一起等用户确认后删。
- **全新目录冒烟**（HF 那一套之上加首启修复的包内核验；合成数据、隔离家目录、随机端口、只停自己 spawn 的进程）：slim 自带 node／slim `Ruyi.exe`／full 自带 node
  三种都起来报 2.8.0、schema 12；三种都服务得出 `/js/boot-failure.js`、`app.js` 接的是新卡；三种都「写坏的请求 → 500、看日志里有 `http_unhandled`、请求体原文不进日志」；
  两个包里的 `server.js` 都带探针未知态与顶层异常记日志；两个包里的 `RuyiDesktop.exe` 都晚于修复提交编出；MCP 资源面在 node 与 exe 两个入口下都 0 泄漏。**38 条全过。**
- **一个读数照实记**：Full 冷启动**第一次 `/api/status` 8.9 s**（HF 那次 5.4 s；slim 0.28 s）—— 冷缓存下的本机 Python 探测（128f 已登记的债）。本版修掉的是它引出的
  「在线判离线」与「卡片看不见／说错话」；**它本身还慢**：Full 首启界面要等这么久才填满。不在本版修（要动启动期探测的次序，风险面比这两刀大），进 3.0。

**没做／照实说**：没重跑发布干跑（`release-dryrun.js`）—— 与 HF 同一理由：版本三角与载荷清单不受影响（载荷表只多了 `js/boot-failure.js` 一行，已由 `overlay-payload-lock` 在全量里核过）。
桌面壳的 `MaybeNavigate` 修复只能读码＋编译＋锁，没法在本机实演（WebView2 目录写死在真实 `%LOCALAPPDATA%\Ruyi`）。

**打标签的位置又变了**：`v2.8.0` 应打在 `release/2.8.0` 上**本节所在的提交**（HF2 的记录提交），**不是** `459313b`（HF 的记录）、也不是 `7ed7f91`／`83e9f7a`。
