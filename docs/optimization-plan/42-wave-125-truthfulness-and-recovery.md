...
## 5-undecies. 干净机器基线（2026-09-16，用户重启后立刻取）——**推翻了「系统性退化」那个说法**

126 波收口时我记下一条判断：本会话 flaky 件数 **1→3→5→7** 单调上升、每轮换名字，判定为「连跑十来个小时浏览器 e2e 之后的**系统性退化**」，并写明下一步是换干净机器状态重取基线（44 号文 §6-bis）。**基线取到了，它不支持那个说法。**

**读数**：同一棵树 `dc32e4d`、同样 `--parallel 4`、机器刚重启、空闲内存 16.9 GB —— **349 pass / 0 fail / 0 known-fail / 5 flaky / 349 ran / 7 skipped，退出码 0**。

- **真回归 0** —— 树是健康的，126 波七刀没留下任何退化。
- **但 flaky 仍有 5 件**，退化理论预测的是接近 0。**修正后的说法**：在这台机器、这个并行档位上，**5 件上下的 flaky 就是这套件的常态**；名字每轮轮换，因为它们抖的是**争用**，不是各自的毛病。早先那两轮 349/0/0 是运气好的样本，**不是基准**。
- **不变的那半条**：逐件治仍然是追噪声。**变的那半条**：别再指望「换台干净机器」能把 flaky 清零 —— 要清就得动并行档位或夹具的时间窗，那是产品外的工程决定，留给 107。

**顺带记一次我自己的读法错误**：跑到一半我 grep `flaky` 数出 5 个名字，与结尾权威名单**一个都不重合** —— 又是日志交错那个坑（`[B1] X … [B0] PASS [flaky]` 里的标记属于 **B0 刚跑完那件**，不属于正在announce的 X）。**只有结尾那段汇总是权威的**，这条纪律第二次被自己验证。

**五件的病历（诊断继续兑现，五件里四件自己说出了红在哪一条）**：

| 件 | 首跑病历 | 读法 |
|---|---|---|
| `agent-deadlock-watchdog.e2e.js` | `FAIL a node recorded a resource wait（会合点到齐:一发都没到）` ＋ `FAIL ...cycle detection（同）` | **这是我 125 波自己装的会合点在如实报告它回落了。** 那两条断言**只有真撞上才成立**，会合点回落时它们必然红 —— 缺的是「回落时这两条应当报不适用，而不是报红」。**这是我上一波留下的判据缺口，不是产品问题**，107 时按这个口径修 |
| `steward-settings.e2e.js` | `FAIL A5 browser target available` | Edge 压根没挂上。与 `dom-screenshot` 老病历同族：**机器级启动抢占**。它不在独占桶里 |
| `perm-v2.e2e.js` | `FAIL ④ turn did NOT wait out the ~6s permission timeout (elapsed 6152ms)` | 固定 6 s 墙钟窗口，实得 6152 ms —— **窗口太紧**。现有那把机械锁只钉「自己算 percentile 的」，**钉不住「写死一个秒数窗口的」** |
| `prefix-layout.e2e.js` | `FAIL E5 开关开:第二回合分类激活 files_read,tools 变多(0 → 0)` ＋ `FAIL E7 tool_schema_freeze init+append 事件落账` | `0 → 0` 是**两回合的工具表都空**，不是「变多了但不够多」。像是第二回合压根没发生（服务/provider 没跟上），**但也可能是真产品条件** —— 下次再红先分清，**不猜着改** |
| `steward-drawer.e2e.js` | 没抓到 FAIL 行 | 超时或被杀，不是断言红。**没病历，不动** |

**给 107 的一句话**：这五件里 **`perm-v2` 指向一条可机械化的锁**（写死秒数窗口的夹具也该进独占桶，与 percentile 那条同模具），**`agent-deadlock-watchdog` 是我自己的判据缺口**，其余三件要么是机器级抢占、要么没病历。**本波不动它们**——127 是产品波，flaky 属 107 前置栏。
