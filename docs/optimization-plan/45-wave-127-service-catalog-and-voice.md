...
#### 107 的干净基线（`eb7334f`，主会话跑）

开跑前 CPU 17%、桌面资源管理器窗口 3 个（均非测试所开）。`run-all.js --parallel 4` **退出码 0，356 pass / 0 fail / 2 flaky / 356 ran（7 skipped 为既有 live probe）**，unit 全绿、build 新鲜、端口零撞车。跑完桌面窗口仍是 **3 个**（artifacts (f)④ 收尸在全量里生效，本轮零泄漏）。本批治的三件（playbooks ①、foreign-turn-busy-guard A11/C4、thread-arbiter ①）**均未上榜**。

| flaky | 首跑实得 | 归类 |
|---|---|---|
| `mission-index-scale.e2e.js` | (c) 列表热 P95 ≤ 300 ms，实 546 ms | 独占桶里的墙钟性能门（125 波起登记过同族：9-14 冷门 1921 ms），回归内重跑过 |
