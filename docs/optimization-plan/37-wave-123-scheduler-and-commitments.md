...
- **出入（四处，均采纳）**：① `origin:'schedule'`（02:2819 早为本波预留的第三值，用 steward 会让 UI 把定时线程说成管家开的）；② 不写 `aux/scheduled` 用量行（回合已记 `kind:'turn'`，再记等于同一笔钱两遍；零 token 行会被丢）→ `costTokens` 现场累计进 fires 行，台账打标登记后续；③ 不复用 13k `stewardRecordLaunchOutcome`（会把 13k/13j/13i 拽进 SCC，环边 29→7），就地 `updateSessionMeta` 同口径；④ `target` 不收 `cwd`（禁止键，显式 400）／`engineRoute`（127 波）。
  > **【2026-09-16 用户真机走查】这个「留给 127 波」当场被用户撞上了，而且它同时是另一个症状的根因。** 用户先报「14:00 的定时任务没触发」——实为**触发了但等锁**；再要求「指定用 DeepSeek 跑定时任务」——做不到。查下来两件事同源：`steward_schedule_create` **有权限档、没有模型档**，触发时 `13s:462` 直接 `createSession` 不经过 `stewardApplyThreadTier`（`13q:575`），所以既拿不到 `engineRoute`，**工作目录也一律回落 `defaultWorkspace`** —— 于是**所有定时任务天然互抢同一把 `cwd-write` 锁**（`13n:93`），还要跟任何一条也在默认文件夹的手工线程抢。用户那句「为啥不能同时开多条 Kimi CLI 线程并行」的答案因此是：**跟 Kimi 无关**（`05b` 里没有任何并发限制），是按工作文件夹的写互斥，而且 `13n:182` 那一刀**一律按「写」处理**（只读回合的放宽自己写着「留后续波」）。治法、判据与反向见 [45 号文 §2-ter](45-wave-127-service-catalog-and-voice.md)：S-a 补 `tier`、S-b 照搬 `steward_thread_new` 的「自动开一个自己的文件夹」（**仍不放开 `cwd` 入参**，那道禁止键是安全线）、S-c 只读回合放宽**记债不进本波**。
