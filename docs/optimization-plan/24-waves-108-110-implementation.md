...
- **110-4a（2026-09-03，opus 实现／主会话亲核）**：边界修正为 `09:1-174`（文件无模块头，1-6 是块首注释）；09 3375→3202；`09b-replan-ledger.js` 175 行；逐字节相等（11785 字节）且程序化重建的 `server.js` 与产物逐字节相等（代码零位移）；自由标识符与 09 其余顶层符号交集为空；图 34/255/65 → 35/259/65，provides 总集合不变；policy 环边 +3（`09b→00-boot` consumer 侧、`13d→09b` provider 侧、`09→09b` 原文件内调用随 provider 搬家——三者均非新依赖，裁决通过）；静态锁零改动；门：`--fast` 48/48、10 件定向全绿、全量 257/1/2（`mcp-ops-closure` 与两件 flaky 串行通过，`mcp-ops-closure` 补入偶发名单）。提交 `a91448f`。
- **110-4b（2026-09-03，opus 实现／主会话亲核）**：token 估算簇 `09:3075-3202`（128 行含块首归属注释）→ `09d-token-estimation.js`；09 3202→3075；逐字节相等（8526 字节），按 HEAD 片段重建的 `server.js` 与产物逐字节相等；图 35/259/65 → 36/265/67，provides 总集合不变；policy 双向重归属（`07→09` 整条改名 `07→09d`；`08/10/14→09` 保留＋新增 `→09d`；consumer 侧 `09d→10`（`ESTIMATION_RULES`）与 `09d→07`（`estimateToolSchemaTokens`）；`09→09d` 原文件内调用），forward +2 来自拆分登记，无第五类新边；静态锁零改动；门：`--fast` 48/48、8 件定向全绿、全量 257/1/7（`mcp-ops-closure` 与 7 件 flaky 串行通过）。
- **110 波出门裁决（2026-09-03）**：纯搬家共六刀（110-1、2a、2b、3b、4a、4b）＋两件 harness（2-pre、h1），模块 30→36、前向边 65→67（均为拆分登记）、SCC 1；后端最大文件 `09-workflow.js` 3375→3075、`02-session-store.js` 3176→2943、`01-config.js` 2423→2175、`13-http-router.js` 2603→1861。**继续纯搬家已无高性价比目标**：剩余大块要么是巨型函数闭包（110-r1）、要么引用源模块内部 helper（110-3c′）、要么非顶层声明（01 默认值块／sanitize 表）；交办台前端两刀已随管家立项取消；05b 延后。待办登记：**110-r1** 巨型函数行为中性闭包提炼、**110-h2** 扫描器普通形参与嵌套声明绑定、**110-3c′** 02 内部 helper 前置搬家、**110-4c** 两小块可选。出门证据：**串行全量 `run-all.js` = 258 pass／0 fail／2 flaky**（`interventions-pool`／`interventions-snapshot` 首跑失败重跑通过，均在偶发名单口径内）。**第 110 波宣告出门（2026-09-03）**。
