...
| 23 | [架构偿还与上下文演进序列（第 103–107 波）](23-architecture-repayment-sequence.md) | 2026-08-30 revise-major 后纳入：command core 已交付不重做；103 路由／依赖／持久化地基 → 104 上下文结构 → 105 行为证据 → 106 Agent SoC 收敛 → 107 发布批准点。**第 103 波已交付**：路由清册、24 模块依赖契约、39 项持久化 surface 清册及 durable JSON 试点；当前进入 104 |

## 当前实施入口（2026-08-31）

- 当前下一实施入口为 **23 号第 104 波零行为内聚与上下文结构**：第 103 波已交付，持久化清册和依赖契约成为 104 搬迁门；优先按职责拆出视觉／桌面 shell／记忆与压缩簇，保持提示词、规则和 API 快照不变。
