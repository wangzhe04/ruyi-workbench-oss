...
| 23 | [架构偿还与上下文演进序列（第 103–107 波）](23-architecture-repayment-sequence.md) | 2026-08-30 revise-major 后纳入：command core 已交付不重做；103 路由／依赖／持久化地基 → 104 上下文结构 → 105 行为证据 → 106 Agent SoC 收敛 → 107 发布批准点。**103a 治理面已交付**：路由清册生成器+静态门（101 判定点／92 鉴权行零漂移）、决策端点逐端点快照、facts 新鲜度修复（实施记录见文内 Release Brief）；if 链→descriptor 运行时迁移按域分批待后续 |

## 当前实施入口（2026-08-30）

- 当前下一实施入口为 **23 号第 103 波**：103a 治理面已交付（路由清册+静态门、决策端点快照、facts 修复；descriptor 运行时迁移按域分批待后续），下一切片为 **103b** 依赖图／`provides/requires`／隔离试点 → 103c durable data surface。原提案中的 17 模块、旧测试数和“command core 未落码”不再作为施工事实。
