...
| 27 | [第 115–117 波实施方案（工作台管家 Steward）](27-waves-115-117-steward.md) | **115 已交付（2026-09-05）**：原型 `docs/mockups/steward-shell.html` 经三轮走查定稿；拍板：两壳长期并存、线程单一权限四档取代三档自治、管家对话只保留本次、递话「输入即预判」。**116 已交付（2026-09-05 → 09-06，引擎侧，默认关）**：配置、收件箱三源轮询、26 个 `steward_*` 工具经命令核心、记忆完整版、线程级权限与只降不升、回合运行器与到访、事项跨会话升格、线程仲裁、设置三级分级、速查线程；串行全量 280/6/4，前向边 67 不变，逐片记录见 §11.6。**117 已交付（2026-09-06，管家壳，默认仍是经典布局）**：壳模式三态并存、avatar 七态、对话区与递话（输入即预判、10 秒撤回）、线程抽屉（它刚说／你可以说、三处共用快切 chip）、设置页签与记忆面板与行动流水、2.0 视窗返回带、看板与「现在这一件」；顺带修 116g 待决门真 bug 与撤回两处竞态；app.js 1277/1280、前向边 67 不变。**117f dogfood 留用户**，默认壳切换不自动恢复。**2026-09-06 夜停点（§11.7）**：117i 视觉对齐原型、providers 数据事故三次修复、116-3 后端对抗修复两批入库；待派 116-4 收件箱第四源／116-5 线程自动摘要（新需求）／117j 前端走查修复。原计划 117 壳层（壳模式并存、avatar、话＋一行按钮、抽屉「它刚说／你可以说」、2.0 视窗、看板与「现在这一件」、设置、dogfood 门）；交办台停止独立演进 |
