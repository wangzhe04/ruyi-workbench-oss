...
| 27 | [第 115–117 波实施方案（工作台管家 Steward）](27-waves-115-117-steward.md) | **115 已交付（2026-09-05）**：原型 `docs/mockups/steward-shell.html` 经三轮走查定稿；拍板：两壳长期并存、线程单一权限四档取代三档自治、管家对话只保留本次、递话「输入即预判」。下一步 116 引擎侧（默认关：配置、收件箱三源轮询、管家工具集经命令核心、记忆层、线程权限映射与熔断、回合运行器、事项升格、线程仲裁）→ 117 壳层（壳模式并存、avatar、话＋一行按钮、抽屉「它刚说／你可以说」、2.0 视窗、看板与「现在这一件」、设置、dogfood 门）；交办台停止独立演进 |
...
- 当前下一实施入口为 **27 号第 116 波管家引擎侧**（115 原型已于 2026-09-05 定稿）；执行序 116 →（119 可并行）→ 117 → 114 → 111 → 107 批准点。108–110、118、112a–c、113a/b 均已出门。
