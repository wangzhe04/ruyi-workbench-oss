...
> **状态说明（2026-07-24）**：这是 v1.6.7 后、Escapade 2.0 立项前的分析存档；第 46–52 波已纳入 **如意 Ruyi Escapade 2.0 (`v2.0.0`)**。当前执行中的发布规则、内部波次和 Escapade / Pretender 后续计划，以 [`../OPTIMIZATION-ROADMAP.md`](../OPTIMIZATION-ROADMAP.md) 的「发布线、产品代号与内部波次」与「后续计划」章节为准。
...
Agent 研究支线当前执行顺序：`C1（已完成） → C2（已完成：M2/R1 证据衔接） → C3（已完成：模板回填 + M4 回测） → R4（S1/S2 已完成，S3 待推进） → R5（待推进）`。R2/R3 暂缓，不再阻塞 R4/R5；恢复时仍须遵守“只生成候选/替换建议、必须人工审批、绝不自动覆盖模板”的边界。
