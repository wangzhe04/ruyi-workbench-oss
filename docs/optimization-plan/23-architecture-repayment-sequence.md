...
- **非目标**：不改摘要主 prompt／五节结构／reseed 字节形状；不做 4.2 单发优先与 4.3 refine；不做 notes 回注与增量合并；不做无界重试；不改 `validateStructuredSummary` 既有语义；估算因子分桶（4.1 末项）留后续切片。本切片为成本承担行为（触发修补时多一次 LLM 调用），**取证期默认关闭**，不改变任何默认行为。
- **交付物**：
  - 开关 `runtimeSummaryEntityCheckV1`（默认 `false`，sanitize 只认 JSON 布尔）+ 唯一判定 `summaryEntityCheckEnabled(config)`（`01-config.js`）；`runtime-optimization.static.e2e.js` 静态锁同步。
...
- **发布判断**：默认关闭、零用户可见行为变化，不触发版本升级；可随后续 Escapade 补丁或 105 后续切片一并发布。
- **默认开启判据（flip 前置，未做）**：真实历史门＋fake 集成门全绿（本切片已取得）；遥测证明修补每次压缩 ≤1 次额外调用且无修补风暴；可选真实 provider 回放（DeepSeek）确认真实模型能按缺失清单修稿。满足后 flip = 默认值一行＋静态锁一行＋本节补记。
