...
实验使用 session-sticky 分桶，确保同一长会话不在中途切换历史布局。推荐顺序：E2 → E3 → E4 → E5；每项先 shadow/离线，再 5% canary，再 25%，最后才评估默认开启。E3 与 E4 都会改变后续模型输入，shadow 只能验证确定性投影和协议兼容，**不能**估计模型行为变化；主动 A/B 不可省略。
