...
→ **【121 波实施中】~~K0 默认入口翻转（`effd3cd`，34 号文 §13.1）~~ → ~~K0b 投影索引空目录守卫（`826504c`，§13.2）~~ → ~~K1 交办台删除＋壳模式搬家（`df4c9cf`→`3e8e65e`，§13.3）~~ → ~~K2a 事件流后端（`e3acedf`→`c215c06`，§13.4）~~ → ~~K3 索引口径＋在场门＋交接（`f0fd655`→`768040b`，§13.5）~~ → ~~K2b 事件流前端（`3c15b97`→`56997a2`，§13.6）~~ → ~~K4 外框＋左栏＋动效（`69a505b`→`2a69a7b`，§13.7）~~ → **K5（下一刀；换机器从 34 号文 §14 进）** → K5 → K6 → K8 → K7；一台两视（34 号文：K0 壳模式搬家＋默认入口 → K1 交办台删除 → K2a 事件流后端 ∥ K3 索引口径＋在场 → K2b 事件流前端 → K4 外框＋左栏 → K5 工作台线程头 → K6 焦点栏＋安静卡 → K7 口袋）——用户 2026-09-10 六条诉求的整波答复；交办台退役与「2.0 视窗」删除在此波**
