...
| **`13h-steward-runner.js` 2522 行** | 同一把 SPEC 2000 行闸，**比拆之前的 13g 还长**；31 号文七轴的回合层改动（污染打标、代答/代批、通知）全落在它身上 | **E 批第一刀之前**；照抄 §2.1 那份逐行归属表的方法 |
...
~~（合并已完成）~~ → ~~拆 13g~~ → ~~F1–F5 把设计稿落地~~ → **【下一刀】拆 `13h`（2522 行，E 批的地基）** →
