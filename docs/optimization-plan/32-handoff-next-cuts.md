...
| **`01-config` 对 `workspaces` 的 20 行帽子吞掉派生行** | `if (clean.length >= 20) break;` 两处；用户已有 20 个工作区时 W1 派生的那一行下次 normalize 被截掉，线程 cwd 指向表外目录、再用被拒；候选表折叠句生产不可达 | **W1④，合并后第一刀**：帽子抬 64、派生超帽 fail-closed 拒开并明说，锁「要么行落盘要么拒」 |
...
~~（合并已完成）~~ → ~~拆 13g~~ → ~~F1–F5~~ → ~~117v~~ → ~~117x M1/M2~~ → ~~T2 拆 `13h`~~ → ~~117y~~ → ~~W1＋E-手① ①②③（§11.19）~~ → **【下一刀】W1④ 工作区表 20 行帽子（§11.19.8 第一行裁决）** → E-手②（§11.21，默认全档只提议）→ **W2（§11.22，放宽安全边界，不拍不做）** →
