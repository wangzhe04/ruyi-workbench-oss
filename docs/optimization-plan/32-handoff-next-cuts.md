...
**另立一把小刀（用户拍板①的另一半）：「冷启动为什么要 6 秒」。** 闸已抬到 7500（`perf.e2e.js:22`，带实测范围注释），但本机 `/health ready` 实测 5506–6282 ms，5 秒的承诺没有兑现。先量启动各阶段耗时（`ensureDirs` → 读 config → 模块加载 → 监听），找出大头再决定。查清后若回到 5 秒内，把闸改回去。
...
~~（合并已完成）~~ → ~~拆 13g~~ → ~~F1–F5 把设计稿落地~~ → ~~117v~~ → ~~117x M1/M2~~ → ~~T2 拆 `13h`~~ → **【下一刀】117y（管家的话说得完，§11.18）** → W1＋E-手① 合刀（§11.19）→
