...
## 3. 拍板记录（原「挂起三条」；2026-09-09 已全部拍完，无挂起项）
...
| **`STEWARD_ACTION_HOOKS` 没有 `steward_thread_permission`** | E-手② 登记：`desktop:true` 降级成按钮，按下去 `13p:93` 查表不中 → 4xx；标签兜底「去做」。用户可见的死路 | **E-手②b 第一件**（合并后第一刀）：挂钩＋人话标签＋前端 `STEWARD_TOOL_LABEL_KEYS`＋两个 locale 键；裁决见 27 号文 §11.21.7 |
| **PATCH `desktopTools:true` 不要 `confirm:true`** | 与 `permissionMode` 切全自动同一道门两种口径（`13d:371` 前） | **E-手②b 第二件**：同一个 409，清除与 false 不要确认 |
| **`allowDesktopTools` 只滤两个原生工具，桥接 desktop 档不经它** | auto 模式因 `tool_invoke_*` 无 desktop 档而不可达；full 模式（`07:720`）未核 | **E-手②b 第三件是核不是改**：钉一条 full 模式锁，可达就按 `allowDesk` 滤，不可达就钉住不可达 |
| **夹具设 `RUYI_HOME` 不设 `USERPROFILE` 会摸真机主目录** | E-手② 执行者的夹具在真机 `~/Ruyi/` 建过一条测试目录（已清） | **进「先占位再建目录」小刀**：公共起服 helper 把 HOME 一律指向临时目录，反向要红 |
| **派生的「占位＋建目录」不是原子的** | W1④ 登记：63 行时两条线程同时过预检 → 65 行 → 被截；且 `stewardRegisterDerivedWorkspace` 吞掉写失败。两条同一来路：「目录建了、行没了」 | 小刀「先占位再建目录」：占位＋帽检查进 `mutateConfig` 串行段，行落盘才算派生成功 |
...
~~（合并已完成）~~ → ~~拆 13g~~ → ~~F1–F5~~ → ~~117v~~ → ~~117x M1/M2~~ → ~~T2 拆 `13h`~~ → ~~117y~~ → ~~W1＋E-手① ①②③（§11.19）~~ → ~~W1④~~ → ~~E-手②（§11.21，全档只提议）~~ → **【在跑】E-手②b（§11.21.7：挂钩＋按钮标签、PATCH 二次确认、桥接 desktop 档核锁）** → 小刀「先占位再建目录」（§11.19.9 ＋ §11.21.7 ④ 夹具 HOME 守卫）→ E-嘴（§11.23，通道走已通着的 WebView2 托盘气泡；新建最小在场信号；本仓第一条桌面→页面回话）→ **W2（§11.22，放宽安全边界，不拍不做）** →
