...
~~（合并已完成）~~ → ~~拆 13g~~ → ~~F1–F5~~ → ~~117v~~ → ~~117x M1/M2~~ → ~~T2 拆 `13h`~~ → ~~117y~~ → ~~W1＋E-手① ①②③（§11.19）~~ → ~~W1④~~ → **【在跑】E-手②（§11.21，默认全档只提议）** → 小刀「先占位再建目录」（§11.19.9）→ E-嘴（§11.23，通道走已通着的 WebView2 托盘气泡；新建最小在场信号；本仓第一条桌面→页面回话）→ **W2（§11.22，放宽安全边界，不拍不做）** →
