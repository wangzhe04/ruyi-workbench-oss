...
~~29 号文 119（Cron，与 E-时间合并）→ 28 号文 118（新手引导）→ 26 号文 114（ASR）→ 25 号文 111（压缩策略 v2）→ 23 号文 107（发布批准点）。~~→ **本队列 2026-09-15 作废**：119 与 118 早已出门（123 波／118 波），114／111／107 已按用户指示融进产品线 —— 以 [43 号文 §2](43-merge-114-111-107-into-product-line.md) 为准：**126（记忆时效 ∥ 压缩 v2 111a–e）→ 127（服务目录 ＋ 语音回填 114a–c）→ 107 批准点 → 128+**。
...
16-bis. **纪律 7 第五个样本（2026-09-13，123-M1）**：补丁传输层会把源码里的 `\uXXXX` 当转义解释——`06j` 第一版的控制字符字符类真被写成了裸 NUL＋0x1F（`cat` 看不出来），commit message 同样中招（git 直接拒「NUL byte in commit log message」）。修法：源码里改用 `charCodeAt` 逐码位判，全文件零转义序列；提交前的 0x00–0x1f 扫描仍是唯一可靠的闸。
...
→ **【123 波已收（37 号文 §5／§6，2026-09-13，全量 338/5/6 真回归 0）】** ~~M1 后端地基 ∥ M3 清障 → N1 话术与对话流 ∥ N2 新线程默认引擎 → M2 管家与界面接线~~ → ~~123-P1 管家契约兜底（38 号文，用户 2026-09-14 真机 bug：模型只回 say 一个键还编造已完成动作）~~ → ~~桌面 MCP 探针异步预热那一刀（39 号文：同步签名保留＋异步孪生＋预热闸门；同一时刻的 /health 2389 ms → 2 ms）~~ → **124 波 交办与交付贯通（35 号文 §2 ／ 40 号文）**：~~P0 清障三小件（二次确认／稍后推迟多久／订阅推送，`fde1fc1`，40 号文 §6-bis）~~ → ~~P1 四态标签与只读投影（40 号文 §6-ter：机器检查／人工复核／自报完成／未记录四态、无账本线程说「未记录验收」、新件 acceptance-provenance.e2e 43 条＋四处真反向；**证伪了派单稿 §4 的「一个字段都不加」一处**——模型改得动 status，只按 check.type 推「机器检查」会在 J08 那一格说谎，故补了一枚只有机器写得了的 lastCheck 章，写入口一处、调用点两处、输入侧进不来）~~ → ~~P2 委托书与原件（40 号文 §6-quater：线程头下第一条系统消息，目标＝用户原话逐字、管家补充原样、「看原件」跳这条线程第 1 回合；**零 src 改动、零新字段、零新请求**；新件 thread-commission.static 25 条＋thread-commission.browser 34 条＋三处真反向；证伪了派单稿的命名——`threadBrief` 撞 116-5b 的线程自动摘要，界面侧整族改名 `commission`；全量 347/0/3 真回归 0）~~ → ~~124 走查两件（40 号文 §6-quinquies，用户 2026-09-15 真机：① `steward_schedule_*` 漏登记 act 白名单——同一个坑第三次，补五条写类＋机械锁 ①e「凡会回 propose_required 的工具必须在表里」，另加 steward-guardrails (S) 段七条行为面；② 焦点线程第三档「已停工」没有时效尺，旧的停工线程永远赢过刚做完的，用户拍板改成「最近发生的那一件」）~~ → ~~124 走查③（40 号文 §6-quinquies ③，P2 实拍复盘逼出来的：委托书带展开之后与第一条用户消息是同一段话——那条消息本来就是管家递过去的原件。用户三选一选 B：第一条折成一行，点「看原件」才展开；折的是显示不是数据）~~ → ~~P3 回执才算数（40 号文 §6-sexies：三态判据 done／failed／**no_receipt**，回执从 ※ 浮层上到可见层，live 与回放两条路都画；**证伪了派单稿的落点表——零 src 改动**，谎在前端那条两值判据 `okFlag = !(result && result.ok === false)` 上；新 unit 8 例＋W 组四条真浏览器＋AB 组七条静态锁「画回执的那三段一个 say 字样都不许有」；两处真反向；全量 347/0/3 flaky，三件 flaky 逐件串行复跑全绿，真回归 0）~~ → **124 波四刀全收，退出门 J01／J08／J15 过门、J06 如实不过门**。→ ~~还债④ 焦点线程收成一处（40 号文 §6-septies：服务端 13q 不再自己挑「现在这一件」、只投影 threads，判据搬进两边都已 import 的叶子 thread-facts.js（零新模块边），看板按原名 re-export；新 unit 身份锁（看板导出的必须是叶子那个函数对象本身）⊕ steward-runner.static ⑥ 十三条 ⊕ AD 组三条真浏览器（问候语那枚钮修前零行为断言）；三处真反向）~~ → ~~124 走查④ 回合结束后发不出话去（40 号文 §6-octies，用户 2026-09-15 真机：按钮写着「插话」、点下去「插话失败：请求失败。」。根因：发送门读的 relay 快照会过期且无自纠正，CDP setter 探针抓到是「迟到的推送 vs 服务端释放登记」的竞态把它重新写回 steer；三层都修：兜底按码重发、八处拒绝改发稳定码、兜底码不再吞掉原话；新 S31–S37 静态锁 ⊕ H8 段真浏览器十条（H7 只盖了「管家起的回合」那一半）；三处真反向）~~ → ~~还债① 看板也说得出「未记录验收」（40 号文 §6-nonies：列表路由补上 ledger／tracked 两个组级事实；用户拍板「只在收工了却没人记过时印」，两道门缺一不可，否则复活 P1 刻意回避的满栏灰字；判据仍只有 acceptanceRecorded 一处；unit 11 例 ⊕ (g)(g2) 八条真 HTTP ⊕ C4c/d/e 真浏览器 ⊕ P 组九条静态锁；四处真反向，其中一处逆向暴露「锁写松了」当场收紧）~~ → **§8.5 两笔全清，125 可开波**。→ **125 号文已写（[42 号文](42-wave-125-truthfulness-and-recovery.md)，2026-09-15）**：取证四块——自理六道闸没一道问「用户刚按过停」（而 `failed` 的默认处置就是重开，`retry_node` 在 HTTP 面恰恰「run 不活着才允许」）／失败原因三张表两套词汇管家一张不用（模型看见的是裸 `idle_timeout`）／`web_fetch` 断网回落缓存回 `ok:true` ＋ 旧正文而 `fromCache`／`ts` 无人读／自动升档压根没实现（退出门靠巧合满足）。刀序 **P0 喊停就是喊停 → P1 失败说得出原因 → P2 拿旧的当新的 → P3（J09）先取证**；字段一个都不加，靠三把静态锁（判据唯一／表唯一／值域唯一）撑住。三件要拍板的在 42 号文 §6（停止判据要不要时间窗、时效徽标印在哪、J09／T03 的真模型样本本波做到哪），不回按推荐走。 → **用户 2026-09-15「按你推荐的来」＋「真模型盲评直接调本机 api 端点，deepseek 的 deepseek-flash」。** → ~~P0 喊停就是喊停（`21bcb4d`，42 号文 §5-bis：判据落 06i 一处、13k／13l／13p 三处调，条件都是 `trigger !== 'user'`，所以「降级成按钮、用户自己按」照旧走得通；零新字段，靠既有 `run.status` 与 `stewardLastTurn.aborted`＋一道「账要盖得住当前回合」；新件 steward-stop-respected 20 条 ⊕ 静态 ⑦ 组；四处真反向，其中「只拔 13p 预闸」恰红一条，证明两层防御各有各的用）~~ → ~~P1 失败说得出原因（`26189dd`，§5-quater：`errorClass` 在管家侧 grep 零命中，接上 06 的既有 `ERROR_CLASSES` 并按实测缺口把表从 12 条补到 32 条——`timeout`／`network`／`subagent_failed` 这三个最常见的默认出口修前竟都不在表里；取话口落 13m 不落 06i，因为 06i 读 06 会造 7 条循环边）~~ → ~~P2 拿旧的当新的（`d4c1003`，§5-quinquies：`web_fetch` 断网回落缓存仍回 `ok:true`，而生产注释自己写着「so the model can judge freshness」；徽标「缓存 · N 天前抓的」印在工具卡摘要行，判据只读 `fromCache`／`ts`，**src/ 零改动**；新件真浏览器 16 条 ⊕ 静态 J 组；三处真反向，其中第一处第一版**照样全绿**——对照组不在屏上，补第三张卡才咬住）~~ → ~~P3／P4 J09 用真模型量过（`8e95db9`，§5-sexies：`truthfulness-eval-live.js`，不进默认回归；18 发有效样本里分歧仍可辨认 17、关键实体全留 17；T03 成对判对 12/12。**结论是不改 prompt**——证据不支持「系统性抹平」，17/18≈94% 够不到零违例故标部分过门。顺带记下三个量具 bug：题面抄答案／思维链吃光预算被当成产品失败／要求模型保住输入里没有的词）~~ → **两笔 harness 债顺带还了**（`08dd129`／`e5a7c27`：失败件先列所有 FAIL 行；flaky 件留住首跑 FAIL 行——当场逮到 `dom-screenshot` 首跑红在「dark screenshot captured」，是截图没抓到不是像素比对）。**125 波退出门：J07 过门、J02 部分过门（`web_search` 面未覆盖）、J09 部分过门。**四条 J 已随 41 号方案入库对上（40 号文 §6-bis-J），J06 本波不过门（属 T02）。（steward-board 的 300 s 豁免根因已还，撤销复测要在 24 核那台跑 8 路，39 号文 §8 ①。）
→ **【2026-09-15 晚】125 收波之后做的三件**：① 用户「把 114 → 111 → 107 融进产品线」→ [43 号文](43-merge-114-111-107-into-product-line.md) 重排波次（**126 ＝ 记忆时效 ∥ 压缩 v2；127 ＝ 服务目录 ＋ 语音回填；107 排到 127 之后**），并裁掉两处文档打架（114 的归属、双壳退出时钟 —— 后者**用户当场拍板「长期并存，就现在的架构模式」**，22 号文 §8 那一条已就地划掉并写明是显式撤销）。② **flaky 治理两批，五件全部先拿病历再动手、每件都有真反向**（42 号文 §5-octies／§5-nonies）：`dom-screenshot`（四类结局压成一行 ＋ 全仓唯一漏收尸的浏览器夹具）、`quiet-card-typing`（切过去了但没待住）、`mission-index-scale` ＋ `ec-d-performance`（**`PARALLEL_EXCLUSIVE` 只按事故补过，16 个成员全是浏览器件，于是两件「自己算 P95」的墙钟门一直留在并行桶里**）、`agent-deadlock-watchdog`（「两条节点同时握租约」从来没人保证，装会合点）；**外加两把机械锁**（开浏览器必收尸 ／ 自己算 percentile 必进独占桶）—— 同一族教训第二、三次：**补完要留锁**。③ **126 派单稿已写＝[44 号文](44-wave-126-memory-scope-and-compaction-v2.md)**，出门序 111a→111e→M02→M01→111d→111b→111c→M03。

→ **【2026-09-16】126 波出门七刀**（[44 号文](44-wave-126-memory-scope-and-compaction-v2.md)）:**B 道压缩 v2 五刀全收** —— ~~111a L1 边界改 token 预算(老边界护 1 条观测,新边界护 10 条)~~ → ~~111e 历史内重复读取去重(键=路径＋正文哈希,零误报)~~ → ~~111d 摘要 prompt 双语(英文界面一直拿中文摘要,而读的那一半早就双语了)~~ → ~~111b L2 尾部单元边界＋桥接(最新一整回合放不下就一条不留)~~ → ~~111c 重播种后有界重附最近读过的文件(零磁盘读取)~~;**A 道出门两刀** —— ~~M02 时效 expiresAt~~ → ~~M01 作用域 scope~~;**M03 代答试点按设计开不了**(前置是 31 号文「眼睛」准入门,不在本波范围,如实不过门)。五个引擎开关一律默认关,B 类真模型读数按 §6 ③ 留到 107 前统一跑一轮。**三条跨刀的教训**:① 依赖图两次纠正落层(06i 够不着 06d、10 引用 06b 是循环边),这类判断机械锁比直觉靠谱;② **反向五次逮到我的判据不承重** —— 纪律升级成「反向不红先怀疑判据」＋「扫一段区间还不够,形状也要换一种」;③ **flaky 件数在同一会话里 1→3→5→7 单调上升、每轮换名字** —— 判断是长会话后的系统性退化而非逐件毛病,**下一步该换干净机器状态重取基线**,这条要进 107 的「并行回归偶发治理」栏。

→ **【2026-09-16】127 波派单稿已写＝[45 号文](45-wave-127-service-catalog-and-voice.md)**(用户重启机器后开波)。出门序 **114a→114b→S01→附件转写→`audio_transcribe`→S02→麦克风→F01**。取证第一条就是 **26 号文的摸底做于 09-03,十三天里五处坐标全失效**(工具数 63→64 实为 **96→97**、`13f` 已拆完、附件上传行号已搬家)——**设计有效、坐标失效**,照老坐标施工会踩空。另点出两处会咬人的:**`caps` 一词已被能力矩阵占用**(`models[].caps` 是模型能力标签,与 `getCapabilities()` 同名不同物);**`CONFIG_SCHEMA` 该不该 bump 与 26 号文正面打架**(118a 之后仓里约定「纯增量故意不 bump」,`01:298` 原话)。A 道实盘 15 个内置模板:**全压在前三类,代码任务／定时汇总／变化守望三类全空**(与 41 号文「前四类先盘点复用」的假设不符),且两个模板**落不进这六类**——所以分类字段必须允许为空。B 道的真风险是**无头浏览器里没有麦克风**,开工前先单跑最小夹具证 `--use-fake-device-for-media-stream`,**证不出来就如实降级判据、不假装有覆盖**。同日**用户真机走查又带进一刀**(45 号文 §2-bis):管家「老是问」查下来是**永久豁免的工具名判据写宽了** —— 裸子串无词边界,**`shell_send` 只因为名字里有 `send` 就命中**(`keyboard_send_keys` 同样躺枪),而 `run_command`／`powershell_run` 反倒不命中,方向是反的;31 号文四条红线把永久豁免排除在放权之外,**所以按现有规划这件事永远不会好**,该按 bug 修不该等放权那一波。**前提**:先证明内容扫描真的拿得到 `shell_send` 的 input,否则放开工具名判据就是开洞 —— 验不红就只做「命中原因说得出来」那一处。截图里另一条(PowerShell 命令被命令正文判据拦住)**按设计工作,不动**。**同日第二刀**(45 号文 §2-ter,用户两问):① 管家在开线程时**本来就能指派模型**(`steward_thread_new` 的 `tier` → `13q:575` 写会话级 `engineRoute`,还会把线程切到原生 provider 引擎),**断的是定时任务那条路** —— `steward_schedule_create` 有权限档没有模型档,触发时 `13s:462` 直接 `createSession` 不经过它;而这不是疏漏,`06j:532` 白纸黑字写着「**engineRoute 留给 127 波**」(同文件 `:43` 还有 `playbook/workflow → 127 波`)。② 「多条 Kimi CLI 线程不能并行」**跟 Kimi 无关**(`05b` 零并发限制) —— 卡的是按工作文件夹的写互斥 `cwd-write:<hash>`(`13n:93`),且 `13n:182` **一律按「写」处理**(只读回合放宽自己写着「留后续波」);放大它的是 `06j:532` 那句「工作目录回落 **defaultWorkspace**」,**于是所有定时任务天然互抢同一把锁**。不同文件夹是能真并行的(上限默认 5)。治法 S-a 补 `tier`／S-b 照搬「自动开自己的文件夹」(**不放开 `cwd` 入参**)进本波,S-c 只读回合放宽**记债**。
