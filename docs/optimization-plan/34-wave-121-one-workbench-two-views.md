...
- **全量**：`3990a3e` 上 306/3/7，三条红串行复验：`repo-hygiene`（端口争用）、`context-compact-v2` 绿，`mission-index-scale` 真红→`3e8e65e` 还原后串行 4 次 3 绿 1 红（性能预算 P95 445 ms，机器没凉）。最终 HEAD 未重跑全量——主会话在验收修补提交后补跑一轮。
