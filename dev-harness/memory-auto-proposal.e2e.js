...
ok(/turnState\.engine\s*===\s*'openai'/.test(uiSource) && /resultOk\s*===\s*true/.test(uiSource) && /suggestMemoryFromTurn/.test(uiSource), 'UI requests a candidate only after a clean provider turn');
