...
ok(names.length === 100, `L1 注册表 100 个工具(got ${names.length}; 第 116 波 116c 新增 17 个 steward_* 管家工具:63→80;116-2a 增 steward_thread_permission:80→81;116-2b 增 steward_thread_note:81→82;116g 增 steward_missions:82→83;116h 增 steward_thread_prioritize:83→84;116-2e 增 steward_config_get/config_set/playbook_draft/skill_toggle/quick_ask:84→89;117m-A4 增 steward_thread_stop:89→90;123-M2 增六件定时任务 steward_schedule_{create,list,pause,resume,run_now,delete}:90→96;127-114c③ 增 audio_transcribe:96→97;129b 增三张只读清单 steward_{skills,providers,playbooks}:97→100)`);
...
ok(names.length === require(path.join(__dirname, '..', 'facts.json')).nativeTools,
  `L1b 注册表工具数与 facts.json 同源(${names.length} vs ${require(path.join(__dirname, '..', 'facts.json')).nativeTools})`);
