...
(async () => {
  ...
  try {
    ...
    const capped = await server.saveAgentWorkflow('personal', PROJECT, { id: 'capped-budget-flow', title: 'Capped budget flow', nodes: [{ id: 'worker', task: 'explicit large budget', role: 'worker', maxIters: 350 }] });
    ok(capped.nodes[0].maxIters === 300, 'explicit workflow maxIters is capped at 300');
