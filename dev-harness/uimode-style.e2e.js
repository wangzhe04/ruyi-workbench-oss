...
const FAKE_PORT = 8995, WB_PORT = 8996;
...
(async () => {
  ...
  // 青花设计 token 补全 + 如意云纹极淡水印。纯字符串/正则断言（与本仓其它 e2e 同款静态护栏风格）。
  {
    ...
    ok(/无法连接本地服务/.test(appjs), '(S2) 故障卡大标题「无法连接本地服务」');
    for (const r of ['端口被占用', '服务未启动', '被安全软件拦截']) ok(appjs.includes(r), `(S2) 故障卡列可能原因「${r}」`);
    ok(/'重试连接'/.test(appjs) && /await bootData\(\)/.test(appjs), '(S2) 「重试连接」按钮重跑 bootData（不重复 bindEvents）');
    ok(/查看日志 \/ 诊断/.test(appjs), '(S2) 「查看日志 / 诊断」入口存在');
    ...
    for (const t of ['--sp-7', '--sp-8', '--r-sm', '--r-md', '--r-pill', '--ease-out', '--dur-fast', '--dur-base', '--dur-slow', '--info']) {
    ...
    ok(/opacity:\s*\.0[0-5]\b/.test(css.match(/::before\s*\{[^}]*var\(--ruyi-cloud\)[^}]*\}/)?.[0] || css), '(S4) 云纹水印 opacity ≤ 0.05');
  }
  ...
  try {
    ...
    ok(st.json && st.json.config && st.json.config.uiMode === 'pro', '(A) illegal uiMode cleansed → pro (got ' + (st.json && st.json.config && st.json.config.uiMode) + ')');
