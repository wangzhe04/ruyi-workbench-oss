...
| **9125** | **autonomy-shell-sandbox(第31波B · shell 沙箱化 L1)**(单 WB 端口;A 段 require server.js 直接单测 guardFileToolPath 17 断言[全模式 autoexec 拒绝 + 路径变形大小写/尾点/尾空格 + CI 扩展/.gitignore 不误伤/file_edit/delete 对称/只拦写不拦读];B 段起 WB `/api/tools/file_write` 全工具分发验证 6 断言)。L1: `AUTOEXEC_DENYLIST` 从授权书层下沉到 `guardFileToolPath`,bypass/plan/default 全模式覆盖 .git/hooks/.github/workflows/.gitlab-ci.yml/Jenkinsfile 等自动执行入口 |
