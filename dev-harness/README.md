...
| **9101-9102** | **agent-steer-node(v1 定向插话)**(9101 = 内联 fake-openai[子请求逐轮吐**distinct-arg** file_read × ROUNDS 再吐结论文本,每帧 sleep 200ms 把节点拉长;捕获全部请求体供断言插话注入];9102 = WB,挂 `WCW_FAKE_CLAUDE` 令 claude 节点确定性)。运行中 DAG 对**单个模型节点**定向插话(steer_node action):4 节点 DAG `work→{later,claudenext,gate}`,work 运行时 later/claudenext/gate 处 queued(队列断言零竞态)。(a)对运行中 work 插话 `STEER_MARK`→后续 fake 请求体现 `[编排者插话] STEER_MARK`(迭代边界 drain);(b)持久化 work.progressLog 出「插话 · STEER_MARK」里程碑;(c)对 queued Claude 节点插话进入持续 stream-json 输入队列;(d)对已 succeeded 节点(run 仍 live)steer→409「节点已结束」;(e)每节点队列 cap:queued 节点连插 3 成功、第 4 条 409「插话队列已满」;(bonus)3 条 queued 预插按 CAP1→CAP2→CAP3 顺序整批出现在 later 启动后的**同一**请求体里;(f)对 vote 质量门节点(确定性短路,不经过模型)steer→409「确定性质量门…」;(g)伪造 sessionId + 真实 runId→404(run 归属校验,pause/resume/stop/steer_node 共用同一守卫);(h)空文本→400「插话内容不能为空」;(i)不存在的 nodeId→404「节点不存在」;(j)run 到达终态后再 steer→409「工作流当前未运行」) |
...
| **动态端口** | **workbench-memory(工作台跨会话记忆)**：文件型记忆库 `dataRoot()/memory/{global,project/<projectKey>}/<id>.md`；覆盖起草确认落盘、项目隔离、来源锁定、关系冲突标记、正文/索引预算、伪造围栏中和，以及 Provider/Claude 双引擎真实注入。默认模式每条用户消息轻量扫描当前项目 + 全局元数据、只注入 Top-3，并在零命中时发送机器可读回执；同时验证逐条排除、当前会话关闭、恢复默认、活动流计数与 Claude 最小 `--add-dir` 授权。 |
...
| **9118** | **wait-primitive(第28e波 · wait_for 等待原语)**(单 WB 端口,哑 provider 仅过 launch 引擎门,纯 wait DAG 不调用它)。[S] 静态锁:reducer 传 isWaitNode、就绪 wait 节点单列 toArm(不占并发槽)、外壳 arm(queued→waiting+waitStartedAt)/poll(evalWaitCondition)/tick(仅 waiting 时可中断 abortableSleep 防 busy-spin)、wait×worktree 互斥、process 仅信号 0、file 过 guardWorkspacePath、url 过 ssrfCheck+httpGetGuarded、状态文案 waiting:等待条件。[P] normalizeWaitSpec(mode 校验 + timeoutMs[1s,24h]/pollMs[500ms,60s] 夹取 + pid≤1 拒 + 缺 path/url 拒)+ evalWaitCondition 四模式(timer 到点 / file 存在性匹配+护栏拒→failed / process 信号0 存活 / url SSRF 拒→failed·可达→done·连不上→维持等待)。[H] Live:纯 timer wait 节点 arm→waiting→succeeded(零 provider 调用);file wait 中途建文件→succeeded;文件永不建+短 timeout→failed(超时)。reducer 组合锁在 scheduler-reducer.e2e §16(toArm 分拣/anyWaiting 不判环/失败依赖 block/条件 skip) |
...
| **9120** | **monitor-incremental(第29波 · §29a 增量监控 + §29c 指标)**(单 WB 端口,哑 provider,wait DAG + 手工胖历史快照)。[S] 静态锁:config 键归一、readAgentRunEvents 坏行免疫 + 分页上限、events 路由排在通配 GET 前、node_progress 跳过 gen、digest 视图、run_pool 事件 ≥4 处、单 run GET 内存路径 + 跨会话 404、bumpRunIntervention ≥7 点、failuresByClass 幂等重算、errorClass ≥10 设置点、mission 预算分子/分母落账、intervention logEvent ≥3、ops 端点、两引擎 subagent_usage、前端增量缓存/回落/force 全量/乱序守卫/僵局自愈。[H] Live:digest 字段与瘦身(<20% 全量字节 + 不带 result)、events afterSeq 全量/尾段/幂等重放(断线补播)/越界空/limit+hasMore、坏行注入免疫、三端点无 token 403、错 session 空+404、活跃 run live 叠加、pause 后 digest 即时 paused、run.metrics 干预计数、【验收锁】忠实模拟客户端算法 20 tick(中途触发 settle 制造真实事件流量)增量字节 ≤ 全量 20%(实测 ~11%)、ops 形状 + mission_start 计分母 |
| **9122** | **orchestrate-model-select(第30波 · AI 自主编排按难度选模型)**(单 WB 端口,哑 provider,DAG 物化不执行也能验 node.model)。[S] 静态锁:config agentAutoModelTiering(opt-in)· 三写入点/spawn/池用 resolveNodeModel(带 engine)· propose_task model 通道 · 两引擎注入 buildModelHint(引擎分组)· Claude 钳拆分 · 【不】白名单丢弃(isKnownModelId 已移除)· inherit→空归一 · schema 描述引擎匹配。[P] 纯逻辑:modelCapabilityTier(flash→快/max→强/plus→均衡[对抗轮修]/reasoner→强/标签参与)· resolveNodeModel(显式合法与未列出都尊重原样[消除误杀回归]/inherit→空/角色/tier兜底)· tierModelForNode(引擎感知+池拓 knownModels+排除预设别名,修 provider.models=[] 静默失效)· buildModelHint 引擎分组(Claude 预设别名不进 OpenAI 组)。[H] Live:显式合法/未列出模型原样落盘、inherit→空、省略→空。对抗轮 7 CONFIRMED+6 DOWNGRADED 全修:去白名单丢弃根治回归+假安全+两写入点不一致，引擎分组+钳拆分+tier池拓宽 |
| **9123-9124** | **autonomy-benchmark(第31波 · §5 产品侧三任务基准)**(9123 = 内联 fake[按 session 初始消息任务标记分支:REFACTOR 按 [mK] 驱动 file_write 重构文件+mission_update done;DIGEST file_read 来源->file_write 汇编片段->mission_update done;BUILDFAIL m1 推进后 m2+ 纯文本停滞模拟卡在修复];9124 = WB,permissionMode bypass,mission until-done + budget{maxAutoTurns:12,maxTokens:50000})。**A** REFACTOR 4 里程碑无人值守跑完(mission_complete+4 文件改对);**B** DIGEST 3 里程碑无人值守跑完(3 汇编片段);**C** BUILDFAIL m1 推进后 m2 停滞->state:stuck+autoMode=supervised(无进展自动暂停触发);**§5 产品侧验收** ① 完成率≥2/3(实 2/3) ② 干预≤1次/任务(实 0) ③ 暂停触发率 100%(实 1/1) ④ token 不破预算(autoTurns≤12)。另:autonomy-durability C 段新增 §5 MTTR<30s 时钟断言(实 8647ms) |
| **9125** | **autonomy-shell-sandbox(第31波B · shell 沙箱化 L1)**(单 WB 端口;A 段 require server.js 直接单测 guardFileToolPath 18 断言[全模式 autoexec 拒绝 + 路径变形大小写/尾点/尾空格 + CI 扩展 + `.git/config`/`config.worktree` 的 `core.hooksPath` 绕过封堵 + .gitignore 不误伤/file_edit/delete 对称/只拦写不拦读];B 段起 WB `/api/tools/file_write` 全工具分发验证 6 断言)。L1: `AUTOEXEC_DENYLIST` 从授权书层下沉到 `guardFileToolPath`,bypass/plan/default 全模式覆盖 .git/hooks/.git/config/.github/workflows/.gitlab-ci.yml/Jenkinsfile 等自动执行入口 |
| **9126** | **auth-deny-default(第33波 · 全 GET 面 host 校验 + 声明式 auth 路由表 deny-by-default)**(单 WB 端口,临时 HOME;无 fake provider,直探鉴权门)。A 段顶层 hostAllowed 门(rebinding Host=evil 拦 GET / 与 /api/status 403、body 不含 token、loopback 200);B 段 deny-by-default(未声明 /api/typo -> 403 非 404);C 段 3 个 GET 收紧[agent-roles/agent-workflows/playbooks 原 GET 无 host 门+无 token 门,现 token-browser:浏览器无 token 403/有 token 200/loopback 豁免 200/rebinding+token 仍 403];D 段鉴权级别回归[open(status)/token-browser(sessions,loopback 豁免保持)/token(agent-runs,audit 始终须)/body-token(mission)]。治第29波 backlog #0 + S0 教训 opt-in 名单根因 |
| **9121** | **autonomy-resume(第29波 · §29b 自动恢复分级)**(单 WB 端口,同端口串行 4 次 boot;token 每 boot 重生须重读 runtime.json)。[P] 源抽取 classifyNodeResumeRisk/classifyRunResumeTier:wait/gate(vote|dedupe)/read=safe、exec 一律 manual(worktree 不豁免)、edit 无证据 safe/有 artifacts 或续点写族 ok 步 manual(ok:false 与只读步不算)、run 级任一危险→manual、succeeded 不进 reset 集不拖累、权限面变化→manual、老 run 缺字段跳过该信号。[S] 静态锁:opt-in 默认 false(===true)、boot 挂点在标死之后 fire-and-forget、AUTO_RESUME_MAX=2、崩溃环护栏先落盘 fail-closed、boot 标死写盘防炸、冷 resume/retry 计干预(boot 不计)、resume 清 resumeTier/pendingReview、中断时盖分级戳、permissionModeAtLaunch 存档、run_auto_resume/run_resume_deferred 事件。[H] 三次 boot:①默认关=interrupted 停住只盖戳(零行为变化);②开=安全 run(含上次遗留)自动续跑到 succeeded + autoResumeCount=1 + 事件链,危险 run(exec)停 paused + manual_resume_required(exec_tier) + run_resume_deferred,人工 resume 入口不受限;③autoResumeCount=2 → 降 manual(auto_resume_loop) |
...
| **动态端口** | **claude-exe-resolve(P1 · cmd8191 根治:npm shim → 真身 claude.exe 解析)**(`getFreePort()` 动态端口,WB 挂 `WCW_FAKE_CLAUDE`,临时 HOME;假 npm 布局 = `claude.cmd` + `node_modules/@anthropic-ai/claude-code/bin/claude.exe`[node.exe 副本,`--version` 探测即过])。[A] 单元:绝对路径 shim 解析出 exe、裸名沿 PATH 解析、无布局/shim 不存在原样返回、非 batch 输入不动、预算 7900→32000 升档(未解析 shim 仍 7900 回退设防)、normalizeConfig 咽喉点替换、memoize 一致;[B] 集成:config.claudePath 指假 shim → /api/status 的 config.claudePath 与健康详情均为 exe(全部消费方经咽喉点一致受益) |
| **动态端口** | **index-dedup(第35波 P2 · 索引去重注入:技能/记忆/编排索引改走 stdin 一次性注入)**(`getFreePort()` 动态端口,WB 挂 `WCW_FAKE_CLAUDE`+`WCW_FAKE_ARGV_CAPTURE`+`WCW_FAKE_STDIN_CAPTURE`+`WCW_FAKE_SID`[resume 保真:回放同一 session id],临时 HOME + 用户技能库 2 技能)。[A] 首轮:stdin `<workbench-context>` 注入技能索引、`meta.indexInjected=true` 带 hash、append 不含技能索引、`<current_user_message>` 定界保留;[B] 次轮同内容:去重跳过(indexInjected=false、hash 不变、stdin 无注入块、recovery history 照常);[C] 启用技能集变化:hash 变→重注(新旧技能都在);[D] 再稳定:恢复跳过;[E] slash 轮:不注入(占首 token)不污染 hash,下轮仍去重 |
| **动态端口** | **agent-node-wrapup(多 Agent 长时运行控制)**。Provider 协作节点在下一迭代边界收到自动 `[编排者插话]` 后正常收尾；忽略指令的长流节点在宽限期后仅自身被中止、并行兄弟继续成功；Claude 工作流节点以持续 `stream-json` stdin 实时收到同一收尾指令并在结果/进度日志/捕获信封三处留证。 |
| **9137-9138** | **agent-workflow-transient-repro**(第36波自 9087/9088 迁出,消除与 agent-workflow-audit-fixes 撞车) |
| **动态端口** | **storage-steward(v1.9 数据管家 · 统一保留策略 + 各仓统计 + sweep)**(A 段 require server.js[临时 RUYI_HOME];B 段 `getFreePort()` 动态端口起 WB)。[A] 单元:normalizeStoragePolicy clamp(越界/非数/缺省)、logs 按文件名日期保留(2020 删/今日留)、真终态 run 事件日志 gzip 归档(原文删+.gz 在+readAgentRunEvents gz 回退全读 3 事件)、paused/新终态不压、webcache 默认不限(尊重"离线无价"设计)/opt-in LRU 删最老、单目标 sweep 不碰它仓、collectStorageStats 各仓 bytes/files、重复 sweep 幂等;[B] 集成:summary 无 token 403/有 token stores 齐、policy 越界 clamp 回显并持久、clean 未知 target 400/无 token 403/target=logs ok |
| **动态端口** | **session-storage-v2(v1.9 第二波 · 会话存储 v2[头+append-only NDJSON 正文]+ 引擎转录 GC)**(A/B 段 require server.js[临时 RUYI_HOME + `WCW_CLAUDE_PROJECTS_DIR` 重定向];C 段 `getFreePort()` 动态端口起 WB)。[A] 存储 v2:legacy 懒迁移(head v2 标记+计数、双正文落盘、v1bak 原文备份)、v2 回读一致+v1bak 自清、快路径 append(前缀字节不动)、零增长 save 不碰正文、前缀改写→全量重写、撕裂尾行物理截断修复(防「焊行」→ corrupt 丢数据回归)、中间行损坏 v1bak 回退/无备份 .corrupt 隔离、**prevbody 慢路径崩溃防线**(收缩中断回滚/增长型中断按计数差异回滚/陈旧快照不恢复顺带清)、deleteSession 全载体清、头是提交点(未提交尾巴按头计数截断)、listSessions 兼容;[B] 转录 GC:dirKey 映射、saveSession 钩子自动登记白名单账本、策略 clamp(负→0/缺省 30)、sweep 全谱(活引用保留/无引用超期删+子代理侧录连带/未超期留/**红线:账本外绝不碰**/账本除名)、0=关不动;[C] 集成:summary 带默认转录策略 30 天、policy 越界 clamp 持久、clean target=engine-transcripts ok |
...
| **动态端口** | **boot-resume-parallel(第40波 · boot 中断恢复并发化 + syncRunEventSeq 尾窗化)**(U 段 require server.js[临时 RUYI_HOME];L 段 `getFreePort()` 动态端口起 WB)。[U] 单元:mapPool 保序/全执行/真并发(1<maxInFlight≤limit,wall-clock 反串行证明)、syncRunEventSeq 小文件全读/大文件(>512KB)尾窗==独立全扫/窗内无 seq(巨单行)回落全读/撕裂尾行同旧语义;[L] 集成:6 个 running run(各 ~1MB events)boot 全标 interrupted + run_interrupted 落 seq 1001(大文件尾窗快进精确未错号)、boot 防卡死上界、autoResume(exec-tier→paused+run_resume_deferred / wait-node→autoResumeCount+1 护栏先落盘+run_auto_resume 事件) |
| **动态端口** | **metrics-panel(第40波 · /api/metrics 性能观测面)**(U 段 require server.js[临时 RUYI_HOME];B 段 `getFreePort()` 动态端口起 WB)。[U] 单元:normalizeMetricsPath id 归一化、recordRequestMetric 六桶/环形 300 封顶/slowest ≤8 降序且无会话 id 泄漏、maybeRecordStorageTrend 追点+1h 节流、readStorageTrend 缺文件空史;[B] 集成:无 token 403/有 token 字段齐、buckets 之和==total、/health 不计数(探针不淹没真分布)、storage-trend.json 落盘+再调不追点 |
| **无端口** | **tool-dispatch(第41波 V2.0「立柱」41a/41b · toolCall 表驱动注册表 + guard 声明化行为锁)**(纯 require server.js,不起 WB)。[L1-L5] 行为锁(内省 TOOL_HANDLERS 注册表,非 grep 源码形状):每条目 handler/paths 声明齐整、paths:null 必带 guardNote(忘声明 vs 录在案豁免结构分离)、paths 非 null 的 handler 源必含 guardFileToolPath/guardDownloadDest 调用(archive 漏 guard/desktop_screenshot 越界这类漏审整类收口)、注册表键集===NATIVE_TOOL_PACKS 键集、每键有 tier、edit/exec 级全在册、permission_prompt tier 显式化(原靠 unknown→exec 兜底,行为锁首擒);[B1-B3] 分发行为:未知工具 throw 同形同文、glob 真实分发、远端 provider 越界 file_read 被拒、**project_snapshot 补读闸**(41a 表驱动首擒:file_list 同族都有闸唯独它没有)越界拒/界内照常 |
...
| **动态端口** | **context-compact-v2(第45波 · 上下文压缩 v2)**(fake-openai 扩 FAKE_CONTEXT_400_ONCE/FAKE_RECORD_SUMMARY_DIR;真 WB)。[U] 45d 校准原语(EMA 样本门/clamp/窗口学习只降不升/providerContextWindow 咽喉点);[A] 45a 预算化(小历史单次/超大历史 fit 截断 droppedMiddle/摘要 payload ≤ 预算/fit 不 mutate 调用方/保头保尾);[B] 45b 真回合 400 强压重试(forced_400 事件/回合 ok/重播种落正文/窗口学习落盘);[C] 45c 分类器(over_window 可重试/progress_made 防重放/auth 仍 definitive)。隔离纪律:require 前无条件覆盖 WIN_CLAUDE_WORKBENCH_HOME(本机系统级指向真实根,曾翻车污染) |
| **LIVE(手工)** | **compact-quality-live(第45波45e · 压缩质量评测夹具,SKIP 登记)**。10 事实标记埋入合成历史 → 真调 providerSummaryCall(读真实 config 的 provider 凭据)→ 摘要文本命中计数(确定性,无裁判模型)。验收 ≥8/10。实测基线:deepseek-v4-flash 裸四段式 6/10 → 加保真要求 9/10(prompt 迭代由数据驱动) |
