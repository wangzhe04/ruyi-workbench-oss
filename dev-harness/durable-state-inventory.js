...
const entries = [
  ...
  ['session-head','<data>/sessions/<id>.json','02-session-store.js','sessionPath(id)','object / storageVersion 2 (116-4 added launchedBy + stewardLastTurn; 116-5 added threadBrief {title,gist,at,model,stage} + titleSource - all additive, no schema bump, absent on every pre-existing session)','atomicWriteJson + per-session chain','rename to .corrupt on invalid','one per session','session cache/index','load v1 backup or reconstruct v2 head','session v2 head has dedicated migration and body coordination',DEDICATED],
