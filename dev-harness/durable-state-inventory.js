...
const entries = [
  ...
  ['session-head','<data>/sessions/<id>.json','02-session-store.js','sessionPath(id)','object / storageVersion 2 (116-4 added launchedBy + stewardLastTurn; 116-5 added threadBrief {title,gist,at,model,stage} + titleSource; 117z-E2 added createdBy (origin mark, written once at the two steward createSession sites, never in the applySessionMetaPatch whitelist) + desktopTools (true|false|absent session-level override of the global allowDesktopTools gate); 127-2quater-B2 added stewardTaint {by,at,turnSeq} (sticky external-content taint: written in-memory by runSessionTurn emit when a web/http/browser/bridged tool call returns its result (not at tool_use, so a pending call never taints itself) or a subagent/workflow starts, persisted by the engine save, deleted when the next user-authored turn starts) - all additive, no schema bump, absent on every pre-existing session)','atomicWriteJson + per-session chain','rename to .corrupt on invalid','one per session','session cache/index','load v1 backup or reconstruct v2 head','session v2 head has dedicated migration and body coordination',DEDICATED],
  ...
  // W2 迁移中心:指令文件导入的所有权账、安装登记表、迁移日志与改写前备份。
  ['agent-instructions-import','<data>/memory/.agent-instructions-import-v1.json','06d-memory-domain.js',"'.agent-instructions-import-v1.json'",'object / schema 1 (sources.<key>: file/sourceHash/importedAt/entries[{id,hash,core}]/dismissed/sourceChanged/userModified)','atomicWriteJson inside a serialized chain','wrong schema or unreadable = empty (re-imports only what is still absent; entry-file hashes gate every overwrite)','one record per instruction source (4)','none','delete it: the next sync re-imports sources whose agentmd-* entries are missing','ownership proof for imported agentmd-* memories: without it a user edit could be silently overwritten',SHARED],
  ['install-registry','<data>/install-registry.json','00-boot.js',"'install-registry.json'",'object / schema 1 (packages[{packageRoot,version,launchMode,firstSeenAt,lastLaunchedAt,pid}])','atomicWriteJson (boot upsert; migration center remembers reference-discovered packages and forgets recycled ones)','unreadable = empty list; rows with missing/relative roots dropped','64 rows; rows whose directory is gone are pruned on the next boot','none','every launch re-registers itself','the only place that knows where the other Ruyi packages on this machine live (all packages share one data root)',SHARED],
  ['migration-log','<data>/migrations/<ts>.json','13u-migration-center.js',"'migrations'",'object / schema 1 (files[{file,kind,backup,items[{serverId,action,changes,removed}]}], undoneAt)','atomicWriteJson once per apply; rewritten once on undo','unparsable log is skipped (that migration can no longer be undone from the UI; its backups remain)','newest 20 listed; one per apply','none','backups next to each edited file','undo ledger: undo only reverses values that still equal what the migration wrote',DEDICATED],
  ['migration-backups','<file>.bak-ruyi-migrate-<ts> beside ~/.claude.json / ~/.kimi-code/mcp.json / ~/.codex/config.toml / <data>/config.json / generated mcp','13u-migration-center.js',"'.bak-ruyi-migrate-'",'verbatim copy of the file before the migration edited it','fsp.copyFile before the first write to that file','never read by the app (human recovery only)','one per edited file per apply; removed when nothing in that file ended up changing','n/a','copy back by hand','insurance for edits to externally owned CLI configs',EXTERNAL],
  ...
  // 134: 后台任务完成台账 —— shell_start(command) 后台命令与后台子代理的完成回执,按线程一个文件,
  // loadSession/saveSession 两头按 jobId 合并进消息流(重开线程也只看一次)。
  ['background-job-ledger','<data>/sessions/background-jobs/<sessionId>.json','11-native-tools.js','backgroundJobFile','array of job receipts / implicit v1 (id/shellId/name/sessionId/status/exitCode/output/truncated/completedAt)','unique fixed tmp name per file + renameSync INSIDE the child-close synchronous event (single-threaded event loop serializes read-modify-write; async atomicWriteJson would interleave concurrent closes and lose updates)','unreadable ledger yields an empty list: a lost receipt means a missing notification, never a wrong one','last 100 receipts per session (read side slices, write side trims)','seen job ids per session (backgroundJobSeen) so a reconnect never re-pushes a receipt','the receipts are the record itself: a lost row only loses that one notification; running background jobs die with the server and are never replayed','fixed-tmp JSON writer exempted beside the autonomy-durability whitelist (same 5-site lock)',DEDICATED],
  ...
  // 116b: 管家收件箱与游标(27 号文 §11.3)。开关关(stewardEnabledV1=false;121 波 K0 起默认【开】)时这两个面一个字节都不写,
  ...
  ['steward-cursor','<data>/steward/cursor-v1.json','13i-steward-inbox.js','STEWARD_CURSOR_FILE','object / schema 1','atomicWriteJson','unreadable or wrong-schema cursor restarts from a cold baseline: sources are re-baselined instead of replayed, and the inbox-tail dedupe set prevents duplicates','active sessions and runs only, capped at 500 sessions / 500 runs / 2000 pending ids / 500 sessionTurns entries per save (116-4 added sources.sessionTurns: sid -> {turnSeq, stamp}, a backward-compatible field — an older cursor without it simply makes every session first-sight)','stewardRuntime holds the live cursor for this process','rebuild from the inbox tail plus a fresh source baseline','small workbench-owned JSON on the shared atomic-write lifecycle',SHARED],
  // 117 波 T1 重钉:owner 由 13g-steward.js 改为 13j-steward-tool-base.js
  // (决策日志与记忆存储的常量与原语随拆分前移，零行为搬家)。
  // 116c: 管家决策日志与管家记忆(27 号文 §3.5 / §4)。同 116b 的两个面:开关关(stewardEnabledV1=false;
  // 121 波 K0 起默认【开】)时一个字节都不写,目录也不建 —— 工具层的门控壳在写任何东西之前就返回 steward.disabled。
  ['steward-decisions','<data>/steward/decisions-v1.ndjson','13j-steward-tool-base.js','STEWARD_DECISIONS_FILE','NDJSON decision rows / decisions-v1','repairMissionChangeTornTail + appendFile through a per-process serialized chain (the session-changes primitive)','torn tail truncated before append; unparsable rows are skipped by readers','append-only; one row per steward write action (thread_new/continue/rename, decide, run_action, memory_write/veto)','the per-process seq counter only labels rows within one process lifetime','the ledger is the record itself: a lost append means one action has no explanation row, never a wrong one','append-only ledger cannot use whole-file JSON replacement',DEDICATED],
  ['steward-memory','<data>/steward/memory-v1.json','13j-steward-tool-base.js','STEWARD_MEMORY_FILE','object / schema 1','atomicWriteJson through a per-process read-modify-write chain','unreadable or wrong-schema file reads as an empty store (never salvaged, never partially trusted): steward memory is a bypass enhancement, a corrupt one must not wedge a turn','at most 200 active entries; each entry text is capped at 300 characters','no process cache: every read re-reads the file inside the write chain','the user re-states the fact and the steward writes it again; nothing else depends on it','small workbench-owned JSON on the shared atomic-write lifecycle',SHARED],
  // 116f: 到访归档(27 号文 §8.1 原则 8「对话不是档案」)。同上面四个面:开关关时一个字节都不写,
  // 目录也不建 —— /api/steward/visit 在开关关时直接 409。
  // 117 波 T2 重钉:owner 由 13h-steward-runner.js 改为 13q-steward-runner-turn.js —— 拆分把这个面的
  // 写原语(stewardVisitsDir / stewardTrimVisits / stewardArchiveConversation,那条 atomicWriteJson 就在
  // 其中)整体搬进 13q,13h 只剩注册表与路由。锚点仍是 STEWARD_VISITS_DIR(它在 13q 的 stewardVisitsDir
  // 里被读到);常量本身随共享常量块落在 13m,与 T1 那次「常量与原语同去一个文件」不同,故这里跟【写的那一头】。
  ['steward-visits','<data>/steward/visits/<ISO>.json','13q-steward-runner-turn.js','STEWARD_VISITS_DIR','object / schema 1','atomicWriteJson','an unreadable archive is a lost transcript copy, never a wedged steward: the live conversation was already truncated inside the same write chain and nothing reads archives back','one file per visit, oldest files pruned beyond 200','none: written once at visit start and never read back by the runtime','nothing rebuilds it - the archived turns are gone from the live session by design (the decision ledger keeps what the steward actually did)','small workbench-owned JSON on the shared atomic-write lifecycle',SHARED],
  // 117w-W1 提交②(27 号文 §11.19.2 / §11.19.5 推荐 A):管家省略 cwd 时派生的子工作区。登记它是因为
  // 这一刀让工作台【第一次主动在用户的主目录下建目录】——目录本身由 13k 建,里面的东西全是线程按
  // 自己的权限写下的用户工作产物(报告、抓下来的数据)。工作台管着的那一半(W7 起是 stewardManagedWorkspaces
  // 里的那一行,不再进 workspaces[];以及 stewardWorkspaceRoot 键)住在 config.json 里,由上面的 config 条目覆盖。
  ['steward-workspace-derived','<stewardWorkspaceRoot, default ~/Ruyi>/<slug(title)>[-N]','13k-steward-threads.js','stewardClaimDerivedWorkspace','directory only / no schema (the workbench writes no file of its own inside it)','fsp.mkdir({recursive:true}) inside the mutateConfig critical section; the row that records it goes into config.stewardManagedWorkspaces [{path,at,adopted?}] (W7: NOT into workspaces[], which is the user-facing favorites list) through mutateConfig -> writeConfig','a collision with an existing NON-EMPTY directory takes the next -N suffix; an existing EMPTY directory is reused (reopening the same task must not grow a second empty shell); a same-named plain file or an unreadable entry skips to the next suffix; after 50 tries derivation gives up and the thread keeps the defaultWorkspace fallback; a failed config write removes the directory it just created','slug capped at 64 characters; 50 collision suffixes per title; the stewardManagedWorkspaces table caps at 200 rows (01-config STEWARD_MANAGED_WORKSPACES_CAP) and drops its OLDEST rows beyond that instead of refusing (the user cannot see or clear that table), so a fresh derivation always lands; W7 retired the 117w-W1 4th-cut pre-derivation cap check on workspaces[] because derivation no longer writes there','none: the path is written into the session head and never cached (127 2-ter: a prompt scheduler task derives ONE such folder on its first steward-on trigger through the same 13k primitives and reuses it via tasks-v1.json workdir while the folder is still a known workspace; W7: a mission container remembers the first folder one of its threads used and later steward threads of that mission reuse it)','nothing rebuilds it - the files inside are user work products a thread produced, and the workbench never regenerates user content','the workbench creates the directory but owns nothing inside it: threads write there under their own permission and the workbench never reads it back, rewrites it or deletes it',EXTERNAL],
  ...
  // 123-M1（37 号文 §3.2；设计权威 29 号文 §3「数据模型」）：定时任务的两个持久化面。
  // 同 116b 的收件箱两面：schedulerEnabledV1 显式 false 时一个字节都不写，<data>/scheduler/ 目录本身
  // 也不建 —— startScheduler 立即返回，六条路由一律 409。开着但零任务时也不起 interval。
  ['scheduler-tasks','<data>/scheduler/tasks-v1.json','13s-scheduler.js','SCHEDULER_TASKS_FILE','object / schema 1 (tasks[] = 29 号文 §3 的 Commitment 形状：schedule/payload/target/autonomy/policy/state + revision; 127 2-ter: optional target.tier strong|fast (absent = follow global, kept only for prompt + new-session) and optional server-owned workdir = the folder derived for this task on its first steward-on trigger, never accepted from HTTP/tool input and re-checked against workspaces[] on every trigger)','atomicWriteJson through a per-process serialized chain (schedulerTasksChain)','an unparsable or wrong-schema file is RENAMED to tasks-v1.json.corrupt (rename, not copy: leaving it in place would re-quarantine on every read) and the scheduler starts from an empty table with lastError set; individual rows that fail normalizeSchedulerTask are skipped rather than silently rewritten','200 tasks (POST refuses beyond it with scheduler.capacity_exceeded); per-task maxRunsPerDay and a global 200 fires/day counter live in the same file','schedulerRuntime.tasks is the live copy for this process; every mutation writes the whole file back through the chain','the fires ledger says what actually ran, but a lost tasks file loses the DEFINITIONS - the user re-creates them (nothing else in the workbench can reconstruct what someone promised themselves)','small workbench-owned JSON on the shared atomic-write lifecycle',SHARED],
  ['scheduler-fires','<data>/scheduler/fires-v1.ndjson','13s-scheduler.js','SCHEDULER_FIRES_FILE','NDJSON fire rows / fires-v1 (seq + occurrenceKey + executionGeneration + phase registered|dispatched|running|reconciled + outcome succeeded|failed|needs_you|skipped|unknown)','repairMissionChangeTornTail + appendFile through a per-process serialized chain (the session-changes primitive, same as steward-inbox)','torn tail truncated before append; unparsable rows and rows whose phase is outside the four-phase whitelist are skipped by readers','append-only; four rows per fire at most, bounded by the per-task and global daily caps above','the monotonic seq and the per-occurrence attempt counter are rebuilt from the tail at load; no other in-process cache','the ledger is the record itself: a crashed run is reconciled as outcome unknown on the next boot (never as success, never re-dispatched), and DELETE of a task deliberately leaves its rows in place','append-only ledger cannot use whole-file JSON replacement',DEDICATED],
  ['pretender-index','<data>/sessions/.pretender/projection-index.json','13e-pretender-index.js','PRETENDER_INDEX_FILE','object / schemaVersion 5','atomicWriteJson','invalid or older schema triggers full rebuild (117r-D1: 4 -> 5, the card production CONDITION changed - steward-watched threads now get a card too; without the bump those rows stay card:null forever because their session files never change again)','one materialized projection','stamp-aware process cache','rebuild from sessions/runs/usage','fully regenerable materialized index',REBUILD],
