...
const entries = [
  ...
  // 自己的权限写下的用户工作产物(报告、抓下来的数据)。工作台管着的那一半(workspaces[] 里的那一行
  // 与 stewardWorkspaceRoot 键)住在 config.json 里,由上面的 config 条目覆盖。
  ['steward-workspace-derived','<stewardWorkspaceRoot, default ~/Ruyi>/<slug(title)>[-N]','13k-steward-threads.js','STEWARD_WORKSPACE_DERIVE_NOTE','directory only / no schema (the workbench writes no file of its own inside it)','fsp.mkdir({recursive:true}); the workspaces[] row that records it goes through mutateConfig -> writeConfig','a collision with an existing NON-EMPTY directory takes the next -N suffix; an existing EMPTY directory is reused (reopening the same task must not grow a second empty shell); a same-named plain file or an unreadable entry skips to the next suffix; after 50 tries derivation gives up and the thread keeps the pre-117w defaultWorkspace fallback','slug capped at 64 characters; 50 collision suffixes per title; the workspaces[] table that records the row caps at 64 rows (117w-W1 4th cut, 01-config WORKSPACE_TABLE_CAP); a derivation that would overflow that cap is refused BEFORE the directory is created (13k stewardWorkspaceTableFull -> invalid_request/workspace_table_full), so the table never ends up with a directory it does not record','none: the path is written into the session head and never cached (127 2-ter: a prompt scheduler task derives ONE such folder on its first steward-on trigger through the same 13k primitives, remembers it as tasks-v1.json workdir and reuses it while its workspaces[] row survives, so a daily task never grows -2 folders or extra rows)','nothing rebuilds it - the files inside are user work products a thread produced, and the workbench never regenerates user content','the workbench creates the directory but owns nothing inside it: threads write there under their own permission and the workbench never reads it back, rewrites it or deletes it',EXTERNAL],
