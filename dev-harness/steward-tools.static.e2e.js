#!/usr/bin/env node
'use strict';
// 静态锁:第 116 波 116c(27 号文 §3.5 工具面 / §11.3 第 49 波入库门)—— 管家 19 个工具的
...
//      四个表的 steward_* 键集必须【完全相同】且恰好是这 19 个名字(任何一处漏登 = 锁红)。
...
ok(JSON.stringify(schemaNames) === JSON.stringify(expected), `① 13f schema 恰好登记 19 个 steward_*(got ${schemaNames.length})`);
ok(JSON.stringify(regNames) === JSON.stringify(expected), `① 12 TOOL_HANDLERS 恰好登记 19 个 steward_*(got ${regNames.length})`);
ok(JSON.stringify(tierNames) === JSON.stringify(expected), `① 07 NATIVE_TOOL_TIER 恰好登记 19 个 steward_*(got ${tierNames.length})`);
ok(JSON.stringify(packNames) === JSON.stringify(expected), `① 07 NATIVE_TOOL_PACKS 恰好登记 19 个 steward_*(got ${packNames.length})`);
ok(Object.keys(srv.TOOL_HANDLERS).length === 82, `① 注册表总数 82(63 + 19;116-2b 增 steward_thread_note;got ${Object.keys(srv.TOOL_HANDLERS).length})`);
...
ok(offeredSteward.length === 19, `⑤ 回环:管家会话 buildOpenAiTools 拿到 19 个 steward_*(got ${offeredSteward.length})`);
