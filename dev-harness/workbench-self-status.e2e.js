...
//   (b) counts.nativeTools === 107(注册表工具数;129b 增三张只读清单 97→100,129d 增眼睛四件 100→104,129e 增一件 104→105,129f 增 steward_notify 105→106,137 代理模式 v2 增 agent_result 106→107;第 116 波 116c 起 63→80,新增 17 个 steward_* 管家工具;123-M2 增六件定时任务 90→96;
...
(async () => {
  ...
  try {
    ...
    // ── (a)(b)(c) 默认(section 省略 = all)──
    {
      ...
      ok(r && r.counts && r.counts.nativeTools === 107, `counts.nativeTools === 107(got ${r && r.counts && r.counts.nativeTools})`);
      ...
      ok(r && r.counts && r.counts.nativeTools === require(path.join(ROOT, 'facts.json')).nativeTools,
        `counts.nativeTools 与 facts.json 同源(${r && r.counts && r.counts.nativeTools} vs ${require(path.join(ROOT, 'facts.json')).nativeTools})`);
    ...
    // ── (e) section:'health' / 'counts' / 'config' 各自只带对应段 ──
    {
      ...
      ok(rc && rc.counts && rc.counts.nativeTools === 107, `section:'counts' 含 counts.nativeTools=107`);
