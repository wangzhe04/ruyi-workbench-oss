...
//   (b) counts.nativeTools === 89(注册表工具数;第 116 波 116c 起 63→80,新增 17 个 steward_* 管家工具;
...
(async () => {
  ...
  try {
    ...
    // ── (a)(b)(c) 默认(section 省略 = all)──
    {
      ...
      ok(r && r.counts && r.counts.nativeTools === 89, `counts.nativeTools === 89(got ${r && r.counts && r.counts.nativeTools})`);
    ...
    // ── (e) section:'health' / 'counts' / 'config' 各自只带对应段 ──
    {
      ...
      ok(rc && rc.counts && rc.counts.nativeTools === 89, `section:'counts' 含 counts.nativeTools=89`);
