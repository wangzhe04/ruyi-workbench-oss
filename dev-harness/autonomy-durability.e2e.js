...
(async () => {
  ...
  // ── B) 静态锁 ──
  {
    ...
    // 硬不变量:全文件 " + '.tmp'" 只允许 3 处豁免 —— ①atomicWriteJson 自身的唯一名构造;②flushSessionIndexSync
    // (exit 监听器只能同步 I/O,但 tmp 名同样唯一);③检查点回滚的二进制内容恢复(非 JSON,单写者语义)。
    // 任何人新增第 4 处手写 tmp 写点 → 此断言红,逼着走 atomicWriteJson。
    const tmpSites = (src.match(/\+ '\.tmp'/g) || []).length;
    ok(tmpSites === 3, 'B 手写 tmp 写点=3(白名单豁免;实 ' + tmpSites + ')');
