...
ok(/for \(const id of \(config\.knownModels \|\| \[\]\)\) if \(id\) ids\.push\(String\(id\)\);/.test(src) && /\.filter\(id => !MODEL_PRESET_IDS\.has\(id\) && !claudeCacheIds\.has\(id\)\)/.test(src), 'S tier 池拓宽 knownModels/config.model 且排除 Claude 别名+代理缓存(修 provider.models=[] 静默失效;第44波缓存同属 Claude 端)');
...
ok(/MODEL_PRESET_IDS\.has\(m\.id\) \|\| claudeCacheIds\.has\(m\.id\)/.test(src), 'S buildModelHint Claude 组=别名∪代理缓存');
...
ok(off.some(m => m.id === '') && off.some(m => m.id === 'opus') && off.some(m => m.id === 'sonnet') && off.some(m => m.id === 'haiku'), 'P 离线列表保留 默认+三别名(CLI 内建解析)');
