...
ok(/estimatedTurns/.test(src) && /轮估算/.test(src), '④ estimatedTurns>0 时标注「含 N 轮估算」');
ok(/costsByCurrency/.test(src), '④ 渲染 costsByCurrency（按币种分组，不换算）');
ok(/成本为按量计价的等价估算/.test(src) && /非实际扣费/.test(src), '④ 成本用「估算/非实际扣费」诚实措辞');
ok(/'约 '/.test(src), '④ 金额带「约」前缀（fmtCostsByCurrency prefix）');

// ───────────── ⑤ 分组条：手绘 SVG + 引擎双色 ─────────────
ok(/function usageGroup\(/.test(src), '⑤ 定义 usageGroup（按引擎/服务商/会话）');
ok(/'按引擎'/.test(src) && /'按服务商'/.test(src) && /'按会话'/.test(src), '⑤ 三种分组标题齐备');
...
ok(/'计划内计费'/.test(src), '⑧ 计划内条目标注「计划内计费」');
ok(/sourceLabel\s*\|\|\s*(entry\.)?source/.test(src), '⑧ 显示来源名(sourceLabel/source)');
ok(/计划内（订阅）计费/.test(src), '⑧ 计划内不伪造金额（订阅计费说明）');
...
ok(/还没有用量记录，发起对话后这里会汇总花费/.test(src), '⑪ 空状态引导文案');
