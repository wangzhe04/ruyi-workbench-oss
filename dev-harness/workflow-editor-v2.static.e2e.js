...
ok(/t\('workflow\.canvas\.currentEffect\.node'\)/.test(src) &&
  /t\('workflow\.canvas\.currentEffect\.role'\)/.test(src) &&
  /t\('workflow\.canvas\.currentEffect\.global'\)/.test(src) &&
  zh['workflow.canvas.currentEffect.node'].startsWith('当前生效：节点指定') &&
  zh['workflow.canvas.currentEffect.role'].startsWith('当前生效：角色默认') &&
  zh['workflow.canvas.currentEffect.global'].startsWith('当前生效：全局默认'), 'A1 有效继承链 hint 三态（节点/角色/全局）');
...
ok(/\['cross_review',t\('workflow\.gate\.crossReview'\)\]/.test(src) &&
  /\['dedupe',t\('workflow\.gate\.dedupe'\)\]/.test(src) &&
  zh['workflow.gate.crossReview'] === 'cross_review 交叉审查' &&
  zh['workflow.gate.dedupe'] === 'dedupe 去重', 'A2 质量门下拉含五种模式');
...
ok(/t\('workflow\.nodeMaxIters'\)/.test(src) && /maxIters\.type='number'/.test(src) && /maxIters\.max='300'/.test(src) &&
  /maxIters/.test(zh['workflow.nodeMaxIters']), 'A2 迭代预算 maxIters（number 1-300，空=默认）');
...
ok(/t\('workflow\.nodeToolTier'\)/.test(src) &&
  /\['read',t\('workflow\.toolTier\.read'\)\]/.test(src) &&
  /\['exec',t\('workflow\.toolTier\.exec'\)\]/.test(src) &&
  /联网/.test(zh['workflow.toolTier.read']) && /全量/.test(zh['workflow.toolTier.exec']),
  'A2 工具权限 toolTier 下拉（继承/read/edit/exec;第22波文案含联网）');
...
ok(/minSuccessfulToolCalls/.test(src) && /t\('workflow\.nodeMinToolCalls'\)/.test(src) &&
  /成功工具调用证据/.test(zh['workflow.nodeMinToolCalls']), 'A2 事实探针可要求成功工具调用证据');
...
ok(/wf-canvas-controls/.test(src) && /t\('workflow\.canvas\.fitView'\)/.test(src) &&
  zh['workflow.canvas.fitView'] === '适应视图', 'B2 右下角悬浮控件组「适应视图」');
ok(/if\(!draft\.nodes\.length\)\{const guide=el\('div','wf-canvas-empty'\)/.test(src), 'B2 空画布引导浮层 .wf-canvas-empty');
ok(/t\('workflow\.canvas\.fromTemplate'\)/.test(src) && /t\('workflow\.canvas\.addFirstNode'\)/.test(src) &&
  zh['workflow.canvas.fromTemplate'] === '从模板开始' &&
  zh['workflow.canvas.addFirstNode'] === '＋ 添加第一个节点', 'B2 空态两按钮引导（模板 / 加首节点）');
...
ok(/connectBtn/.test(src) && /t\('workflow\.editor\.connect'\)/.test(src) &&
  zh['workflow.editor.connect'] === '连接箭头', 'C1 保留既有「连接箭头」按钮流程（触屏兜底）');
...
ok(/t\('workflow\.canvas\.cycleDeps'\)/.test(src) && /let cyc=false/.test(src) &&
  /环依赖/.test(zh['workflow.canvas.cycleDeps']), 'C2 环依赖 DFS 检测');
ok(/t\('workflow\.canvas\.missingDep'\)/.test(src) &&
  /t\('workflow\.canvas\.emptyTask'\)/.test(src) &&
  /t\('workflow\.canvas\.duplicateNodeId'\)/.test(src) &&
  /依赖不存在/.test(zh['workflow.canvas.missingDep']) &&
  /任务为空/.test(zh['workflow.canvas.emptyTask']) &&
  /节点 ID 重复/.test(zh['workflow.canvas.duplicateNodeId']), 'C2 覆盖悬空依赖 / 空任务 / 重复 ID');
...
ok(/wf-problem-chip/.test(src) && /t\('workflow\.canvas\.issueCount'\)/.test(src) &&
  /个问题/.test(zh['workflow.canvas.issueCount']), 'C2 工具栏「⚠ N 个问题」chip');
