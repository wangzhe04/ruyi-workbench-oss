...
ok(has(elig, "(node.engine || 'openai') === 'claude'", 'Claude 引擎节点为单发进程，暂不支持中途插话'), 'C claude 引擎禁用 + 409 文案一致');
ok(has(elig, "['vote', 'dedupe'].includes(node.gate.mode)", '确定性质量门节点不经过模型，无法插话'), 'C 确定性门禁用 + 409 文案一致');
ok(has(elig, "['running', 'queued', 'waiting_resource'].includes(node.status)", '节点已结束，无法插话'), 'C 终态禁用 + 409 文案一致');
ok(/function wbSteerBox\(/.test(src), 'C 插话框 wbSteerBox 存在');
const steerBox = fnBody('wbSteerBox');
ok(has(steerBox, 'steerAgentNode(run.id, node.id, node.status, t)'), 'C 内联提交复用 steer_node action(传 presetText 不弹 prompt)');
ok(has(steerBox, 'input.disabled = true', 'send.disabled = true', 'wb-steer-why'), 'C 不符合资格显禁用输入 + 原因');
// steerAgentNode 已支持 presetText 双路径(prompt 兜底不破坏右栏 agent-runs 的 3 参调用)。
ok(/async function steerAgentNode\(runId, nodeId, nodeStatus, presetText\)/.test(src), 'C steerAgentNode 增 presetText 形参(内联/prompt 双路径)');
...
ok(has(pool, '谁提议', '做什么', '预计消耗'), 'D 三行人话卡(谁提议/做什么/预计消耗)');
...
ok(has(mail, 'm.dropped', '未送达', 'm.deliveredAt'), 'E dropped 项标「未送达」+ 已送达时间');
