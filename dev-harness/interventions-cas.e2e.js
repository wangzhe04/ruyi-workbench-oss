...
(async () => {
  ...
  try {
    ...
    for (const [at, diskStatus, recoveryStatus] of WINDOWS) {
      ...
      const tokenBeforeRestart = token; // 117q-P1-32:重启前留旧值,供下面等 token 真的换过再用
      ...
      // 旧值不同,不然带旧 token 的 cas()/waitForIv 后续请求会全数 403。
      token = await waitTokenRotated(tokenBeforeRestart);
