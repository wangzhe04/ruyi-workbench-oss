...
- **右侧线程栏里点别的页签会被拽回来（发布前热修，回归里的偶发件追出来的）**：同一件事下有几条线程时，宽屏右侧那一栏上方有一排页签。以前点别的页签能切过去，但看板下一次刷新（定时刷新、线程有动静、或者你切到别的窗口再切回来）就会把它**拽回原来那一条** —— 页签只换了右栏、没告诉看板。现在点页签（或用 ←／→ 在页签间走）走的是与「打开」同一条聚焦通道，切过去就留在那里。
...
- **Clicking another tab in the right-hand thread panel snapped back (pre-release hotfix, traced from a flaky regression test)**: when one item has several threads, the wide-screen right-hand panel shows a row of tabs. Clicking another tab switched to it, but the board's next refresh (the periodic refresh, activity on a thread, or switching to another window and back) **pulled it back to the previous thread** — the tab only changed the panel and never told the board. Clicking a tab (or moving between tabs with ←/→) now goes through the same focus channel as "Open", so the switch sticks.
