...
[![Offline e2e](https://img.shields.io/badge/%E7%A6%BB%E7%BA%BF%20e2e-313-success.svg)](./dev-harness)
...
| **8 套模板 · 9 种角色 · 320 项 e2e** | 内置 8 套多 Agent 工作流模板与 9 种节点角色。当前默认回归 313 项，另有 7 项需真实 API/桌面环境的 live probe 按需启用；另含 33 组 unit suite 与 16 组 ACC smoke |
...
> 每项功能均经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。当前共 320 项 e2e：默认运行 313 项，另有 7 项真实 API/桌面环境 probe 按需启用；另含 33 组 unit suite。迭代与发布规则见 [优化路线图](docs/OPTIMIZATION-ROADMAP.md)；面向用户的发行摘要见 [`CHANGELOG.md`](./CHANGELOG.md)。
