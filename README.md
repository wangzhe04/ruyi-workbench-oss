...
> **当前稳定技术版本：`v2.8.0`。** 这是「会守时、说得准、听得懂」版本：**把活交给时钟**（定时任务，含档位与固定文件夹）、**说的话追得到出处**（验收带来源标签、委托书看得见、「已发送」只由回执驱动）、**喊停就是喊停 / 失败说得出原因**、**长会话不再把刚才那段丢掉**（三个上下文压缩开关默认打开，升级会一次性为你打开）、**可以用说话代替打字**（语音输入，两种转写协议）、**管家能按规则替你批**命中永久豁免的动作。含第 123–127 波全部交付，加上第 107 波发布批准点里改出来的三处本机泄密面、管家代批的十道闸与若干缺陷修正。上一版「一台两视」（第 121–122 波）的能力全部保留。
...
| **8 套模板 · 9 种角色 · 395 项 e2e** | 内置 8 套多 Agent 工作流模板与 9 种节点角色。当前默认回归 381 项，另有 7 项需真实 API/桌面环境的 live probe 按需启用；另含 72 组 unit suite 与 17 组 ACC smoke |
...
> 每项功能均经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。当前共 395 项 e2e：默认运行 381 项，另有 7 项真实 API/桌面环境 probe 按需启用；另含 72 组 unit suite。迭代与发布规则见 [优化路线图](docs/OPTIMIZATION-ROADMAP.md)；面向用户的发行摘要见 [`CHANGELOG.md`](./CHANGELOG.md)。
...
├── dev-harness/                  验证脚手架(395 项 e2e,默认 381 项,Node 直跑)
├── docs/
