...
[![Offline e2e](https://img.shields.io/badge/%E7%A6%BB%E7%BA%BF%20e2e-336-success.svg)](./dev-harness)
...
> **3.0 预览版：`v3.0.0-preview.2`（2026-09-25，GitHub pre-release）**——第 128 波（偿债波）到第 137 波：子代理只把交付结果带回主会话、可后台运行；从 Claude Code／Codex／Kimi 与老版本如意一键迁移；各引擎都有如意运行环境说明、随包 ripgrep 真正可用；管家界面重设计、设置页重组；管家能自己查资料、读授权文件、按预置流程交办；语音两遍走（边说边出字＋句尾改错）；「等你处理」队列与线程内后台任务条；Mermaid 图随包。自动化全量回归与离线包冒烟已过，人工终验（安全红队、真读屏与人因走查）留到正式 3.0 前，详见 [CHANGELOG](CHANGELOG.md) 与 [55 号文](docs/optimization-plan/55-release-3.0-preview.md)。
>
> **上一个稳定技术版本：`v2.8.0`。** 这是「会守时、说得准、听得懂」版本：**把活交给时钟**（定时任务，含档位与固定文件夹）、**说的话追得到出处**（验收带来源标签、委托书看得见、「已发送」只由回执驱动）、**喊停就是喊停 / 失败说得出原因**、**长会话不再把刚才那段丢掉**（三个上下文压缩开关默认打开，升级会一次性为你打开）、**可以用说话代替打字**（语音输入，两种转写协议）、**管家能按规则替你批**命中永久豁免的动作。含第 123–127 波全部交付，加上第 107 波发布批准点里改出来的三处本机泄密面、管家代批的十道闸与若干缺陷修正。上一版「一台两视」（第 121–122 波）的能力全部保留。
...
| **107 个原生工具 · 108 个 ACC 工具** | 文件/终端/搜索/Git/联网/编排等原生工具按实际 `TOOL_HANDLERS` 可达全集计 107 个；可选 ACC 提供截图/OCR/UIA/键鼠/窗口/Office/PDF/编辑/抓取/记忆等 108 个工具。外部 MCP 另行按连接器计数，不再混入 ACC 数字 |
| **8 套模板 · 9 种角色 · 409 项 e2e** | 内置 8 套多 Agent 工作流模板与 9 种节点角色。当前默认回归 402 项，另有 7 项需真实 API/桌面环境的 live probe 按需启用；另含 76 组 unit suite 与 17 组 ACC smoke |
...
> 每项功能均经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。当前共 409 项 e2e：默认运行 402 项，另有 7 项真实 API/桌面环境 probe 按需启用；另含 76 组 unit suite。迭代与发布规则见 [优化路线图](docs/OPTIMIZATION-ROADMAP.md)；面向用户的发行摘要见 [`CHANGELOG.md`](./CHANGELOG.md)。
...
├── dev-harness/                  验证脚手架(409 项 e2e,默认 402 项,Node 直跑)
├── docs/
