...
| **8 套模板 · 9 种角色 · 392 项 e2e** | 内置 8 套多 Agent 工作流模板与 9 种节点角色。当前默认回归 381 项，另有 7 项需真实 API/桌面环境的 live probe 按需启用；另含 70 组 unit suite 与 16 组 ACC smoke |
...
> 每项功能均经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。当前共 392 项 e2e：默认运行 381 项，另有 7 项真实 API/桌面环境 probe 按需启用；另含 70 组 unit suite。迭代与发布规则见 [优化路线图](docs/OPTIMIZATION-ROADMAP.md)；面向用户的发行摘要见 [`CHANGELOG.md`](./CHANGELOG.md)。
...
├── dev-harness/                  验证脚手架(392 项 e2e,默认 381 项,Node 直跑)
├── docs/
