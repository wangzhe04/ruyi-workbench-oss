...
> **当前发布线：Ruyi Escapade 2.4**（技术版本 `v2.4.0`）。Escapade 是整个 2.x 系列的产品代号；后续修订仍沿用这个名字。内部的「第 N 波」只用于拆分开发计划，绝不直接充当用户版本号。
...
| **8 套模板 · 9 种角色 · 191 项离线 e2e** | 内置 8 套多 Agent 工作流模板与 9 种节点角色;每项功能经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。默认运行 185 项离线 e2e，另有 6 项需真实外部环境的探针按需启用 |
...
> 每项功能均经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。当前共 191 项离线 e2e（默认运行 185 项），迭代与发布规则见 [优化路线图](docs/OPTIMIZATION-ROADMAP.md)；面向用户的发行摘要见 [`CHANGELOG.md`](./CHANGELOG.md)。
