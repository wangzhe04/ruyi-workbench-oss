...
> **3.0 预览版：`v3.0.0-preview.1`（2026-09-24，GitHub pre-release）**——第 128 波（偿债波）到第 136 波：管家能自己查资料、读授权文件、按预置流程交办；语音两遍走（边说边出字＋句尾改错）；「等你处理」队列与线程内后台任务条；Mermaid 图随包。自动化全量回归与离线包冒烟已过，人工终验（安全红队、真读屏与人因走查）留到正式 3.0 前，详见 [CHANGELOG](CHANGELOG.md) 与 [55 号文](docs/optimization-plan/55-release-3.0-preview.md)。
>
...
| **106 个原生工具 · 108 个 ACC 工具** | 文件/终端/搜索/Git/联网/编排等原生工具按实际 `TOOL_HANDLERS` 可达全集计 106 个；可选 ACC 提供截图/OCR/UIA/键鼠/窗口/Office/PDF/编辑/抓取/记忆等 108 个工具。外部 MCP 另行按连接器计数，不再混入 ACC 数字 |
| **8 套模板 · 9 种角色 · 402 项 e2e** | 内置 8 套多 Agent 工作流模板与 9 种节点角色。当前默认回归 395 项，另有 7 项需真实 API/桌面环境的 live probe 按需启用；另含 74 组 unit suite 与 17 组 ACC smoke |
...
> 每项功能均经「实现 → 多视角对抗验证 → 修复 → 独立回归」闭环交付。当前共 402 项 e2e：默认运行 395 项，另有 7 项真实 API/桌面环境 probe 按需启用；另含 74 组 unit suite。迭代与发布规则见 [优化路线图](docs/OPTIMIZATION-ROADMAP.md)；面向用户的发行摘要见 [`CHANGELOG.md`](./CHANGELOG.md)。
...
├── dev-harness/                  验证脚手架(402 项 e2e,默认 395 项,Node 直跑)
├── docs/
