...
本目录下的 `ai-computer-control/` 是**随发行包捆绑的内置桌面控制 MCP**（97 工具：截图、OCR、UIA 定位、鼠标键盘、窗口、文件、Office/PDF 等）。它由工作台的**专用桌面探测**（`desktopMcp` 探测）直接识别并优先加载，**不需要、也不要**给它加 `ruyi-mcp.json` 清单——否则会与专用探测形成**双注册**（同一批工具被登记两次）。
