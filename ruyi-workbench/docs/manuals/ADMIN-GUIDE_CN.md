...
- **存储**：存储管理面板（各仓占用/保留策略/手动清理）与性能指标趋势；部署诊断（引擎路径、git/rg/python 探测、overlay 完整性、`/health` 等）已迁入「设置」面板。

存储面板支持各仓库占用分析、保留策略配置与手动清理；`GET /api/metrics`（需 token）可拉取进程内存、请求耗时分布（P50/P95/P99）与存储趋势等性能观测指标，供运维监控集成。
