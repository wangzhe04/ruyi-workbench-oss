...
2026-09-18 用真模型逐开关跑过配对 A/B（`deepseek-v4-flash` @ `api.deepseek.com`，每开关 **6 对**、111c 共 18 对，窗口等比缩小到 30000 以控成本，进程内直调产品自己的压缩原语、开关在产品自己的判定函数上把门）。**依据这批读数，2.8.0 把其中三个翻成了出厂默认开**（`runtimeSummaryPromptI18nV1`／`runtimeHistoryReadDedupV1`／`runtimeReseedTailUnitsV1`），另外两个留在实验档不动。

> **升级会改盘上的配置（已拍板，照实说）**：光翻出厂默认对存量安装是无效的 —— `normalizeConfig` 里 `config.json` 的值永远压过默认值，而首次读配置就会把整份配置回写，于是所有装过 2.7.0 的机器盘上都实打实写着 `false`。所以 2.8.0 把 `CONFIG_SCHEMA` 抬到 **12**，并带一次性迁移：`configSchema < 12` 的配置里，这三个键**即便你此前显式关过，也会被打开一次**。**2.8.0 之后你再关掉的，升级不会再动它**（迁移只按 `< 12` 触发一次）。不想要就在升级后把对应键写回 `false`。

| 开关 | 是什么 | 门与实测 | 结论 |
|---|---|---|---|
| `runtimeSummaryPromptI18nV1` | 摘要 prompt 双语（`locale` 是 en-US 时用英文那份） | 门「EN 校验通过率 ≥ ZH」达标但**不辨别**（三臂都 1.000）。真正的差别是**摘要语言**：关臂 **6/6** 给英文界面的用户发中文摘要，开臂 6/6 英文；事实保留 **10/10** 对关臂 9.33，墙钟 10.6 s 对 19.6 s，费用 ¥0.132 对 ¥0.167 | **已默认开（2.8.0 起）**（修的是实打实的缺陷，各项非劣信号都不差）。代价只有一条：`locale=en-US` 的用户从此拿到英文摘要（`auto` 与 `zh-CN` 逐字节不变）。**怎么关**：配置里显式写 `runtimeSummaryPromptI18nV1: false` |
| `runtimeHistoryReadDedupV1` | 受保护尾部里同一份文件的多份全文换成指针 | 门「释放 token +10%」**达标且远超**：旧边界子案 **+70.0%**（7076 → 12030），与 111a 同开的子案 **+40.8%**（8090 → 11393）；B 类非劣 12/12 都答出被指针指走的那两个事实 | **已默认开（2.8.0 起）**。**怎么关**：配置里显式写 `runtimeHistoryReadDedupV1: false` |
| `runtimeReseedTailUnitsV1` | L2 尾部按完整单元（assistant＋其全部 tool 回复）保留 | 门「空尾比例 −80%」实测 **−100%**：关臂 11 次重播种 **11 次尾部为空**，开臂 21 次 **0 次**；配对孤儿两臂都是 0 | **已默认开（2.8.0 起）**，**费用代价照实写**：L2 次数 1.83 → 3.50，摘要费用 **+60%**（¥1.681 → ¥2.687／6 次配对）。**怎么关**：配置里显式写 `runtimeReseedTailUnitsV1: false` |
...
上表后两个（`runtimeEvaporateBudgetBoundaryV1`／`runtimeReseedReattachFilesV1`）**仍是出厂默认关**，升级不碰它们。
