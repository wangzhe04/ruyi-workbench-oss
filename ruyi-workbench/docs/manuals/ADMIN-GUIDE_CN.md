...
在工作台 UI 顶栏点击「引擎」切到 Claude CLI 即可使用。模型通过 [Ark 控制台](https://console.volcengine.com/ark/region:ark+cn-beijing/openManagement)切换（设为 `ark-code-latest` 时），或在工作台设置 → Claude CLI → 模型中指定具体模型名（如 `doubao-seed-2.0-code`）。
...
| 401 认证失败 | 确认用的是 `ANTHROPIC_AUTH_TOKEN` 而非 `ANTHROPIC_API_KEY`；确认 API Key 以 `ark-` 开头且未过期 |
| `mcp add-json` 报 Invalid input | 改用 `claude mcp add`（非 JSON 版），见上方第五步 |
| 工作台 spawn 的 CLI 仍走官方端点 | `setx` 设置的环境变量只对新进程生效；重启工作台（或启动工作台的终端）使其继承新环境变量 |
| settings.json 覆盖了环境变量 | 检查 `~/.claude/settings.json` 中 `env.ANTHROPIC_BASE_URL`，若指向官方则改为 Ark 端点 |
| CLI 走了 Bedrock / Vertex 而非 Ark | 检查是否设了 `CLAUDE_CODE_USE_BEDROCK=1` 或 `CLAUDE_CODE_USE_VERTEX=1`，有则删除 |
| `claudePath` 探测到错误的 CLI | 检查是否设了 `CLAUDE_CLI_PATH` 环境变量（探测优先级最高），有则删除或改为正确路径 |

**环境变量干扰与优先级（维护必读）**

工作台 spawn Claude CLI 子进程时，环境变量传播链路为：`用户级环境变量(setx) → 启动终端 → workbench 进程 → { ...process.env } → Claude CLI 子进程`（[server.js](../../app/server.js) 中 `const env = { ...process.env, WIN_CLAUDE_WORKBENCH_HOME: paths.data }`）。即**所有用户级环境变量都会原样继承给 CLI 子进程**，workbench 仅额外注入 `WIN_CLAUDE_WORKBENCH_HOME` 和（如有配置）`MAX_THINKING_TOKENS` / `WCW_PERMISSION_TIMEOUT_MS`。

以下是可能干扰 Ark 端点配置的用户级环境变量，**修改或排查时必须逐一检查**：

**高风险（直接覆盖端点 / 认证）**

| 环境变量 | 影响 | 检查命令 |
|---|---|---|
| `CLAUDE_CODE_USE_BEDROCK` | 设为 `1` 时 Claude CLI 改走 AWS Bedrock，**完全忽略** `ANTHROPIC_BASE_URL` | `[Environment]::GetEnvironmentVariable('CLAUDE_CODE_USE_BEDROCK','User')` |
| `CLAUDE_CODE_USE_VERTEX` | 设为 `1` 时改走 Google Vertex AI，同上 | `[Environment]::GetEnvironmentVariable('CLAUDE_CODE_USE_VERTEX','User')` |
| `ANTHROPIC_API_KEY` | 与 `ANTHROPIC_AUTH_TOKEN` 同时存在时，CLI 可能用错认证方式（Ark 要求 Bearer Token，不是 `x-api-key` 头） | `[Environment]::GetEnvironmentVariable('ANTHROPIC_API_KEY','User')` |
| `~/.claude/settings.json` → `env.ANTHROPIC_BASE_URL` | settings.json 优先级**高于**环境变量，若指向 `https://api.anthropic.com` 会覆盖 Ark | `Test-Path "$env:USERPROFILE\.claude\settings.json"` |
