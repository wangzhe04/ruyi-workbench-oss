...
| `POST /api/audio/transcribe` | **token** | 语音转写。**本版唯一的新出网面**:25 MB 专用闸(Content-Length 预检 ＋ 流式累计双道)、120 s 超时、按 `providers[].asrProtocol` 选的协议打 `audioBaseUrl \|\| baseUrl` —— 缺省 `transcriptions`(OpenAI 形 multipart 打 `/audio/transcriptions`),或 `chat-audio`(`/chat/completions` ＋ `input_audio` data URI,MiMo/百炼的文档协议);记账 `kind:'aux', note:'asr'`,上游不报 usage 时保守估算并标 `estimated:true` |
...
- **语音转写(第 127 波新增,tier exec —— 用户文件出网)**:`audio_transcribe{path, language?}`,与 `file_read` 同一道路径闸 → 扩展名白名单 → 25 MB 闸 → 读盘 → 经 `05-claude-engine` 的共享出站体打服务商(协议按 `providers[].asrProtocol`,见上表;送的是用户原文件,不转码)。返回值带 **`untrusted:true`**(转写文本一律不可信);未配置／越界／超限／上游失败一律规整成 `ok:false`,不抛。
...
- **子代理(v0.9-S6 新增,tier exec,provider 引擎独占)**:`spawn_agent{task, toolTier?='read', maxIters?=6, model?, background?=false}`——每批任务统一创建持久化 Agent Run 并映射为工作台 DAG；默认同步返回 `{ok, result:子结论, iters, toolCalls}`，`background:true` 则立即返回 `{runId,nodeId,status:'running'}`，父 Agent 可继续推进独立工作，之后以 `wait_agents{runIds?}` 汇总结果。**只在 `runOpenAiTurn` 特判**(需回合闭包)，context-free `toolCall()` / CLI 面恒拒；子回合独立 history + 按 toolTier 过滤工具集 + 独立迭代预算，**禁嵌套、单批次并发 ≤2、`subagentMaxPerTurn` 兜底(0 停用)**；持久化运行沿用工作台的状态查看、取消与重启恢复能力(详见「引擎」节)。**52x**:`config.subagentPreferredProvider`/`config.subagentPreferredModel` 可跨 provider 指定子代理优先端点与模型。**52a**:`PROMPT_EN` 常量控制子代理系统提示词模板。
...
- **第54波有序回合协议**：Claude CLI 与 OpenAI 兼容引擎共用 `createTurnSegmentBuilder()`，把文字、思考、工具、子代理、计划、询问、权限、工作流、Mission 和错误写入 `message.segments`。工具段只引用 `toolCallId`，大输入/结果仍在 `message.toolCalls`；同一模型响应的工具共享 `batchId`，结果按 id 回填状态。`permission_decision`、`plan_decision` 与 `question_answer` 原位闭合待决片段，因此刷新后仍能复原允许/拒绝、批准/驳回、已回答等终态。兼容期旧字段继续写入；旧会话没有 `segments` 时不猜测历史顺序。
- UI:chat-pane 顶部可收起「步骤条」(`session.todos` 非空或流式收到 `todo` 事件时显示);assistant 回合用单一 `.turn-narrative` 按段渲染，尾部保留工具索引（可定位过程）、「本轮变更」摘要、产物和用量；空变更 + 0 命令显示「本次未改动任何文件」安抚线。静态重进由 `public/js/turn-narrative.js` 提供稳定 key、签名与滚动锚点，复用未变化消息 DOM；消息区以 `role="log"` 只播报新增/变化内容，尾部定位会把键盘焦点移回过程行。连续成功工具从第 4 项起折叠，运行中和失败项始终外露；明暗主题由无依赖浏览器截图网格回归锁定。
