...
> 版本基线:`app/server.js` `VERSION 1.6.1` / `configSchema 8` / 会话 `schemaVersion 1`。v1.6.1 增量为 OpenAI 兼容引擎与 Claude CLI 共用的工具目录、任务预路由、按需装载、目录缓存与 schema token 计量；`toolLoadingMode:'full'` 保留旧的全部常驻行为。其余架构延续 v1.5 团队模式、Skills、用量看板及 v0.7d–v1.6 的多引擎、检查点、Agent 工作流、桌面/Office MCP 桥接能力。
>
...
- `POST /api/steer {sessionId, text}`(v0.8-S7,**需 UI header token**):流中插话(steering)。**双引擎**(第47波 a 打通):provider 引擎把 `text` 入队到该会话**活跃 provider turn** 的 `reg.steerQueue`(上限 3),tool 循环在配对安全边界(迭代边界 drain + 第51波 b between-tools 批内中断)注入为 `[用户插话] …` user 消息。语义:无活跃 turn → `{ok:false, error:'当前没有进行中的回合'}`;队列满 → `{ok:false, error:'插话队列已满'}`;成功 → `{ok:true, queued:N, injected?:true}`(injected=true 表示 Claude 引擎已即时注入)。注入点见「引擎」节 steering 说明。
...
- **子代理(v0.9-S6 新增,tier exec,provider 引擎独占)**:`spawn_agent{task, toolTier?='read', maxIters?=6, model?}`——派独立子回合执行子任务,返回 `{ok, result:子结论, iters, toolCalls}`。**只在 `runOpenAiTurn` 特判**(需回合闭包),context-free `toolCall()` / CLI 面恒拒;子回合独立 history + 按 toolTier 过滤工具集 + 独立迭代预算,**禁嵌套、单批次并发 ≤2、`subagentMaxPerTurn` 兜底(0 停用)**(详见「引擎」节)。
...
## 团队模式 v2 与 Skills / 记忆 / 用量(v1.4→v1.5)
...
v1.5 团队模式(设计稿 `docs/TEAM-MODE-V2-DESIGN.md`)在此之上,均**复用既有节点管线、零新执行路径**:
...
## 中英双语系统提示词注册表(v2.0.0 `06b-prompt-registry.js`)

- `PROMPT_PACK_VERSION='2026-w51-1'`
- 系统提示词从 `i18n/prompt-packs/{zh|en}/` 按 UI 语言按需加载，技能文件夹不再承载系统级模板
- `02-session-store.js` 新增 `promptPack` 字段持久化当前语言包版本
- 注入时按 UI 语言选择对应模板（zh → `PROMPT_ZH`，en → `PROMPT_EN`）
