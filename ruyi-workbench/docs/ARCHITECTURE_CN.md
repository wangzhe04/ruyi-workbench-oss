...
- `POST /api/steer {sessionId, text}`(v0.8-S7,**需 UI header token**,**仅 provider 引擎**):流中插话(steering)。把 `text` 入队到该会话**活跃 provider turn** 的 `reg.steerQueue`(上限 3),tool 循环在下一个配对安全边界注入为 `[用户插话] …` user 消息。语义:无活跃 turn → `{ok:false, error:'当前没有进行中的回合'}`;活跃但非 provider 引擎(`reg.kind !== 'openai'`)→ `{ok:false, error:'仅 provider 引擎支持插话'}`;队列满 → `{ok:false, error:'插话队列已满'}`;成功 → `{ok:true, queued:N}`。注入点见「引擎」节 steering 说明。
